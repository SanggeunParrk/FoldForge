...
from foldforge.models.opendde.ported.model.modules.pairformer import PairformerStack
...
from foldforge.models.opendde.ported.model.utils import broadcast_token_to_atom, one_hot
from foldforge.models.opendde.ported.utils.torch_utils import cdist, cleanup_device_memory, disabled_autocast
                ...
                if compute_plddt or compute_resolved:
                    ...
                    plddt_pred = (
                        torch.einsum(
                            "...nc,ncb->...nb",
                            self.plddt_ln(a),
                            self.plddt_weight[atom_to_tokatom_idx].float(),
                        )
                        if compute_plddt
                        else None
                    )
                    resolved_pred = (
                        torch.einsum(
                            "...nc,ncb->...nb",
                            self.resolved_ln(a),
                            self.resolved_weight[atom_to_tokatom_idx].float(),
                        )
                        if compute_resolved
                        else None
                    )
