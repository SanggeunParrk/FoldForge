...
from foldforge.data.constants.chemistry import EvaluationChainInterface
from foldforge.data.cropping import CropData
from foldforge.data.features.atoms import get_antibody_clusters
from foldforge.data.features.atoms import (
    residue_data_type_transform as data_type_transform,
)
from foldforge.data.features.atoms import (
    residue_make_dummy_feature as make_dummy_feature,
)
from foldforge.data.features.constraints import ConstraintFeatureGenerator
from foldforge.data.features.featurizer import ResidueFeaturizer as Featurizer
from foldforge.data.io import read_indices_csv
from foldforge.data.msa.featurizer import MSAFeaturizer
from foldforge.data.pipeline import DataPipeline
from foldforge.data.template.featurizer import TemplateFeaturizer
from foldforge.utils.logging import get_logger
from foldforge.utils.tensor import dict_to_tensor
...
class BaseSingleDataset(Dataset):
    ...
    return a dict of features and labels, the keys and the shape are defined in
    protenix.data.utils
    """
    ...
    def __init__(
        self,
        mmcif_dir: str | Path,
        bioassembly_dict_dir: str | Path | None,
        indices_fpath: str | Path,
        cropping_configs: dict[str, Any],
        msa_featurizer: MSAFeaturizer | None = None,
        template_featurizer: TemplateFeaturizer | None = None,
        name: str | None = None,
        **kwargs: Any,
    ) -> None:
        super().__init__()
        ...
        self.pdb_list = kwargs.get("pdb_list")
        if not self.pdb_list:
            self.pdb_list = None
        # Used for removing rows in the indices list. Column names and excluded values
        # are specified in this dict.
        ...
        if self.constraint.get("enable", False):
            logger.info(
                "[%s] constraint config: %s", f"{self.name}", f"{self.constraint}"
            )
        ...
        self.error_dir = kwargs.get("error_dir")
    ...
    @staticmethod
    def read_pdb_list(pdb_list: list[str] | str) -> list[str]:
    ...
    def read_indices_list(  # noqa: C901, PLR0915 - ordered dataset filtering and feature assembly
        self, indices_fpath: str | Path
    ) -> pd.DataFrame | list[pd.DataFrame]:
        """Read and processes a list of indices from a CSV file.
        ...
        """
        indices_frame: pd.DataFrame = read_indices_csv(indices_fpath)
        num_data = len(indices_frame)
        self.check_indices_list(indices_frame, "initial loading")
        logger.info("#Rows in indices list: %s", f"{num_data}")
        # Filter by pdb_list
        if self.pdb_list is not None:
            pdb_filter_list = self.read_pdb_list(pdb_list=self.pdb_list)
            indices_frame = indices_frame[indices_frame["pdb_id"].isin(pdb_filter_list)]
            logger.info("[filtered by pdb_list] #Rows: %s", f"{len(indices_frame)}")
            self.check_indices_list(indices_frame, "pdb_list filtering")

        # Filter by max_n_token
        if self.max_n_token > 0:
            valid_mask = indices_frame["num_tokens"].astype(int) <= self.max_n_token
            removed_list = indices_frame[~valid_mask]
            indices_frame = indices_frame[valid_mask]
            logger.info("[removed] #Rows: %s", f"{len(removed_list)}")
            logger.info("[removed] #PDB: %s", f"{removed_list['pdb_id'].nunique()}")
            logger.info(
                "[filtered by n_token (%s)] #Rows: %s",
                f"{self.max_n_token}",
                f"{len(indices_frame)}",
            )
            self.check_indices_list(
                indices_frame, f"max_n_token ({self.max_n_token}) filtering"
            )

        if self.min_n_token > 0:
            valid_mask = indices_frame["num_tokens"].astype(int) >= self.min_n_token
            removed_list = indices_frame[~valid_mask]
            indices_frame = indices_frame[valid_mask]
            logger.info("[removed] #Rows: %s", f"{len(removed_list)}")
            logger.info("[removed] #PDB: %s", f"{removed_list['pdb_id'].nunique()}")
            logger.info(
                "[filtered by min_n_token (%s)] #Rows: %s",
                f"{self.min_n_token}",
                f"{len(indices_frame)}",
            )
            self.check_indices_list(
                indices_frame, f"min_n_token ({self.min_n_token}) filtering"
            )
        ...
        for col_name, exclusion_list in self.exclusion_dict.items():
            ...
            valid_mask = indices_frame.apply(is_valid, axis=1)
            indices_frame = indices_frame[valid_mask].reset_index(drop=True)
            logger.info(
                "[Excluded by %s -- %s] #Rows: %s",
                f"{col_name}",
                f"{exclusion_list}",
                f"{len(indices_frame)}",
            )
            self.check_indices_list(
                indices_frame, f"exclusion_dict ({col_name}) filtering"
            )
        self.print_data_stats(indices_frame)

        # Group by pdb_id
        # A list of dataframe. Each contains one pdb with multiple rows.
        indices_list: pd.DataFrame | list[pd.DataFrame] = indices_frame
        ...
        if self.sort_by_n_token:
            # Sort the dataset in a descending order, so that if OOM it will raise Error
            # at an early stage.
        ...
        if self.find_eval_chain_interface:
            # Remove data that does not contain eval_type in the
            # EvaluationChainInterface list
            if isinstance(indices_list, list):
                indices_list = [
                    df
                ...
                # indices_list[indices_list["eval_type"].apply(lambda x: x in
                # EvaluationChainInterface)]
                # .isin() checks each element of the Series against the set, same
                # semantics as the lambda.
        ...
        if self.limits > 0 and len(indices_list) > self.limits:
            logger.info(
                "Limit indices list size from %s to %s",
                f"{len(indices_list)}",
                f"{self.limits}",
            )
    ...
    def check_indices_list(
        self, indices_list: pd.DataFrame | list[pd.DataFrame], step_name: str = ""
    ) -> None:
        """Check indices list."""
    ...
    def print_data_stats(self, df: pd.DataFrame) -> None:
        """Compute print data stats.

        Print statistics about the dataset, including the distribution of molecular
        group types.
        ...
        """
        if self.name:
            logger.info("---------- Dataset %s----------", self.name)
        col1 = df["mol_1_type"].astype(str)
        col2 = df["mol_2_type"].astype(str).str.replace("nan", "intra", regex=False)
        lo = np.where(
            col1.to_numpy() <= col2.to_numpy(), col1.to_numpy(), col2.to_numpy()
        )
        hi = np.where(
            col1.to_numpy() <= col2.to_numpy(), col2.to_numpy(), col1.to_numpy()
        )
        ...
        if "cluster_id" in df.columns:
            ...
            for i in group_size_dict:
                ...
                logger.info(
                    ...
                    f"{n_i}",
                    f"{n_cluster}",
                    f"{round(n_i * 100 / n_cluster, 2)}",
                )
            logger.info("-" * 30)

        logger.info("Final pdb ids: %s", f"{len(set(df.pdb_id.tolist()))}")
        logger.info("-" * 30)
    ...
    def save_error_data(self, idx: int, error_message: str) -> None:
        ...
        Save the error data for a specific index to a JSON file in the error directory.
        ...
        if self.error_dir is not None:
            ...
            filename = (
                f"{sample_indice.pdb_id}-{sample_indice.chain_1_id}-"
                f"{sample_indice.chain_2_id}.json"
            )
            fpath = str(Path(self.error_dir) / (filename))
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        If an error occurs, it attempts to handle it by either saving the error data or
        randomly sampling another index.
        ...
        for _ in range(10):
            try:
                return self.process_one(idx=idx)
                ...
                if self.random_sample_if_failed:
                    logger.exception("[skip data %s] %s", f"{idx}", f"{error_message}")
                    # Random sample an index
                    idx = random.choice(range(len(self.indices_list)))
                    continue
                raise
        message = "Could not load a training sample after 10 attempts"
        raise RuntimeError(message) from last_error

    def _get_bioassembly_data(self, idx: int) -> tuple[pd.Series, dict[str, Any], str]:
        sample_indice = self._get_sample_indice(idx=idx)
        if self.bioassembly_dict_dir is not None:
            bioassembly_dict_fpath = str(
                Path(self.bioassembly_dict_dir) / (sample_indice.pdb_id + ".pkl.gz")
            )
        else:
            message = "A bioassembly directory is required to load training samples"
    ...
    @staticmethod
    def _reassign_atom_array_chain_id(atom_array: AtomArray) -> AtomArray:
        """Compute reassign atom array chain id.

        In experiments conducted to observe overfitting effects using training sets,.
        the pre-stored AtomArray in the training set may experience issues with
        discontinuous chain IDs due to filtering.
        Consequently, a temporary patch has been implemented to resolve this issue.

        e.g. 3x6u asym_id_int = [0, 1, 2, ... 18, 20] -> reassigned_asym_id_int [0, 1,
        2, ..., 18, 19]
        """

        def _get_contiguous_array(array: np.ndarray) -> np.ndarray:
    ...
    @staticmethod
    def _shuffle_array_based_on_mol_id(
    ...
    @staticmethod
    def _assign_random_sym_id(atom_array: AtomArray) -> AtomArray:
    ...
    def process_one(
        ...
        Process a single data sample by retrieving bioassembly data, applying various
        transformations, and cropping the data.
        It then extracts features and labels, and optionally returns the processed atom
        and token arrays.

        Args:
            idx: The index of the data sample to process.
            return_atom_token_array: Whether to return the processed atom and token
                arrays.

        Returns:
            A dict containing the input features, labels, basic_info and optionally the
    ...
    def crop(
        ...
        *,
        ...
        Returns:
            A tuple containing the cropping method, cropped token array, cropped atom
            array,
    ...
    def _get_sample_indice(self, idx: int) -> pd.Series:
        """Return sample indice.

        Retrieve the sample indice for a given index. If the dataset is grouped by PDB
        ID, it returns the first row of the PDB-idx.
        ...
        """
        if isinstance(self.indices_list, list):
            # Row-0 of PDB-idx
    ...
    def _get_pdb_indice(self, idx: int) -> pd.DataFrame:
        if isinstance(self.indices_list, list):
    ...
    def _get_eval_chain_interface_mask(
        ...
        """Retrieve the evaluation chain/interface mask for a given index.
        ...
        Returns:
            A tuple containing the evaluation type, cluster ID, chain 1 mask, and chain
            2 mask.
        ...
        if len(df) < 1:
            msg = "Cannot find a chain/interface for evaluation in the PDB."
            raise ValueError(msg)
        ...
        df["chain_1_mask"], df["chain_2_mask"] = zip(
            *df.apply(get_atom_mask, axis=1), strict=False
        )
        df = df[df["chain_1_mask"].notna()]  # drop NaN

        if len(df) < 1:
            msg = "Cannot find a chain/interface for evaluation in the atom_array."
            raise ValueError(msg)
    ...
    ) -> tuple[TokenArray, AtomArray, dict[str, Any], dict[str, np.ndarray], AtomArray]:
    ...
    def get_feature_and_label(  # noqa: C901, PLR0912, PLR0915 - ordered dataset filtering and feature assembly
        self,
        idx: int,
        token_array: TokenArray,
        atom_array: AtomArray,
        ...
        *,
        ...
        max_entity_mol_id: int | None = None,
    ) -> tuple[dict[str, Any], dict[str, Any], dict[str, torch.Tensor]]:
        """Get feature and label information for a given data point.

        It uses a Featurizer object to obtain input features and labels, and applies
        several
        steps to add other features and labels. Finally, it returns the feature
        dictionary, label
        ...
        Args:
            ...
            is_spatial_crop: Flag indicating whether spatial cropping is applied, by
                default True.
        ...
        labels_dict: dict[str, Any] = feat.get_labels()
        ...
        if self.find_pocket:
            ...
            if sample_indice.mol_1_type == "ligand":
                ...
                msg = "Cannot find ligand from this data point."
                raise ValueError(msg)
            # Make sure the cropped array contains interested ligand
            if lig_entity_id not in set(atom_array.label_entity_id):
                message = (
                    "Invalid state: lig_entity_id in set(atom_array.label_entity_id)"
                )
                raise ValueError(message)
            if lig_chain_id not in set(atom_array.chain_id):
                message = "Invalid state: lig_chain_id in set(atom_array.chain_id)"
                raise ValueError(message)
            ...
            if len(np.unique(lig_asym_id)) != 1:
                message = "Invalid state: len(np.unique(lig_asym_id)) == 1"
                raise ValueError(message)
            ...
            if self.find_all_pockets:
                ...
                ligands.extend(list(all_lig_asym_ids - {lig_asym_id}))
...
def get_msa_featurizer(
    configs: ConfigNode,
    dataset_name: str,
    stage: str,  # noqa: ARG001 - shared callback or fixture signature
) -> Callable | None:
    """Return msa featurizer.

    Create and returns an MSAFeaturizer object based on the provided configurations.
    ...
    Returns:
        An MSAFeaturizer object if MSA is enabled in the configurations, otherwise
        None.
...
def get_template_featurizer(
    configs: ConfigNode, dataset_name: str, stage: str
) -> Callable | None:
    """Return template featurizer."""
    template_info = configs["data"]["template"]
    template_args = deepcopy(template_info)
    template_args["dataset_name"] = dataset_name
    # If the dataset has special template settings, then overwrite the default template
    # settings
...
class WeightedMultiDataset(Dataset):
    ...
    Args:
        ...
        datapoint_weights: A list of lists containing sampling weights for each
            datapoint in the datasets.
        dataset_sample_weights: A list of torch tensors containing sampling weights for
            each dataset.
    ...
    def __init__(
        ...
        dataset_sample_weights: list[float],
    ) -> None:
    ...
    def init_datasets(self) -> None:
        ...
        for dataset_index, (
            _dataset,
            raw_datapoint_weight_list,
            dataset_weight,
        ) in enumerate(
            zip(
                self.datasets,
                self.datapoint_weights,
                self.dataset_sample_weights,
                strict=False,
            )
        ):
            # normalize each dataset weights
            datapoint_weight_list = raw_datapoint_weight_list
...
def get_weighted_pdb_weight(
    ...
    """Get sample weight for each example in a weighted PDB dataset.
        ...
        chain_count (dict): Count of each kind of chains, e.g., {"prot": int, "nuc":
            int, "ligand": int}.
        eps (float, optional): A small epsilon value to avoid division by zero. Default
            is 1e-9.
        beta_dict (Optional[dict], optional): Dictionary containing beta values for
            'chain' and 'interface'.
        alpha_dict (Optional[dict], optional): Dictionary containing alpha values for
    ...
    if not (cluster_size > 0):
        message = "Invalid state: cluster_size > 0"
        raise ValueError(message)
    if data_type not in ["chain", "interface"]:
        message = "Invalid state: data_type in ['chain', 'interface']"
        raise ValueError(message)
    beta = beta_dict[data_type]
    if not (set(chain_count.keys()).issubset(set(alpha_dict.keys()))):
        message = (
            "Invalid state: set(chain_count.keys()).issubset(set(alpha_dict.keys()))"
        )
        raise ValueError(message)
    return (
        beta
...
def calc_weights_for_df(
    ...
    Args:
        eps: Lower bound used when normalizing sampling weights.
...
def get_sample_weights(
    ...
    *,
    ...
    Args:
        ...
        beta_dict: A dictionary containing beta values for different data types.
        alpha_dict: A dictionary containing alpha values for different data types.
        force_recompute_weight: Whether to force recomputation of weights even if they
            already exist.
    ...
    if alpha_dict is None:
        alpha_dict = {"prot": 3, "nuc": 3, "ligand": 1}
    if beta_dict is None:
        beta_dict = {"chain": 0.5, "interface": 1}
    if sampler_type == "weighted":
        if not (indices_df is not None):
            message = "Invalid state: indices_df is not None"
            raise ValueError(message)
...
def get_datasets(
    configs: ConfigNode, error_dir: str | None
    ...
    def _get_dataset_param(
        config_dict: dict[str, Any], dataset_name: str, stage: str
    ) -> dict[str, Any]:
        # Template_featurizer is under development
        # Lig_atom_rename/shuffle_mols/shuffle_sym_ids do not affect the performance
        # very much
        return {
            "name": dataset_name,
    ...
    logger.info("Using train sets %s", f"{data_config.train_sets}")
    if len(data_config.train_sets) != len(
        data_config.train_sampler.train_sample_weights
    ):
        message = (
            "Invalid state: len(data_config.train_sets) == "
            "len(data_config.train_sampler.train_sample_weights)"
        )
        raise ValueError(message)
    train_datasets = []
    datapoint_weights = []
    for train_name in data_config.train_sets:
        ...
        if isinstance(train_dataset.indices_list, list):
            message = "Weighted training requires ungrouped sample indices"
