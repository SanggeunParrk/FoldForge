# ruff: noqa: PLC0415 - optional model dependencies at test boundaries
...
def oracle():
    ...
    for code in json.loads(
        (Path(__file__).parent / "reference_af_family/layout_oracles.json").read_text()
    ).values():
...
def test_af3_atom_layout_shape_contract():
    from foldforge.models.af3.ported.nn.atom_layout import GatherInfo, convert
...
def test_shared_layout_imports_are_live(family):
    ...
    records = json.loads(
        (Path(__file__).parents[1] / "docs/shared-layout-migration.json").read_text()
    )
    for relative, name in records:
        module = relative.removesuffix(".py").replace("/", ".")
        actual = importlib.import_module(f"foldforge.models.{family}.ported.{module}")
