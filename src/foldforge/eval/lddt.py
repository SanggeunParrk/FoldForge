...
def get_complex_level_rankers(scores, keys):
    """Return complex level rankers."""
    if not (all(k in ["plddt", "gpde", "ranking_score"] for k in keys)):
        message = (
            "Invalid state: all([k in ['plddt', 'gpde', 'ranking_score'] for "
            "k in keys])"
        )
    ...
    for key in keys:
        ...
        rank1 = ranking[0].item()
        rankers[f"{key}.rank1"] = lambda x, rank1_idx=rank1: x[..., rank1_idx]
    return rankers
...
class LDDTMetrics(nn.Module):
    ...
    def aggregate_lddt(self, lddt_dict, per_sample_summary_confidence):
        ...
        """Compute aggregate lddt."""
        confidence_scores = (
            sample_confidence.detailed_merge_per_sample_confidence_scores(
...
class LDDT(nn.Module):
    ...
    def _chunk_forward(
        self, pred_distance, true_distance, chunk_size: int | None = None
    ) -> torch.Tensor:
    ...
    def forward(
        ...
        sparse implementation, which largely reduce cuda memory when atom num reaches
        10^4 +
        ...
        Args:
            ...
            true_coordinate (torch.Tensor): the ground truth atom coordinates
                [N_atom, 3]
            lddt_mask (torch.Tensor):
                sparse version of [N_atom, N_atom] atompair mask based on bespoke
                radius of true distance
                [N_nonzero_mask, 2]
