...
from foldforge.models.opendde.ported.data.constants import STD_RESIDUES_WITH_GAP
from foldforge.models.opendde.ported.distributed.foldcp.comm import (
    detach_rank_local_error_traceback,
    run_group_rank_action_synchronized,
)
from foldforge.models.opendde.ported.distributed.foldcp.config import FoldCPConfig
from foldforge.models.opendde.ported.distributed.foldcp.launch import (
...
from foldforge.models.opendde.ported.distributed.foldcp.mesh import FoldCPProcessMesh
from foldforge.models.opendde.ported.distributed.foldcp.msa_pair_weighted import (
    distributed_msa_pair_weighted_average_with_full_value,
    gather_msa_rows_from_cp,
)
from foldforge.models.opendde.ported.distributed.foldcp.opm import (
    shard_msa_tensor_for_opm,
)
from foldforge.models.opendde.ported.distributed.foldcp.pair_sharding import (
    FoldCPPairShardSpec,
    gather_pair_tensor,
...
from foldforge.models.opendde.ported.distributed.foldcp.real_pairformer import (
...
from foldforge.models.opendde.ported.model.modules.primitives import (
    LinearNoBias,
    Transition,
)
from foldforge.models.opendde.ported.model.modules.transformer import AttentionPairBias
from foldforge.models.opendde.ported.model.msa_sampling import (
    subsample_msa_feature_dict_valid_first,
)
from foldforge.models.opendde.ported.model.triangular.layers import (
    LayerNorm,
    OuterProductMean,
)
from foldforge.models.opendde.ported.model.triangular.triangular import (
    TriangleAttention,
    TriangleMultiplicationIncoming,
    TriangleMultiplicationOutgoing,
)
from foldforge.models.opendde.ported.model.utils import (
    checkpoint_blocks,
    expand_at_dim,
    is_fp16_enabled,
)
from foldforge.models.opendde.ported.utils.torch_utils import disabled_autocast
from foldforge.modules.af_family import transition_residual
...
class PairformerBlock(nn.Module):
    ...
    def forward_source(
        ...
        if inplace_safe:
            ...
            z = z.transpose(-2, -3).contiguous()
            z = transition_residual(self.pair_transition, z)
        else:
        ...
        if self.c_s > 0:
            ...
            s = transition_residual(self.single_transition, s)
        return s, z
    ...
    def inference_forward(
        ...
        if foldcp_mesh is None:
            return msa_row_update(
                m, z, attention_delta=self.msa_pair_weighted_averaging,
                transition_residual=lambda value: transition_residual(self.transition_m, value),
                chunk_size=chunk_size,
            )
        num_msa = m.shape[-3]
...
class MSABlock(nn.Module):
    ...
    def __init__(
        ...
        self.msa_update_config = MSAUpdateConfig(order="msa_first", return_msa=not is_last_block)
        self.c_m = c_m
        self.c_z = c_z
    ...
    def forward(
        ...
        if self._maybe_foldcp_mesh() is None:
            def opm(value):
                if not torch.is_grad_enabled() and torch.are_deterministic_algorithms_enabled():
                    return self._deterministic_outer_product_mean_full_update(value, chunk_size=chunk_size)
                return self.outer_product_mean_msa(value, inplace_safe=inplace_safe, chunk_size=chunk_size)
            return msa_pair_update(
                m, z,
                config=self.msa_update_config,
                outer_product_delta=opm,
                msa_residual=self.msa_stack,
                pair_residual=lambda pair: self.pair_stack(
                    s=None, z=pair, pair_mask=pair_mask,
                    triangle_multiplicative=triangle_multiplicative,
                    triangle_attention=triangle_attention, inplace_safe=inplace_safe,
...
class TemplateEmbedder(nn.Module):
    ...
    def forward(
        ...
        u = template_embedding_mean(
            num_templates,
            embed=lambda template_id: self.single_template_forward(
                template_id=template_id, input_feature_dict=input_feature_dict,
                z=z, pair_mask=pair_mask, multichain_mask=multichain_mask,
                triangle_attention=triangle_attention,
                triangle_multiplicative=triangle_multiplicative,
                inplace_safe=inplace_safe, chunk_size=chunk_size),
            empty=z.new_zeros(*z.shape[:-1], self.c),
            project=self.linear_no_bias_u,
        )
