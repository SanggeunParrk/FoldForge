...
def report_stats(data: dict) -> None:
    """Report stats.

    Generate and print a formatted table showing the statistics for each namespace in
    the given data.

    Args:
        data (dict): A dictionary where keys are namespaces and values are dictionaries
            of statistics.

    """
    columns = [list(data[namespace].keys()) for namespace in data]
    columns = functools.reduce(operator.iadd, columns, [])
...
class MMCIFStatsSummary:
    ...
    def add_logger(self, pdbid: str) -> MMCIFStatsLogger:
        ...
        Add a new MMCIFStatsLogger instance to the logger dictionary for the given PDB
        ID.

        If a logger for the provided PDB ID already exists, a warning message will be
        printed,
        ...
        Returns:
            MMCIFStatsLogger: The newly created or overwritten logger instance for the
                given PDB ID.

        """
        if pdbid in self.logger_dict:
            logger.info(
                "%s", f"WARNING: {pdbid} data stats logger exist, new logger initiated."
            )
    ...
    def merge_data(self, data: dict) -> None:
        ...
        """
        for namespace, statistics in data.items():
            if namespace not in self.data:
                self.data[namespace] = {}
            for key in statistics:
                if key not in self.data[namespace]:
                    self.data[namespace][key] = 0
                self.data[namespace][key] += int(statistics[key] != 0)

    def report(self) -> None:
        """Report .

        Print a summary report of the statistics for all PDB IDs in the logger
        dictionary.

        This method generates a formatted table showing the statistics for each
        namespace
        across all PDB IDs, and prints it to the console.
        """
        logger.info("%s", "----------------------------------------------------")
        logger.info("%s", f"statistics for all {len(self.logger_dict)} pdbids")
    ...
    def get_logger_data(self) -> dict:
        ...
        This method iterates over each logger in the `logger_dict` and calls the
        `get_data` method
        of each logger to obtain its data. It then creates a dictionary where the keys
        are the PDB IDs
        and the values are the corresponding logger data.

        Returns:
            dict: A dictionary containing the data from all loggers, with PDB IDs as
                keys.
    ...
    def get_logger_dict(self) -> dict:
        ...
        Returns:
            dict: A dictionary where keys are PDB IDs and values are MMCIFStatsLogger
                instances.
    ...
    def from_logger_dict(cls, logger_dict: dict) -> MMCIFStatsSummary:
        ...
        This class method initializes a new MMCIFStatsSummary object and populates it
        with
        loggers from the provided dictionary. It then merges the data from each logger
        into
        the summary's data attribute.

        Args:
            logger_dict (dict): A dictionary where keys are PDB IDs and values are
                MMCIFStatsLogger instances.

        Returns:
            MMCIFStatsSummary: A new MMCIFStatsSummary instance populated with the
                given loggers and their data.
...
class MMCIFStatsLogger:
    ...
    def __init__(self, pdbid: str | None = None) -> None:
        self.pdbid = pdbid
        self.data = {}
        self.atom_array = None

    def reset_pdbid(self, pdbid: str) -> None:
        """Reset the PDB ID associated with the MMCIFStatsLogger instance.

        This method updates the `pdbid` attribute of the logger with the provided
        value.
    ...
    @contextmanager
    def log(
        self, namespace: str, atom_array: struc.AtomArray, components: list[str]
    ) -> Iterator[MMCIFStatsLogger]:
        """Log the changes in component counts for a given namespace and atom array.

        This method is a context manager that records the initial counts of specified
        components
        in the AtomArray, yields the current instance, and then calculates and logs the
        changes
        in component counts after the context is exited.

        Args:
            namespace (str): The namespace under which the component counts will be
                logged.
            atom_array (AtomArray): The AtomArray containing the structural data.
            components (list[str]): A list of components to count, such as 'atom',
                'bond', 'residue', etc.
        ...
        for component, count, new_count in zip(
            components, counts, new_counts, strict=False
        ):
    ...
    def log_one(self, namespace: str, component: str, count: int = 1) -> None:
        """Log a single component count for a given namespace.

        This method is used to increment the count of a specific component within a
        given namespace.
        If the namespace or component does not exist in the data dictionary, it will be
        initialized.

        Args:
            namespace (str): The namespace under which the component count will be
                logged.
            component (str): The component to be logged.
            count (int, optional): The count to be added to the component. Defaults to
                1.
    ...
    def report(self) -> None:
        ...
        This method generates a formatted table showing the statistics for each
        namespace
        in the current logger's data, and prints it to the console along with the PDB
        ID.
        """
        logger.info("%s", "----------------------------------------------------")
        logger.info("%s", f"statistics for pdbid:{self.pdbid}")
        report_stats(self.data)
        logger.info("%s", "----------------------------------------------------")

    def count_component(self, components: list[str] | None = None) -> list[int]:
        ...
        Returns:
            list[int]: A list of integers representing the counts of each component
            type.

        """
        if not (self.atom_array is not None):
            message = "Invalid state: self.atom_array is not None"
        ...
        for component_type in components:
            if component_type == "atom":
                ...
                counts.append(
                    ...
                    else len(self.atom_array.bonds.as_array())
                )
