...
def _glu_kernel(
    ...
    v_ptrs = v_ptr + (offs_k[:, None] * stride_vk + offs_bn[None, :] * stride_vn)
    ...
    for k in range(tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        v = tl.load(v_ptrs, mask=offs_k[:, None] < K - k * BLOCK_SIZE_K, other=0.0)
...
def gated_linear_unit_triton(x, weight):
    """Compute gated linear unit triton."""
    x = x.contiguous()
    x_shape = x.shape
...
def gated_linear_unit_torch(x, weight):
    """Compute gated linear unit torch."""
    y = torch.matmul(x.to(weight.dtype), weight)
...
def gated_linear_unit(x, weight):
    ...
    if fastnn_config.gated_linear_unit_implementation == "torch":
        return gated_linear_unit_torch(x, weight)
    if fastnn_config.gated_linear_unit_implementation == "triton":
        return gated_linear_unit_triton(x, weight)
    message = f"Unsupported gated linear unit backend: {fastnn_config.gated_linear_unit_implementation}"
