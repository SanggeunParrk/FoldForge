...
class DnaRnaProteinEntityWidget(widgets.VBox):
    """Represent dna rna protein entity widget."""
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        def handle_submit(sender: dict[str, Any]) -> None:
            """Compute handle submit."""
            item = sender["owner"]
    ...
    def add_modification_callback(self, _b: widgets.Button) -> None:
        """Add modification callback."""
        self.modifications_count += 1
        index = self.modifications_count
        modification_type = widgets.Text(
            value="CCD_XXX",
            placeholder="CCD_XXX",
        ...
        def handle_submit(sender: dict[str, Any]) -> None:
            ...
            if len(item.value) < CCD_PREFIX_LENGTH:
                item.value = "CCD_"
            item.value = re.sub(r"[^A-Z0-9_]", "", item.value)

        modification_type.observe(handle_submit, names="value")

        ptm_position = widgets.BoundedIntText(
        ...
        delete_button.on_click(lambda _b: self.delete_ptm_row(index))
        self.modifications.append(
            widgets.HBox([modification_type, ptm_position, delete_button])
        )
    ...
    def delete_ptm_row(self, index: int) -> None:
        """Compute delete ptm row."""
    ...
    def get_result(self) -> dict[str, Any]:  # noqa: C901, PLR0912 - entity-specific widget validation
        """Return result."""
        result = {}
        sequence_type = self.molecule_type_dropdown.value
        if sequence_type is None:
            message = "Select a molecule type before adding an entity"
        ...
        if not (len(result[recode_name]["sequence"]) > 0):
            message = "sequence length must > 0"
            raise ValueError(message)
        ...
        for item in result[recode_name]["modifications"]:
            pos = item["Position"]
            if not (pos <= len(result[recode_name]["sequence"])):
            ...
            for char in sequence:
                if char not in valid_chars:
                    message = (
                        ...
                        f"in sequence. valid set is {valid_chars}"
                    )
                    raise ValueError(message)
...
class LigandSmilesEntityWidget(widgets.VBox):
    """Represent ligand smiles entity widget."""
    ...
    def get_result(self) -> dict[str, Any]:
        ...
        if not (len(result[name][name]) > 0):
            message = "SMILES length must > 0"
            raise ValueError(message)
...
class LigandIonCCDEntityWidget(widgets.VBox):
    """Represent ligand ion c c d entity widget."""
    ...
    def get_result(self) -> dict[str, Any]:
        """Return result."""
        result = {}
        name = "ion" if self.molecule_type_dropdown.value == "Ion" else "ligand"
        result[name] = {}
        result[name][name] = (
...
class CovalentBondsWidget(widgets.VBox):
    """Represent covalent bonds widget."""
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.convalent_bonds = []
        self.convalent_bonds_id = []
    ...
    def on_add_convalent_bond(self, _b: widgets.Button) -> None:
        """Compute on add convalent bond."""
        ...
        delete_button.on_click(lambda _b: self.delete_row(index))
    ...
    def update(self) -> None:
        """Update ."""
        self.children = [*list(self.convalent_bonds), self.add_convalent_bond_buttom]
...
class ProtenixInputViewer(widgets.VBox):
    """Represent protenix input viewer."""
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.ligand_smiles_entities = []
        self.ligand_smiles_entities_id = []
        self.ligand_smiles_entities_count = 0

        self.dna_rna_protein_entities = []
        self.dna_rna_protein_entities_id = []
    ...
    def add_ligand_smiles_callback(self, _b: widgets.Button) -> None:
        """Add ligand smiles callback."""
        ...
        delete_button.on_click(lambda _b: self.delete_ligand_ion_ccd_row(index))
        self.ligand_smiles_entities.append(widgets.HBox([item, delete_button]))
        self.ligand_smiles_entities_id.append(index)
        self.update()

    def add_ligand_ion_ccd_callback(self, _b: widgets.Button) -> None:
        """Add ligand ion ccd callback."""
        self.ligand_smiles_entities_count += 1
        index = self.ligand_smiles_entities_count
    ...
    def delete_ligand_ion_ccd_row(self, index: int) -> None:
        """Compute delete ligand ion ccd row."""
    ...
    def add_dna_rna_protein_callback(self, _b: widgets.Button) -> None:
        """Add dna rna protein callback."""
        ...
        delete_button.on_click(lambda _b: self.delete_dna_rna_protein_row(index))
        self.dna_rna_protein_entities.append(widgets.HBox([item, delete_button]))
        self.dna_rna_protein_entities_id.append(index)
        self.update()

    def delete_dna_rna_protein_row(self, index: int) -> None:
        """Compute delete dna rna protein row."""
    ...
    def update(self) -> None:
        ...
        self.children = [
            ...
            *list(self.dna_rna_protein_entities),
            *list(self.ligand_smiles_entities),
            self.covalent_bonds,
            widgets.HBox(
                [
    ...
    def get_result(self) -> dict[str, Any]:
        ...
        for covalent_bond in result["covalent_bonds"]:
            if not (
                ...
                message = "covalent bond index out of range"
                raise ValueError(message)
            if not (
            ...
            if len(covalent_bond["left_atom"]) == 0:
                message = "left atom is empty"
                raise ValueError(message)
            if len(covalent_bond["right_atom"]) == 0:
                message = "right_atom atom is empty"
                raise ValueError(message)

            left_entiy = result["sequences"][covalent_bond["left_entity"] - 1]
            left_entiy_seq = left_entiy[next(iter(left_entiy.keys()))]
            right_entity = result["sequences"][covalent_bond["right_entity"] - 1]
            right_entity_seq = right_entity[next(iter(right_entity.keys()))]
            if not (
                covalent_bond["left_position"] <= len(left_entiy_seq)
                and covalent_bond["left_position"] >= 1
            ):
                message = "left position out of range"
                raise ValueError(message)
            if not (
                covalent_bond["right_position"] <= len(right_entity_seq)
                and covalent_bond["right_position"] >= 1
            ):
                message = "right position out of range"
                raise ValueError(message)
        return result
