...
from foldforge.data.ccd.components import get_ccd_ref_info, get_component_rdkit_mol
from foldforge.data.constants.residues import STD_RESIDUES
...
class GeometryFeaturizer:
    ...
    def get_chain_topology_features(self):
        ...
        for asym_id_1, idx_1 in zip(unique_asym_ids, index, strict=False):
            for asym_id_2, idx_2 in zip(unique_asym_ids, index, strict=False):
    ...
    def dict_to_tensor(feature_dict):
        ...
        """
        for k, raw_v in feature_dict.items():
            v = raw_v
        ...
        for start, stop in pairwise(starts):
            ...
            idx_mol_to_atom_array = {
                ...
                for atom_name, idx in zip(
                    self.atom_array.atom_name[start:stop],
            ...
            for feature_name in [
                ...
                for feature_index, raw_atom_indices in enumerate(
                    features[atom_index_key]
                ):
                    # Keep only constraints whose atoms exist in this residue.
                    atom_indices = raw_atom_indices
...
def build_angle_triples_from_bonds(bond_index: np.ndarray) -> np.ndarray:
    ...
    angles = []
    for j, raw_neighbor in enumerate(neighbors):
        neighbor = raw_neighbor
        if len(neighbor) < 2:
            continue
...
def extract_chiral_dihedral_from_mol(mol: Chem.Mol):
    ...
    if not (mol.GetNumConformers() > 0):
        message = "mol does not have ref pos"
...
def extract_linear_triple_bond_from_mol(mol: Chem.Mol):
    ...
    for bond in mol.GetBonds():
        ...
        triple_bond_index.extend(
            [neighbor_idx, begin_idx, end_idx] for neighbor_idx in begin_neighbors
        )
        triple_bond_index.extend(
            [begin_idx, end_idx, neighbor_idx] for neighbor_idx in end_neighbors
        )
    return {
...
def extract_planar_improper_from_mol(mol: Chem.Mol):
    """Extract planar improper from mol.
    ...
    for atom in mol.GetAtoms():
        ...
        # Record three quadruples: [nb1, nb2, center, nb3], [nb3, nb1, center, nb2],
        # [nb2, nb3, center, nb1]
...
def get_ccd_geometry_features(
    ccd_code: str, ccd_mols: tuple[tuple[str, Chem.Mol]] | None = None
):
    """Get geometry features for a CCD component.

    The returned dict uses local RDKit atom indices.
    """
    ccd_mols_dict = {} if ccd_mols is None else dict(ccd_mols)
    ...
    if mol is None:
        logger.warning(
            "CCD %s has no RDKit molecule; geometry features are empty", ccd_code
        )
        return {feature_name: [] for feature_name in RDKIT_GEOMETRY_FEATURES}
    ...
    try:
        ...
        logger.exception(
            "Warning: mol %s compute geometry feature from mol failed, return empty features",
            f"{ccd_code}",
        )
...
def update_features_by_perm(features: dict, perm: np.ndarray) -> dict:
    ...
    This function loosens or removes constraints that would otherwise over-constrain
    equivalent atoms.
    """
    ...
    if len(pairwise_distance_index) > 0:
        atom_i, atom_j = (
            list(col) for col in zip(*pairwise_distance_index, strict=False)
        )
        (
...
def update_bounds_by_equivalence_groups(
    ...
    if not (len(atom_idx_j) == len(upper_A) == len(lower_B) == n):
        msg = (
            "`atom_idx_i`, `atom_idx_j`, `upper_A`, and `lower_B` must have "
            "the same length."
        )
    ...
    for i, j, ua, lb in zip(atom_idx_i, atom_idx_j, upper_A, lower_B, strict=False):
    ...
    new_upper, new_lower = [], []
    for i, j in zip(atom_idx_i, atom_idx_j, strict=False):
        ri, rj = reps[int(i)], reps[int(j)]
        if undirected and ri > rj:
            ri, rj = rj, ri
        key = (ri, rj)
...
def compute_equivalence_groups_from_permutations(
    perms, n=None, one_based: bool = False
):
    ...
    for pi in perms:
        if len(pi) != n:
            msg = "All permutations must have the same length n."
            raise ValueError(msg)
        if one_based:
            pi0 = [x - 1 for x in pi]
            if set(pi0) != set(range(n)):
                msg = "Each permutation must be a bijection of 1..n."
                raise ValueError(msg)
            norm_perms.append(pi0)
        else:
            if set(pi) != set(range(n)):
                msg = "Each permutation must be a bijection of 0..n-1."
                raise ValueError(msg)
            norm_perms.append(list(pi))

    dsu = DSU(n)
    # Union all edges (i, sigma(i)) for each generator permutation sigma.
    ...
    groups = []
    for raw_comp in buckets.values():
        comp = raw_comp
        comp.sort()
        if one_based:
