...
from foldforge.data.constants.chemistry import rdkit_vdws
...
class Clash(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.details_dtype = details_dtype
    ...
    def get_chain_info(
        ...
        if self.compute_vdw_clash:
            if not (vdw_mol_id is not None):
                message = "Invalid state: vdw_mol_id is not None"
                raise ValueError(message)
            if not (elements_one_hot is not None):
                message = "Invalid state: elements_one_hot is not None"
                raise ValueError(message)
        ...
        for aid in range(N_chains):
            ...
            if len(atom_type_i.unique()) != 1:
                message = "Invalid state: len(atom_type_i.unique()) == 1"
                raise ValueError(message)
            ...
            if self.compute_vdw_clash:
                ...
                if not (vdw_mol_id is not None):
                    message = "Invalid state: vdw_mol_id is not None"
                    raise ValueError(message)
                mol_id_i = vdw_mol_id[atom_chain_mask].unique().item()
    ...
    def get_chain_pair_violations(
        ...
        if violation_type == "af3":
            ...
            if not (elements_one_hot is not None):
                message = "Invalid state: elements_one_hot is not None"
                raise ValueError(message)
            vdw_radii_i, vdw_radii_j = (
                get_vdw_radii(elements_one_hot[chain_1_mask, :]),
    ...
    def _check_clash_per_chain_pairs(
        ...
        atom_to_token_idx,
        N_chains,
        atom_type,  # noqa: ARG002 - shared callback or fixture signature
        chain_types,
        elements_one_hot,
        asym_id_to_asym_mask,
        mol_id: torch.Tensor | None = None,  # noqa: ARG002 - shared callback or fixture signature
        asym_id_to_mol_id: dict[int, int] | None = None,
    ):
        device = pred_coordinate.device
        N_sample = pred_coordinate.shape[0]

        # Disabled violation modes have no result tensors. Guard the result
        # directly so a disabled mode never allocates GPU memory.
        has_af3_clash_flag = None
        af3_clash_details = None
        has_vdw_clash_flag = None
        vdw_clash_details: dict[tuple[int, int, int], torch.Tensor] = {}
        vdw_asym_id_to_mol_id: dict[int, int] = {}
        # initialize results
        if self.compute_af3_clash:
            ...
            af3_clash_details = torch.zeros(
                N_sample, N_chains, N_chains, 2, device=device, dtype=self.details_dtype
            )
        if self.compute_vdw_clash:
            ...
            vdw_clash_details = {}
            if not (asym_id_to_mol_id is not None):
                message = "Invalid state: asym_id_to_mol_id is not None"
                raise ValueError(message)
            vdw_asym_id_to_mol_id = asym_id_to_mol_id
            ...
            for i in range(N_chains):
                ...
                for j in range(i + 1, N_chains):
                    if chain_types[j] == "UNK":
                        continue
                    chain_pair_type = {chain_types[i], chain_types[j]}
                    # Skip potential bonded ligand to polymers
                    skip_bonded_ligand = False
                    if (
                        has_vdw_clash_flag is not None
                        and "lig" in chain_pair_type
                        and len(chain_pair_type) > 1
                        and vdw_asym_id_to_mol_id[i] == vdw_asym_id_to_mol_id[j]
                    ):
                        common_mol_id = vdw_asym_id_to_mol_id[i]
                        logger.warning(
                            "mol_id %s may contain bonded ligand to polymers",
                            f"{common_mol_id}",
                        )
                    ...
                    if has_vdw_clash_flag is not None and not skip_bonded_ligand:
                    ...
                    if (
                        ...
                        continue
                    if has_af3_clash_flag is not None and af3_clash_details is not None:
        ...
        return {
            "summary": {
                "af3_clash": has_af3_clash_flag,
                "vdw_clash": has_vdw_clash_flag
                if has_vdw_clash_flag is not None
                else None,
                "chain_types": chain_types,
                "skipped_pairs": skipped_pairs,
            },
            "details": {
                "af3_clash": af3_clash_details,
                "vdw_clash": vdw_clash_details
                if has_vdw_clash_flag is not None
                else None,
            },
        }
