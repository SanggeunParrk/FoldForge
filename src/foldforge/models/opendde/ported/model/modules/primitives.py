...
from team_gm.modules.tensor_layout import (
    gather_pair_embedding_in_dense_trunk as gather_pair_embedding_in_dense_trunk,
    optimized_concat_split as optimized_concat_split,
    rearrange_qk_to_dense_trunk as rearrange_qk_to_dense_trunk,
)
...
from foldforge.models.opendde.ported.model.triangular.layers import LayerNorm, trunc_normal_init_
...
def _attention(
    ...
    attn_bias: Optional[torch.Tensor] = None,
    use_efficient_implementation: bool = True, inplace_safe: bool = False,
) -> torch.Tensor:
    from team_gm.modules.blocks.attention_math import attention
    if use_efficient_implementation and q.device.type == "mps":
        bias = attn_bias.float() if attn_bias is not None else None
        return _mps_attention(q.float(), k.float(), v.float(), bias).to(q.dtype)
    return attention(q, k, v, bias=attn_bias, logits_float32=True,
                     efficient=use_efficient_implementation, scale=1.0)
