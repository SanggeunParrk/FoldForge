...
from team_gm.modules.checkpoints.tensor_utils import expand_at_dim, get_checkpoint_fn

from foldforge.eval.rmsd import weighted_rigid_align
from foldforge.modules.flat.frames import (
    expressCoordinatesInFrame,
    gather_frame_atom_by_indices,
)
from foldforge.utils.logging import get_logger
from foldforge.utils.tensor import cdist
...
def loss_reduction(loss: torch.Tensor, method: str = "mean") -> torch.Tensor:
    ...
    if method not in ["mean", "sum", "add", "max", "min"]:
        message = "Invalid state: method in ['mean', 'sum', 'add', 'max', 'min']"
...
class SmoothLDDTLoss(nn.Module):
    """Implement Algorithm 27 [SmoothLDDTLoss] in AF3.

    Args:
        eps (float, optional): avoid nan. Defaults to 1e-10.
        reduction (str, optional): reduction method for the batch dims. Defaults to
            mean.
    """
    ...
    def forward(
        ...
        pred_distance: torch.Tensor,
        true_distance: torch.Tensor,
        distance_mask: torch.Tensor,  # noqa: ARG002 - shared callback or fixture signature
        ...
        Args:
            ...
            distance_mask (torch.Tensor): whether true coordinates exist.
                [N_atom, N_atom]
            lddt_mask (torch.Tensor, optional): whether true distance is within radius
                (30A for nuc and 15A for others)
                [N_atom, N_atom]
            diffusion_chunk_size (Optional[int]): Chunk size over the N_sample
    ...
    def dense_forward(
        ...
        lddt_mask: torch.Tensor,
        diffusion_chunk_size: int | None = None,
        true_distance: torch.Tensor | None = None,
    ) -> torch.Tensor:
        ...
        Args:
            ...
            diffusion_chunk_size (Optional[int]): Chunk size over the N_sample
                dimension. Defaults to None.
            true_distance (Optional[torch.Tensor]): pre-computed true pairwise
                distances.
...
class BondLoss(nn.Module):
    """Implement Formula 5 [BondLoss] in AF3.
    ...
    def forward(
        ...
        bond_mask: torch.Tensor,
        per_sample_scale: torch.Tensor | None = None,
        diffusion_chunk_size: int | None = None,
    ) -> torch.Tensor:
        """BondLoss.
        ...
        Args:
            ...
            bond_mask (torch.Tensor): bonds considered in this loss
                [N_atom, N_atom] or [..., N_atom, N_atom]
            per_sample_scale (torch.Tensor, optional): whether to scale the loss by the
    ...
    def sparse_forward(
        ...
        # Protecting special data that has size: tensor([], size=(x, 0),
...
def compute_lddt_mask(
    ...
    Args:
        ...
        is_nucleotide (torch.Tensor): Indicator for nucleotide atoms.
            [..., N_atom] or [N_atom]
        is_nucleotide_threshold (float): Threshold distance for nucleotide atoms.
            Defaults to 30.0.
        is_not_nucleotide_threshold (float): Threshold distance for non-nucleotide
            atoms. Defaults to 15.0.
    ...
    ) * (~is_nucleotide_mask[..., None])  # [..., N_atom, N_atom]

    # Zero-out diagonals of c_lm and cast to float
    c_lm = c_lm * _get_off_diagonal_mask(
        c_lm.size(-1), c_lm.device, true_distance.dtype
    )
...
class DistogramLoss(nn.Module):
    """Implement DistogramLoss in AF3.

    This head and loss are identical to AlphaFold 2, where the pairwise token distances
    use the representative atom for each token:
    ...
    def calculate_label(
        ...
        Returns:
            ...
            pair_coordinate_mask (torch.Tensor): whether the coordinates of
                representative atom pairs exist.
...
class PDELoss(nn.Module):
    """Implement Predicted distance loss in AF3.

    This loss are between representative token atoms i and j in the mini-rollout
    prediction
...
def compute_alignment_error_squared(
    ...
    """Compute alignment error squared.

    Implement Algorithm 30 Compute alignment error, but do not take the square root.
...
class PAELoss(nn.Module):
    """Implement Predicted Aligned distance loss in AF3.
    ...
    def calculate_label(
        ...
        Args:
            ...
            coordinate_mask (torch.Tensor): whether true coordinates exist
                [N_atom]
            rep_atom_mask (torch.Tensor): masks of the representative atom for each
                token.
                [N_atom]
            frame_atom_index (torch.Tensor): indices of frame atoms (three atoms per
                token(=per frame)).
        ...
        Returns:
            ...
            frame_token_pair_mask (torch.Tensor): whether frame_i token_j both have
                true coordinates.
                [N_frame, N_token]
        """
        ...
        # NOTE: to support frame_atom_index with batch_dims, need to expand its dims
        # before constructing frames.
        if len(frame_atom_index.shape) != 2:
            message = "Invalid state: len(frame_atom_index.shape) == 2"
            raise ValueError(message)
    ...
    def forward(
        ...
        if len(has_frame.shape) != 1:
            message = "Invalid state: len(has_frame.shape) == 1"
            raise ValueError(message)
        if len(frame_atom_index.shape) != 2:
            message = "Invalid state: len(frame_atom_index.shape) == 2"
            raise ValueError(message)
...
class MSELoss(nn.Module):
    """Implement Formula 2-4 [MSELoss] in AF3."""
    ...
    def weighted_rigid_align(
        ...
        Args:
            pred_coordinate (torch.Tensor): the denoised coordinates from diffusion
                module
                [..., N_sample, N_atom, 3]
...
def calculate_atom_bespoke_lddt(
    ...
    diagonal_mask = _get_off_diagonal_mask(
        N_atom, true_d_lm.device, true_d_lm.dtype
    ).bool()[..., atom_m_mask]  # [N_atom, N_atom(m)]
...
class PLDDTLoss(nn.Module):
    """Implement PLDDT Loss in AF3, different from the paper description.

    This loss are between atoms l and m (has some filters) in the mini-rollout
    prediction.
    ...
    Args:
        ...
        is_nucleotide_threshold (float, optional): threshold for nucleotide atoms.
            Defaults 30.0.
        is_not_nucleotide_threshold (float, optional): threshold for non-nucleotide
            atoms. Defaults 15.0
        eps (float, optional): small number added to denominator. Defaults to 1e-6.
        reduction (str, optional): reduction method for the batch dims. Defaults to
            mean.
    ...
    def forward_given_atom_lddt(
        ...
        """Args:
            ...
            logits (torch.Tensor): predicted plddt logits [..., N_sample, N_atom,
                N_bins]
    ...
    def forward(
        ...
        if not (
            ...
            message = (
                "Invalid state: is_nucleotide.shape == is_polymer.shape == "
                "rep_atom_mask.shape == coordinate_mask.shape == "
                "coordinate_mask.view(-1).shape"
            )
            raise ValueError(message)
...
class ProtenixLoss(nn.Module):
    ...
    def __init__(self, configs) -> None:
        ...
        self.plddt_loss = PLDDTLoss(
            **configs.loss.plddt,
            is_nucleotide_threshold=self.lddt_radius["is_nucleotide_threshold"],
            is_not_nucleotide_threshold=self.lddt_radius["is_not_nucleotide_threshold"],
        )
    ...
    def calculate_label(
        ...
        Returns:
            label_dict (dict): with the following updates:
                distance (torch.Tensor): true atom-atom distance.
                    [..., N_atom, N_atom]
                distance_mask (torch.Tensor): atom-atom mask indicating whether true
                    distance exists.
                    [..., N_atom, N_atom]
        """
        # Distance mask
        ...
        # Note: we convert to bf16 for saving cuda memory, if performance drops, do not
        # convert it
        distance = (
            cdist(
            ...
            * distance_mask
        ).to(label_dict["coordinate"].dtype)  # [..., N_atom, N_atom]
    ...
    def calculate_prediction(
        ...
        if not self.configs.loss_metrics_sparse_enable:
            ...
            ).to(pred_dict["coordinate"].dtype)  # [..., N_atom, N_atom]
        return pred_dict

    def aggregate_losses(
        self,
        loss_fns: dict[str, Callable],
        has_valid_resolution: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Aggregates multiple loss functions and their respective metrics.

        Args:
            loss_fns (dict[str, Callable]): Dictionary of loss functions to be
                aggregated.
            has_valid_resolution (Optional[torch.Tensor]): Tensor indicating valid
                resolutions. Defaults to None.

        Returns:
            tuple[torch.Tensor, dict[str, torch.Tensor]]:
                - cum_loss (torch.Tensor): Cumulative loss.
                - all_metrics (dict[str, torch.Tensor]): Dictionary containing all
                metrics.
        """
        cum_loss = 0.0
        all_metrics = {}
        for loss_name, loss_fn in loss_fns.items():
            ...
            if isinstance(loss_outputs, tuple):
                ...
                if not (isinstance(loss_outputs, torch.Tensor)):
                    message = "Invalid state: isinstance(loss_outputs, torch.Tensor)"
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                logger.warning("%s loss is NaN. Skipping...", f"{loss_name}")
            if (
        ...
        if not isinstance(cum_loss, torch.Tensor):
            message = "At least one loss term is required"
            raise TypeError(message)
    ...
    def calculate_losses(
        ...
        Calculate the cumulative loss and aggregated metrics for the given predictions
        and labels.

        Args:
            feat_dict (dict[str, Any]): Feature dictionary containing additional
                features.
            pred_dict (dict[str, torch.Tensor]): Prediction dictionary containing model
                outputs.
            label_dict (dict[str, Any]): Label dictionary containing ground truth data.
            mode (str): Mode of operation ('train', 'eval', 'inference'). Defaults to
                'train'.

        Returns:
            tuple[torch.Tensor, dict[str, torch.Tensor]]:
                - cum_loss (torch.Tensor): Cumulative loss.
                - metrics (dict[str, torch.Tensor]): Dictionary containing aggregated
                metrics.
        """
        if mode not in ["train", "eval"]:
            message = "Invalid state: mode in ['train', 'eval']"
            raise ValueError(message)
        diffusion_per_sample_scale: torch.Tensor | None = None
        if mode == "train":
                ...
                loss_fns.update(
                    {
                        "smooth_lddt_loss": lambda: (
                            self.smooth_lddt_loss.sparse_forward(
                                pred_coordinate=pred_dict["coordinate"],
                                true_coordinate=label_dict["coordinate"],
                                lddt_mask=label_dict["lddt_mask"],
                                diffusion_chunk_size=self.configs.loss.diffusion_lddt_chunk_size,
        ...
        # Confidence Loss:
        # Only when resoluton is in [min_resolution, max_resolution] the confidence loss
        # is considered
    ...
    def forward(
        ...
        Returns:
            tuple[torch.Tensor, dict[str, torch.Tensor]]:
                - cum_loss (torch.Tensor): Cumulative loss.
                - losses (dict[str, torch.Tensor]): Dictionary containing aggregated
                metrics.
        """
        ...
        if mode == "train":
            ...
            if "coordinate" in pred_dict:
                ...
                msg = "Missing key: coordinate (in pred_dict)."
                raise KeyError(msg)
            no_chunks = N_sample // diffusion_chunk_size + (
                N_sample % diffusion_chunk_size != 0
            )
            cum_loss = pred_dict["coordinate"].new_zeros(())
            losses = {}
            for i in range(no_chunks):
                cur_sample_num = min(
