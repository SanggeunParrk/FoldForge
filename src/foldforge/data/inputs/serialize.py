...
from foldforge.data.constants.chemistry import STD_RESIDUES
...
def residue_atom_array_to_input_json(
    atom_array: AtomArray,
    parser: ResidueMMCIFParser,
    ...
    *,
    ...
    Args:
        ...
        save_entity_and_asym_id (bool, optional): Whether to save entity and asym ids
            to json.
                                                  Defaults to False.
        include_discont_poly_poly_bonds (bool, optional): Whether to include
            discontinuous polymer-polymer bonds.
                                                          Defaults to False.
    ...
    lig_polymer_bonds = get_ligand_polymer_bond_mask(
        atom_array=atom_array, lig_include_ions=False
    )
    lig_lig_bonds = get_lig_lig_bonds(atom_array=atom_array, lig_include_ions=False)
...
def residue_cif_to_input_json(
    ...
    *,
    ...
    """
    parser = ResidueMMCIFParser(mmcif_file)
    atom_array = parser.get_structure(altloc, model=1, bond_lenth_threshold=None)

    # remove HOH from entities
    atom_array = ResidueFilter.remove_water(atom_array)
    atom_array = ResidueFilter.remove_hydrogens(atom_array)
    atom_array = parser.mse_to_met(atom_array)
    atom_array = ResidueFilter.remove_element_x(atom_array)

    # remove crystallization_aids
    if any("DIFFRACTION" in m for m in parser.methods):
        atom_array = ResidueFilter.remove_crystallization_aids(
            atom_array, parser.entity_poly_type
        )
    ...
    if sample_name is None:
        sample_name = Path(mmcif_file).name.split(".")[0]

    return residue_atom_array_to_input_json(
        atom_array,
        parser,
        assembly_id,
        output_json,
...
def structural_atom_array_to_input_json(
    atom_array: AtomArray,
    parser: StructuralMMCIFParser,
    assembly_id: str | None = None,
    output_json: str | None = None,
    sample_name: str | None = None,
    *,
    ...
    Args:
        atom_array (AtomArray): Biotite Atom array.
        parser (MMCIFParser): Instantiated OpenDDE MMCIFParer.
        assembly_id (str, optional): Assembly ID. Defaults to None.
        output_json (str, optional): Output json file path. Defaults to None.
        sample_name (str, optional): The "name" filed in json file. Defaults to None.
        save_entity_and_asym_id (bool, optional): Whether to save entity and asym ids
...
def structural_cif_to_input_json(
    mmcif_file: str,
    assembly_id: str | None = None,
    altloc: str = "first",
    output_json: str | None = None,
    ...
    *,
    ...
    """Convert mmcif file to OpenDDE input json file.
    ...
    """
    parser = StructuralMMCIFParser(mmcif_file)
    atom_array = parser.get_structure(altloc, model=1, bond_lenth_threshold=None)

    # remove HOH from entities
    atom_array = StructuralFilter.remove_water(atom_array)
    atom_array = StructuralFilter.remove_hydrogens(atom_array)
    atom_array = parser.mse_to_met(atom_array)
    atom_array = StructuralFilter.remove_element_x(atom_array)

    # remove crystallization_aids
    if any("DIFFRACTION" in m for m in parser.methods):
        atom_array = StructuralFilter.remove_crystallization_aids(
            atom_array, parser.entity_poly_type
        )
    ...
    return structural_atom_array_to_input_json(
        atom_array,
        parser,
        assembly_id,
        output_json,
