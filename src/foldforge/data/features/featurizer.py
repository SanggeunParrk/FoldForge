...
from foldforge.data.features.tokenizer import (
    StructuralAtomArrayTokenizer as AtomArrayTokenizer,
)
from foldforge.data.features.tokenizer import Token, TokenArray
from foldforge.utils.geometry import angle_3p, random_transform
...
class _CommonFeaturizer(ABC):
    """Represent common featurizer."""
    ...
    def restype_onehot_encoded(restype_list: list[str]) -> torch.Tensor:
        """Implement this operation for the selected token layout."""
    ...
    def get_prot_nuc_frame(token: Token, centre_atom: Atom) -> tuple[int, list[int]]:
        ...
        Returns:
            ...
            frame_atom_index (List[int]): The index of the atoms used to construct the
                frame.
        """
        ...
        for i in abc_atom_name:
            if (centre_atom.mol_type == "protein" and "N" not in token.atom_names) or (
                centre_atom.mol_type != "protein" and "C1'" not in token.atom_names
            ):
                return 0, [-1, -1, -1]
    ...
    def get_bond_features(self) -> dict[str, torch.Tensor]:
        ...
        A 2D matrix indicating if there is a bond between any atom in token i and token
        j,
        restricted to just polymer-ligand and ligand-ligand bonds and bonds less than
        2.4 Å during training.
        The size of bond feature is [N_token, N_token].
        Returns:
            Dict[str, torch.Tensor]: A dict of bond features.
        """
        bonds = self.cropped_atom_array.bonds
        bond_array = (
            np.empty((0, 3), dtype=np.uint32) if bonds is None else bonds.as_array()
        )
        ...
        # the polymer-polymer (std-std, std-unstd, and inter-unstd) bond will not be
        # included in token_bonds.
        ...
        if not (np.all(atom_idx_to_token_idx >= 0)):
            message = "Some atoms are not in any token"
    ...
    def get_extra_features(self) -> dict[str, torch.Tensor]:
        ...
        if "resolution" in self.cropped_atom_array.get_annotation_categories():
    ...
    def get_lig_pocket_mask(
        atom_array: AtomArray, lig_label_asym_id: str | list
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Ref: AlphaFold3 Chapter Methods.Metrics.

        the pocket is defined as all heavy atoms within 10 Å of any heavy atom of the
        ligand,
        restricted to the primary polymer chain for the ligand or modified residue
        being scored,
        and further restricted to only backbone atoms for proteins. The primary polymer
        chain is defined variously:
        for PoseBusters it is the protein chain with the most atoms within 10 Å of the
        ligand,
        for bonded ligand scores it is the bonded polymer chain and for modified
        residues it
        is the chain that the residue is contained in (minus that residue).

        Args:
            atom_array (AtomArray): atoms in the complex.
            lig_label_asym_id (Union[str, List]): The label_asym_id of the ligand of
                interest.

        Returns:
            tuple[torch.Tensor, torch.Tensor]: A tuple of ligand pocket mask and pocket
            mask.
        """
        ...
        ligand_mask_list = []
        pocket_mask_list = []
        for ligand_chain_id in lig_label_asym_ids:
            if not (np.isin(ligand_chain_id, atom_array.label_asym_id)):
                message = (
                    f"{ligand_chain_id} is not in "
                    f"the label_asym_id of the cropped atom array."
                )
                raise ValueError(message)

            ligand_mask = atom_array.label_asym_id == ligand_chain_id
            ...
            if not (primary_chain_asym_id_int is not None):
                message = f"No primary chain found for ligand ({ligand_chain_id=})."
                raise ValueError(message)
...
class ResidueFeaturizer(_CommonFeaturizer):
    """Args:
    ...
    ref_pos_augment (bool): Boolean indicating whether apply random rotation and
        translation on ref_pos
    lig_atom_rename (bool): Boolean indicating whether rename atom name for ligand
        atoms
    ...
    @staticmethod
    def encoder(
        encode_def_dict_or_list: dict | list[str], input_list: list[str] | np.ndarray
    ) -> torch.Tensor:
        """Compute encoder.

        Encode a list of input values into a binary format using a specified encoding
        definition list.

        Args:
            encode_def_dict_or_list (list or dict): A list or dict of encoding
                definitions.
            input_list (list): A list of input values to be encoded.

        Returns:
            torch.Tensor: A tensor representing the binary encoding of the input
            values.
        """
        num_keys = len(encode_def_dict_or_list)
        if isinstance(encode_def_dict_or_list, dict):
            if num_keys != max(encode_def_dict_or_list.values()) + 1:
                message = (
                    "Do not use discontinuous number, which might causing potential "
                    "bugs in the code"
                )
                raise ValueError(message)
    ...
    def restype_onehot_encoded(restype_list: list[str]) -> torch.Tensor:
        ...
        Args:
            restype_list (List[str]): A list of residue types.
                                      The residue type of ligand should be "UNK" in the
                                      input list.

        Returns:
            torch.Tensor:  A Tensor of one-hot encoded residue types
        """
        return ResidueFeaturizer.encoder(STD_RESIDUES_WITH_GAP, restype_list)

    @staticmethod
    def elem_onehot_encoded(elem_list: list[str] | np.ndarray) -> torch.Tensor:
        ...
        """
        return ResidueFeaturizer.encoder(get_all_elems(), elem_list)

    @staticmethod
    def ref_atom_name_chars_encoded(atom_names: list[str] | np.ndarray) -> torch.Tensor:
    ...
    def get_lig_frame(
        ...
        For ligands, we use the reference conformer of the ligand to construct the
        frame.

        Args:
            token (Token): Token object.
            centre_atom (Atom): Biotite Atom object of Token centre atom.
            lig_res_ref_conf_kdtree (Dict[str, Tuple[KDTree, List[int]]]): A dictionary
                of KDTree objects and atom indices.
            ref_pos (torch.Tensor): Atom positions in the reference conformer.
                Size=[N_atom, 3]
            ref_mask (torch.Tensor): Mask indicating which atom slots are used in the
                reference conformer. Size=[N_atom]

        Returns:
            tuple[int, List[int]]:
                has_frame (int): 1 if the token has frame, 0 otherwise.
                frame_atom_index (List[int]): The index of the atoms used to construct
                    the frame.
        """
            ...
            if has_frame:
                ...
                if is_zero_norm:
                    ...
                    theta_degrees = angle_3p(
                        *[ref_pos[idx] for idx in frame_atom_index], eps=1e-4
                    )
    ...
    def get_token_frame(
        ...
        The atoms (a_i, b_i, c_i) used to construct token i's frame depend on the chain
        type of i:
        Protein tokens use their residue's backbone (N, Calpha, C),
        while DNA and RNA tokens use (C1', C3', C4') atoms of their residue.
        All other tokens (small molecules, glycans, ions) contain only one atom per
        token.
        ...
        If this set of three atoms is close to colinear (less than 25 degree
        deviation),
        ...
        Returns:
            TokenArray: A TokenArray with updated frame annotations.
                        ...
                        - frame_atom_index: The index of the atoms used to construct
                        the frame.
        """
        ...
        for token in token_array_w_frame:
            ...
            if (
                ...
                has_frame, frame_atom_index = ResidueFeaturizer.get_prot_nuc_frame(
                    token, centre_atom
                )

            else:
                has_frame, frame_atom_index = ResidueFeaturizer.get_lig_frame(
    ...
    def get_reference_features(self) -> dict[str, torch.Tensor]:
        ...
        for ref_space_uid in np.unique(self.cropped_atom_array.ref_space_uid):
            ...
            ref_pos[mask] = random_transform(
                ...
                centralize=True,
                translation_before_rotation=True,
            )
        ...
        ref_features["ref_element"] = ResidueFeaturizer.elem_onehot_encoded(
        ...
        ref_features["ref_atom_name_chars"] = (
            ResidueFeaturizer.ref_atom_name_chars_encoded(atom_names).long()
        )
    ...
    def get_atom_permutation_list(
        ...
        atom_perm_list.extend(
            [int(j) for j in i.split("_")] for i in self.cropped_atom_array.res_perm
        )

        # Atoms connected to different residue are fixed.
        # Bonds array: [[atom_idx_i, atom_idx_j, bond_type]]
        if self.cropped_atom_array.bonds is None:
            return atom_perm_list
        idx_i = self.cropped_atom_array.bonds.as_array()[:, 0]
        idx_j = self.cropped_atom_array.bonds.as_array()[:, 1]
        diff_mask = (
            self.cropped_atom_array.ref_space_uid[idx_i]
            != self.cropped_atom_array.ref_space_uid[idx_j]
        )
        inter_residue_bonds = self.cropped_atom_array.bonds.as_array()[diff_mask]
        ...
        for r_start, r_stop in itertools.pairwise(res_starts):
            ...
            # Identify the column where the positions of the fixed atoms remain
            # unchanged
    ...
    def get_gt_full_complex_features(
        atom_array: AtomArray,
        cropped_atom_array: AtomArray | None = None,
        ...
        Args:
            ...
            get_cropped_asym_only (bool, optional): Defaults to True.
                - If true, a chain is returned only if its asym_id (mol_id) appears in
                the
                cropped_atom_array. It should be a favored setting for the spatial
                cropping.
                - If false, a chain is returned if its entity_id (entity_mol_id)
                appears in
        ...
        if cropped_atom_array is not None:
            ...
            mask = [
                ...
                for (entity, atom) in zip(
                    atom_array.entity_mol_id, atom_array.mol_atom_index, strict=False
                )
            ]
...
class StructuralFeaturizer(_CommonFeaturizer):
    """Args:
    ...
    def restype_onehot_encoded(restype_list: list[str]) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor:  A Tensor of one-hot encoded residue types
        """
        return StructuralFeaturizer.encoder(STD_RESIDUES_WITH_GAP, restype_list)
    ...
    def elem_onehot_encoded(elem_list: list[str] | np.ndarray) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor:  A Tensor of one-hot encoded elements
        """
        return StructuralFeaturizer.encoder(get_all_elems(), elem_list)
    ...
    def get_lig_frame(
        token: Token,
        centre_atom: Atom,
        lig_res_ref_conf_kdtree: dict[str, tuple[KDTree | None, np.ndarray]],
        ref_pos: torch.Tensor,
        ref_mask: torch.Tensor,
    ) -> tuple[int, list[int]]:
        """Ref: AlphaFold3 SI Chapter 4.3.2.
    ...
    def get_token_frame(
        ...
        for token in token_array_w_frame:
            ...
            if (
                ...
                has_frame, frame_atom_index = StructuralFeaturizer.get_prot_nuc_frame(
                    token, centre_atom
                )

            else:
                has_frame, frame_atom_index = StructuralFeaturizer.get_lig_frame(
                    token, centre_atom, lig_res_ref_conf_kdtree, ref_pos, ref_mask
                )
    ...
    def get_reference_features(self) -> dict[str, torch.Tensor]:
        ...
        ref_features["ref_element"] = StructuralFeaturizer.elem_onehot_encoded(
            self.cropped_atom_array.element
        ).long()
        ref_features["ref_charge"] = torch.from_numpy(
            self.cropped_atom_array.ref_charge.astype(np.int64)
        ...
        ref_features["ref_atom_name_chars"] = (
            StructuralFeaturizer.ref_atom_name_chars_encoded(atom_names).long()
        )
        ref_features["ref_space_uid"] = torch.from_numpy(
            self.cropped_atom_array.ref_space_uid.astype(np.int64)
        )
    ...
    def get_structural_token_features(
        ...
        """Return structural token features.
        ...
        if not (np.all(atom_to_structural_token_idx >= 0)):
            message = "Some atoms are not in any structural token"
            raise ValueError(message)
        if not (np.all(atom_to_structural_tokatom_idx >= 0)):
            message = "Some atoms are not assigned a structural token atom slot"
            raise ValueError(message)
    ...
    def get_polymer_residue_graph(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        graph_edges = set()
        if self.cropped_atom_array.bonds is not None:
