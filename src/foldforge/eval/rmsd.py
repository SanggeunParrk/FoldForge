...
def rmsd(
    ...
    """
    # mask [..., N]
    if pred_pose.shape != true_pose.shape:
        message = "Invalid state: pred_pose.shape == true_pose.shape"
        raise ValueError(message)  # [..., N, 3]
...
def align_pred_to_true(
    ...
    weight = atom_mask if weight is None else weight * atom_mask
...
def partially_aligned_rmsd(
    ...
    """Compute partially aligned rmsd.

    RMSD when aligning parts of the complex coordinate, does NOT take permutation
    symmetricity into consideration.
    Arguments:
    ...
    aligned_mask, unaligned_mask = (
        atom_mask * align_mask.float(),
        atom_mask * (1 - align_mask.float()),
    )
...
def weighted_rigid_align(
    ...
    """Implement Algorithm 28 in AF3. Wrap `align_pred_to_true`.
    ...
    Args:
        ...
        atom_weight (torch.Tensor): weights for each atom.
            [..., N_atom] or [N_atom]
        stop_gradient (bool): whether to detach the output. If true, will run it with
            no_grad() ctx.

    Returns:
        x_aligned (torch.Tensor): rotated, translated x which should be closer to
            x_target.
            [..., N_atom, 3]
    """
    if len(atom_weight.shape) == len(x.shape) - 1:
        if atom_weight.shape[:-1] != x.shape[:-2]:
            message = "Invalid state: atom_weight.shape[:-1] == x.shape[:-2]"
            raise ValueError(message)
    else:
        if len(atom_weight.shape) != 1:
            message = "Invalid state: len(atom_weight.shape) == 1"
            raise ValueError(message)
        if atom_weight.shape[-1] != x.shape[-2]:
            message = "Invalid state: atom_weight.shape[-1] == x.shape[-2]"
            raise ValueError(message)
    ...
    if stop_gradient:
        with torch.no_grad():
            x_aligned, _rot, _trans = align_pred_to_true(
                pred_pose=x,
                true_pose=x_target,
                atom_mask=None,
                weight=atom_weight,
