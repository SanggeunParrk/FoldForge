...
from foldforge.models.config.protenix.config.extend_types import (
    GlobalConfigValue,
    ListValue,
)
...
data_configs = {
    ...
    # NOTE:
    # `weightedPDB_before2109_wopb_nometalc_0925` is compatible with the `2024.05.22`
    # data version
    # downloaded via `scripts/database/download_protenix_data.sh`.
    # `weightedPDB_before250701_v20260101` and `weightedPDB_before210930_v20260101` are
    # compatible
    # with the `2026.01.01` data version.
    "weightedPDB_before2109_wopb_nometalc_0925": {
        "base_info": {
            "mmcif_dir": str(Path(PROTENIX_ROOT_DIR) / ("mmcif")),
            "bioassembly_dict_dir": str(
                Path(PROTENIX_ROOT_DIR) / ("mmcif_bioassembly")
            ),
            "indices_fpath": str(
                Path(PROTENIX_ROOT_DIR)
                / (
                    "indices/weightedPDB_indices_before_2021-09-30_wo_posebusters_res"
                    "olution_below_9.csv.gz"
                )
            ),
            "pdb_list": "",
    ...
    "recentPDB_1536_sample384_0925": {
        "base_info": {
            "mmcif_dir": str(Path(PROTENIX_ROOT_DIR) / ("mmcif")),
            "bioassembly_dict_dir": str(
                Path(PROTENIX_ROOT_DIR) / ("recentPDB_bioassembly")
            ),
            "indices_fpath": str(
                Path(PROTENIX_ROOT_DIR)
    ...
    "posebusters_0925": {
        "base_info": {
            "mmcif_dir": str(Path(PROTENIX_ROOT_DIR) / ("posebusters_mmcif")),
            "bioassembly_dict_dir": str(
                Path(PROTENIX_ROOT_DIR) / ("posebusters_bioassembly")
            ),
            "indices_fpath": str(
                Path(PROTENIX_ROOT_DIR)
    ...
    "msa": {
        ...
        "prot_seq_or_filename_to_msadir_jsons": ListValue(
            [str(Path(PROTENIX_ROOT_DIR) / ("common/seq_to_pdb_index.json"))]
        ),
        "prot_msadir_raw_paths": ListValue(
            [str(Path(PROTENIX_ROOT_DIR) / ("mmcif_msa_template"))]
        ),
        "prot_pairing_dbs": ListValue(["pairing"]),
        "prot_non_pairing_dbs": ListValue(
            ["pairing-non_pairing"]
            # Separated by "-", "pairing-non_pairing" means both pairing and non_pairing
            # are
            # used as non_pairing,
        ),
        ...
        "rna_seq_or_filename_to_msadir_jsons": ListValue(
            [str(Path(PROTENIX_ROOT_DIR) / ("rna_msa/rna_sequence_to_pdb_chains.json"))]
        ),
        "rna_msadir_raw_paths": ListValue(
            [str(Path(PROTENIX_ROOT_DIR) / ("rna_msa/msas"))]
        ),
    ...
    "template": {
        ...
        "prot_template_mmcif_dir": str(Path(PROTENIX_ROOT_DIR) / ("mmcif")),
        ...
        "release_dates_path": str(
            Path(PROTENIX_ROOT_DIR) / ("common/release_date_cache.json")
        ),
        "obsolete_pdbs_path": str(
            Path(PROTENIX_ROOT_DIR) / ("common/obsolete_to_successor.json")
        ),
        "kalign_binary_path": "/usr/bin/kalign",  # apt-get install kalign
    },
    "ccd_components_file": str(Path(PROTENIX_ROOT_DIR) / ("common/components.cif")),
    "ccd_components_rdkit_mol_file": str(
        Path(PROTENIX_ROOT_DIR) / ("common/components.cif.rdkit_mol.pkl")
    ),
    "obsolete_release_data_csv": str(
        Path(PROTENIX_ROOT_DIR) / ("common/obsolete_release_date.csv")
    ),
    "pdb_cluster_file": str(
        Path(PROTENIX_ROOT_DIR) / ("common/clusters-by-entity-40.txt")
    ),
}
