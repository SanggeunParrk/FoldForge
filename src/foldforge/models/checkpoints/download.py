...
"""Checkpoint downloads and managed inference assets."""
...
from foldforge.models.config.opendde.config.dependency_url import (
    CHECKPOINT_FILES,
    MANAGED_ASSETS,
    URL,
    ManagedAsset,
...
def progress_callback(block_num: int, block_size: int, total_size: int) -> None:
    ...
    if total_size <= 0:
        logger.info("%s", f"\rDownloaded {downloaded} bytes")
        return
...
def _decompress_zst(zst_path: str, output_path: str, source_url: str) -> None:
    ...
    if zstd is not None:
        try:
            with (
                Path(zst_path).open("rb") as compressed,
    ...
    try:
        subprocess.run(  # noqa: S603 - argument vector; no shell expansion
    ...
    for attempt in range(1, _DOWNLOAD_ATTEMPTS + 1):
        try:
            with requests.get(
                ...
                downloaded = 0
                with Path(destination).open("wb") as destination_file:
...
def _temporary_download_path(checkpoint_path: str, *, suffix: str) -> Iterator[str]:
    ...
    destination_dir = str(Path(str(Path(checkpoint_path).absolute())).parent)
    destination_name = Path(checkpoint_path).name
    ...
    with _temporary_download_path(checkpoint_path, suffix=".part") as staged_path:
        ...
        try:
            ...
            if check_weight and validator is None:
                ...
                raise RuntimeError(msg) from e
            msg = f"Downloaded asset from {tos_url} failed validation: {e}"
            raise RuntimeError(msg) from e

        Path(staged_path).replace(checkpoint_path)
...
def resolve_checkpoint_path(configs: Any) -> str:
    ...
    return str(Path(configs.load_checkpoint_dir) / (checkpoint_file))
...
def _validate_managed_asset(path: str, asset: ManagedAsset) -> None:
    ...
    if actual_size != asset.size:
        msg = f"expected {asset.size} bytes, got {actual_size} bytes"
        raise ValueError(msg)
    actual_sha256 = _sha256(path)
    if actual_sha256 != asset.sha256:
        msg = f"expected SHA256 {asset.sha256}, got {actual_sha256}"
        raise ValueError(msg)
...
def _ensure_managed_asset(asset_name: str, destination: str) -> None:
    ...
    if Path(destination).exists() and Path(destination).stat().st_size == asset.size:
        return
    ...
    Path(str(Path(str(Path(destination).absolute())).parent)).mkdir(
        parents=True, exist_ok=True
    )
    source_url = URL[asset_name]
    if Path(urlsplit(source_url).path).name != Path(destination).name:
        msg = f"Managed asset file name mismatch: {source_url} and {destination}."
        raise ValueError(msg)
    logger.info(
