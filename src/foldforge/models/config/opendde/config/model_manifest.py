...
def load_model_manifest(path: str | Path | None = None) -> dict[str, Any]:
    ...
    try:
        if path is None:
            resource = files("foldforge.models.config.opendde.config").joinpath(
                "model_manifest.json"
            )
        ...
        msg = f"invalid JSON in model manifest: {exc}"
...
def get_model_manifest(
    ...
    for model in manifest["models"]:
        if model["name"] == model_name:
            return model
    msg = f"Unknown released model: {model_name}"
    raise KeyError(msg)
...
def get_checkpoint_manifest(
    ...
    msg = f"Unknown checkpoint {checkpoint_filename!r} for model {model_name!r}"
    raise KeyError(msg)
...
def find_checkpoint_manifest(
    ...
    for model in manifest["models"]:
        for checkpoint in model["checkpoints"]:
            if checkpoint["filename"] == checkpoint_filename:
                return checkpoint
    msg = f"Unknown released checkpoint: {checkpoint_filename}"
    raise KeyError(msg)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "resolve-model":
            ...
            logger.info(
                "%s", checkpoint_url(args.checkpoint, manifest, model_name=args.model)
            )
        else:
