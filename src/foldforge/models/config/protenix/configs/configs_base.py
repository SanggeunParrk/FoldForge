...
basic_configs = {
    ...
    "finetune_params_with_substring": [
        ""
        # params with substring will be finetuned with different learning rate:
        # the finetuning optimizer learning rate.
    ],
    "eval_only": False,
...
model_configs = {
    ...
    "blocks_per_ckpt": ValueMaybeNone(
        1
        # NOTE: Number of blocks in each activation checkpoint, if None, no
        # checkpointing is
        # performed.
    ),
    # whether to scale up hidden dim in pairformer and confidence head
    "hidden_scale_up": False,
    # switch of kernels
    "triangle_multiplicative": "cuequivariance",  # cuequivariance, torch
    # Leaf backend; public execution policy is owned by the shared runtime.
    ...
    "skip_amp": {
        ...
        # If confidence_head (below) set to True and triangle_attention set to
        # cuequivariance,
        # RuntimeError: ERROR: Full precision FP32 backward pass for triangle attention
        # is not
    ...
    "infer_setting": {
        "chunk_size": ValueMaybeNone(
            256
            # should set to null for normal training and small dataset eval [for
            # efficiency]
        ),
