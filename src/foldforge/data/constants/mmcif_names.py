...
from foldforge.data.constants import atom_types, residue_names
...
DNA_RNA_HYBRID_CHAIN: Final[str] = "polydeoxyribonucleotide/polyribonucleotide hybrid"
...
STANDARD_POLYMER_CHAIN_TYPES: Final[AbstractSet[str]] = {
    PROTEIN_CHAIN,
    DNA_CHAIN,
    RNA_CHAIN,
}
...
LIGAND_CHAIN_TYPES: Final[AbstractSet[str]] = {
    BRANCHED_CHAIN,
    MACROLIDE_CHAIN,
    NON_POLYMER_CHAIN,
}
...
NON_POLYMER_CHAIN_TYPES: Final[AbstractSet[str]] = {
    *LIGAND_CHAIN_TYPES,
    WATER,
}

# Peptide possible values for _entity_poly.type.
PEPTIDE_CHAIN_TYPES: Final[AbstractSet[str]] = {
...
NUCLEIC_ACID_CHAIN_TYPES: Final[AbstractSet[str]] = {
    RNA_CHAIN,
    DNA_CHAIN,
...
POLYMER_CHAIN_TYPES: Final[AbstractSet[str]] = {
...
CRYSTALLIZATION_METHODS: Final[AbstractSet[str]] = {
    "X-RAY DIFFRACTION",
    "NEUTRON DIFFRACTION",
...
def is_standard_polymer_type(chain_type: str) -> bool:
    """Return if chain type is a protein, DNA or RNA chain type.
...
def guess_polymer_type(chain_residues: Sequence[str]) -> str:
    ...
    """
    residue_types = {
        **dict.fromkeys(residue_names.RNA_TYPES, RNA_CHAIN),
        **dict.fromkeys(residue_names.DNA_TYPES, DNA_CHAIN),
        **dict.fromkeys(residue_names.PROTEIN_TYPES_WITH_UNKNOWN, PROTEIN_CHAIN),
    ...
    def order_fn(item: tuple[str, int]) -> tuple[int, int]:
        """Compute order fn."""
        name, count = item
        return count, tie_braker[name]

    return max(counts.items(), key=order_fn)[0]


def fix_non_standard_polymer_res(*, res_name: str, chain_type: str) -> str:
    """Return the res_name of the closest standard protein/RNA/DNA residue.
    ...
    if chain_type == RNA_CHAIN:
        ...
        return one_letter_code if one_letter_code in residue_names.RNA_TYPES else "N"
    if chain_type == DNA_CHAIN:
        return residue_names.DNA_COMMON_ONE_TO_TWO.get(one_letter_code, "N")
    if chain_type == DNA_RNA_HYBRID_CHAIN:
        return res_name if res_name in residue_names.NUCLEIC_TYPES_WITH_UNKNOWN else "N"
    msg = f"Expected a protein/DNA/RNA chain but got {chain_type}"
