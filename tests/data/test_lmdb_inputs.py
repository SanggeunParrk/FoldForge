...
def write_db(path, key, value, *, subdir=True):
    with (
        lmdb.open(str(path), map_size=32 * 1024 * 1024, subdir=subdir) as env,
...
def msa_record(sequence="ARNDCQEG", kind="protein"):
    from foldforge.data.inputs.lmdb import ALPHABETS
...
def test_af3_native_template_feature_alignment(tmp_path):
    from alphafold3 import structure
    from alphafold3.constants import atom_types, mmcif_names
    ...
    features = get_polymer_features(
        ...
        query_to_hit_mapping=dict(
            zip(payload["queryIndices"], payload["templateIndices"], strict=False)
        ),
    )
