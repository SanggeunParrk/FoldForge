...
SEARCH_DATABASE_ROOT = str(Path(OPENDDE_ROOT_DIR) / ("search_database"))
...
data_configs: dict[str, Any] = {
    ...
    "template": {
        ...
        "max_templates": 4,
        "fetch_remote": True,
        "prot_template_mmcif_dir": str(Path(SEARCH_DATABASE_ROOT) / ("mmcif")),
        "prot_template_cache_dir": str(Path(SEARCH_DATABASE_ROOT) / ("template_cache")),
        "kalign_binary_path": "kalign",
        "release_dates_path": str(Path(DATA_ROOT) / ("common/release_date_cache.json")),
        "obsolete_pdbs_path": str(
            Path(DATA_ROOT) / ("common/obsolete_to_successor.json")
        ),
    },
    "ccd_components_file": str(Path(DATA_ROOT) / ("common/components.cif")),
    "ccd_components_rdkit_mol_file": str(
        Path(DATA_ROOT) / ("common/components.cif.rdkit_mol.pkl")
    ),
}
