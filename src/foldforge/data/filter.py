...
from foldforge.data.constants.chemistry import CRYSTALLIZATION_AIDS
...
class _CommonFilter:
    ...
    def remove_element_x(atom_array: AtomArray) -> AtomArray:
        ...
        - ASX: ASP/ASN ambiguous, two ambiguous atoms are marked as X, 6 entries in the
        PDB
        - GLX: GLU/GLN ambiguous, two ambiguous atoms are marked as X, 5 entries in the
        PDB
        """
    ...
    def remove_polymer_chains_too_short(
        ...
        """Remove polymer chains too short."""
        chain_starts = struc.get_chain_starts(atom_array, add_exclusive_stop=True)
        invalid_chains = []  # list of [start, end)
        for index in range(len(chain_starts) - 1):
            start, end = chain_starts[index], chain_starts[index + 1]
            ...
            if (
                entity_poly_type.get(entity_id, "non-poly")
                in (
                    "polypeptide(L)",  # This filter selects L-polypeptide chains.
                    "polyribonucleotide",
                    "polydeoxyribonucleotide",
                )
    ...
    def remove_polymer_chains_with_consecutive_c_alpha_too_far_away(
        ...
        """Remove polymer chains with consecutive c alpha too far away."""
        chain_starts = struc.get_chain_starts(atom_array, add_exclusive_stop=True)
        invalid_chains = []  # list of [start, end)
        for index in range(len(chain_starts) - 1):
            start, end = chain_starts[index], chain_starts[index + 1]
    ...
    def _get_clash_mask(
        ...
        Args:
            ...
            kdtree_query_result (np.ndarray): Precomputed neighbor indices from
                KDTree.query_ball_point
            removed_chain_ids (set[str]): Chains already scheduled for removal

        Returns:
            np.ndarray: Boolean array where True indicates atoms involved in
            inter-chain clashes
        ...
        for atom_idx, clashes in enumerate(kdtree_query_result):
            ...
            bonded_atoms = (
                ...
                else atom_array.bonds.get_bonds(atom_idx)[0]
            )
    ...
    def remove_clashing_chains(
        atom_array: AtomArray,
        clash_radius: float = 1.6,
        clash_ratio_threshold: float = 0.3,
    ) -> list[str]:
        """Implement AlphaFold3's chain clash removal protocol (SI Chapter 2.5.4).
        ...
        Args:
            ...
            clash_radius: Distance threshold (Å) for atomic clashes (1.6Å ~ 0.4Å less
                than vdW radii)
            clash_ratio_threshold: Minimum fraction of clashing atoms (0.3 = 30%) to
                trigger removal
        ...
        # eg: 1qzb
        if not (is_resolved_centre_atom.sum() > 0):
            message = "No resolved center atoms found"
        ...
        for atom_idx, clashes in enumerate(query_result):
            ...
            filtered_clashes = set(clashes) - set(bonded_atoms) - {atom_idx}
            filtered_clashes = {
                clash_atom_idx
        ...
        for _cycle in range(len(chain_ids)):
            if last_removed_chain_id is not None:
                ...
                for atom_idx in clashing_atoms:
                    ...
                    if not (clash_ratio_per_chain[chain_j] > 0):
                        message = "Invalid state: clash_ratio_per_chain[chain_j] > 0"
                        raise ValueError(message)

                    # update atom_index_to_num_clashed_chains after removing a chain
                    atom_index_to_num_clashed_chains[atom_idx] -= 1
                    if not (atom_index_to_num_clashed_chains[atom_idx] >= 0):
                        message = (
                            "Invalid state: "
                            "atom_index_to_num_clashed_chains[atom_idx] >= 0"
                        )
                        raise ValueError(message)

                    if atom_index_to_num_clashed_chains[atom_idx] == 0:
                        # when num_clashed_chains for some atom reach zero, substract 1
                        # from clash_num_per_chain
                        clash_num_per_chain[chain_j] -= 1
                        if not (clash_num_per_chain[chain_j] >= 0):
                            message = "Invalid state: clash_num_per_chain[chain_j] >= 0"
                            raise ValueError(message)
            ...
            msg = (
                "Find the number of clashes in chain incidents "
                "matching the total number of chains, "
    ...
    def remove_unresolved_chains(atom_array: AtomArray) -> AtomArray:
        ...
        return atom_array[np.isin(atom_array.chain_id, valid_chain_id)]

    @staticmethod
    def remove_asymmetric_polymer_ligand_bonds(
        atom_array: AtomArray, entity_poly_type: dict[str, str]
    ) -> AtomArray:
        """Remove asymmetric polymer ligand bonds.

        Remove asymmetric polymer ligand bonds (including protein-protein bond, like
        disulfide bond).

        AF3 SI 5.1 Structure filters
        Bonds for structures with homomeric subcomplexes lacking the corresponding
        homomeric symmetry are also removed
        e.g. 4MZ in 1keq

        - if a certain bonded ligand only exists for some of the symmetric copies, but
        not for all,
        ...
        bond_site_and_paired_entity_to_num_copies = {
            ...
            for bond_site_and_entity_key, chain_ids in (
                bond_site_and_paired_entity_to_chains.items()
            )
        }
    ...
    def remove_ligand_absent_atoms(atom_array: AtomArray) -> AtomArray:
        ...
        For each ligand type (by res_name), retain only atoms present in at least one
        ligand copy in the input.
        ...
        for res_name, atom_name in zip(
            lig_res_name[lig_is_resolved], lig_atom_name[lig_is_resolved], strict=False
        ):
            res_to_atom_name[res_name].add(atom_name)

        remove_indices = []
        for idx, res_name, atom_name in zip(
            lig_indices, lig_res_name, lig_atom_name, strict=False
        ):
    ...
    def _filter_altloc_by_local_largest(atom_array: AtomArray) -> np.ndarray:
        """Filter altloc by local largest.
        ...
        3. Select altloc with highest average occupancy that exists in all group
        residues
        ...
        for start, end in itertools.pairwise(res_starts):
            ...
            if np.all(altloc == "."):
                ...
                if not (last_res_has_alt):
                    message = "Invalid state: last_res_has_alt"
                    raise ValueError(message)
        ...
        for group in res_groups_list:
            ...
            for start, end in group:
                ...
                for altloc_char, idx in zip(altloc_chars, char_indices, strict=False):
            ...
            for start, end in group:
                ...
                for i in sorted_altloc_chars:
                    if (
                        i == "."
                        or i not in label_alt_id
                        or (chars_set and i not in chars_set)
                    ):
                        continue
                    chosen_char = i
                    break
    ...
    def remove_too_far_away_chains(
        ...
        in_dist_mask = [bool(q) for q in query_result]
...
class ResidueFilter(_CommonFilter):
    """Ref: AlphaFold3 SI Chapter 2.5.4."""
    ...
    def too_many_chains_filter(
        ...
        core_indices: list[int] | np.ndarray | None = None,
        max_tokens_num: int | None = None,
    ) -> tuple[AtomArray | None, int]:
        """Ref: AlphaFold3 SI Chapter 2.5.4.

        For bioassemblies with greater than 20 chains, we select a random interface
        token
        ...
        Args:
            atom_array (AtomArray): Biotite AtomArray Object of a Bioassembly.
            interface_radius (int, optional): Atoms within this distance of the central
                atom are considered interface atoms.
                                            Defaults to 15.
            max_chains_num (int, optional): The maximum number of chains permitted in a
                bioassembly.
                                            Filtration will be applied if exceeds this
                                            value. Defaults to 20.
            core_indices (list[int], optional): A list of indices to be used as chose
                the central atom.
                                                     And corresponding chains in the
                                                     list will be selected proriority.
                                                     If None, a random index from whole
                                                     AtomArray will be selected.
                                                     Defaults to None.
            max_tokens_num (int, optional): The maximum number of tokens permitted in a
                bioassembly.
                                            If not None,  after more than
                                            max_chains_num, if the max_tokens_num is
                                            not reached,
                                            it will continue to append the chains.

        Returns:
            tuple:
                - atom_array (AtomArray): An AtomArray that has been processed through
                this filter.
                - input_chains_num (int): The number of chain in the input AtomArray.
                                          This is to log whether the filter has been
                                          utilized.
        ...
        if not (resolved_centre_atom):
            message = "There is no resolved central atom."
            raise ValueError(message)
        ...
        # The distance between the central atoms in any two chains is greater than 15
        # angstroms.
        if chosen_centre_atom is None:
            return None, input_chains_num

        dist_mat = cdist(
            chosen_centre_atom.coord.reshape((1, -1)), resolved_centre_atom.coord
        )
        sorted_chain_id = np.array(
            [
                chain_id
                for chain_id, _dist in sorted(
                    zip(resolved_centre_atom.mol_id, dist_mat[0], strict=False),
        ...
        for chain_id in sorted_chain_id:
            ...
            if (len(closest_chain_id) >= max_chains_num) and (
                tokens + chain_token_num > max_tokens_num
            ):
                break
        ...
        if not (
            ...
            message = (
                "Invalid state: output_chains_num == max_chains_num or "
                "atom_array.centre_atom_mask.sum() <= max_tokens_num"
            )
            raise ValueError(message)
        return atom_array, input_chains_num
    ...
    def remove_ligand_unresolved_leaving_atoms(atom_array: AtomArray) -> AtomArray:
        """Remove ligand unresolved leaving atoms.

        For a ligand involved in covalent bonding, remove the unresolved leaving atoms
        from the covalently-bonded central atom,.
        regardless of whether they are marked as leaving atoms in the CCD (Chemical
        ...
        for centre_idx in np.unique(inter_residue_bonds):
            ...
            if atom_array.is_resolved[neighbors].sum() == 1:
                # only one neighbor is resolved, means one atom is resolved, eg: 4MZ in
                # 1keq
    ...
    def filter_altloc(
        ...
        Filter alternate conformations (altloc) of a given AtomArray based on the
        specified criteria.
        ...
        Args:
            ...
            altloc : str, optional
                The criteria for filtering alternate conformations. Possible values
                are:
                ...
                - "local_largest": Keep the alternate conformation with the largest
                average occupancy of
                                   a contiguous set of residues with alternate
                                   locations.
        ...
        if altloc == "local_largest":
            selected_mask = ResidueFilter._filter_altloc_by_local_largest(atom_array)
    ...
    def remove_dissociation(
        ...
        Args:
            atom_array (AtomArray): Biotite AtomArray Object of a Bioassembly.
            all_chain_pairs (list[tuple[str, str]]): A list of chain pairs that
        ...
        return atom_array[np.isin(atom_array.chain_id, list(largest_cc))]


class StructuralFilter(_CommonFilter):
    """Ref: AlphaFold3 SI Chapter 2.5.4."""
    ...
    def filter_altloc(
        ...
        if altloc == "local_largest":
            selected_mask = StructuralFilter._filter_altloc_by_local_largest(atom_array)
            return atom_array[selected_mask]
    ...
    def remove_dissociation(
        ...
        largest_cc = cast("set", max(nx.connected_components(interface_graph), key=len))
        return atom_array[np.isin(atom_array.chain_id, list(largest_cc))]
