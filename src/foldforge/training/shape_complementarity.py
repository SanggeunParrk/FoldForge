...
from team_gm.modules.checkpoints.tensor_utils import aggregate_atom_to_token
from torch.utils.checkpoint import checkpoint

from foldforge.utils.scatter import scatter
...
def get_shape_comp_atom_mask(
    ...
    """Return shape comp atom mask."""
    atom_to_token_idx = feat_dict.get("atom_to_token_idx")
    if atom_to_token_idx is None:
        msg = "shape complementarity requires atom_to_token_idx"
        raise KeyError(msg)
...
class _ShapeCompTokenFeatures(TypedDict):
    """Represent shape comp token features."""
...
def shape_comp_pred_uses_structural_tokens(
    ...
    """Compute shape comp pred uses structural tokens."""
...
def structural_shape_comp_feature_dict(feat_dict: dict[str, Any]) -> dict[str, Any]:
    """Compute structural shape comp feature dict."""
    missing = [
...
def resolve_shape_comp_feature_dict_for_pred(
    ...
    """Resolve shape comp feature dict for pred."""
...
def resolve_shape_comp_token_features(
    ...
    if is_structural and token_role_id is not None:
...
def _sorted_rep_atom_indices(
    ...
    if not torch.equal(sorted_token_idx, expected):
        msg = "Representative atoms do not cover the active token space exactly"
        raise ValueError(msg)
...
def summarize_shape_comp_pair(
    ...
    """Compute summarize shape comp pair."""
...
def build_shape_comp_pred_outputs(
    ...
    if keep_pair_map:
        if (
            ...
            msg = "keep_pair_map=True requires shape_comp_pair and mask"
            raise KeyError(msg)
...
def _resolve_pair_chunk_size(pair_chunk_size: int | None, n_token: int) -> int:
...
def compute_shape_complementarity_fields(  # noqa: PLR0915 - reference geometric scoring algorithm
    ...
    pair_chunk_size: int | None = 128,
    checkpoint_chunks: bool = True,
    return_pair_map: bool = True,
    eps: float = 1e-6,
    **_: Any,
) -> dict[str, torch.Tensor]:
    """Compute shape complementarity fields."""
