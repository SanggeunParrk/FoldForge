...
from foldforge.data.ccd.components import get_ccd_ref_info
...
from foldforge.data.features.atoms import (
    ...
    residue_get_inter_residue_bonds,
    residue_map_annotations_to_atom_indices,
    structural_get_inter_residue_bonds,
    structural_map_annotations_to_atom_indices,
)
from foldforge.data.filter import ResidueFilter, StructuralFilter
from foldforge.data.statistics import MMCIFStatsLogger
from foldforge.data.structure import _parse_inter_residue_bonds, concatenate
...
warnings.filterwarnings(
    "ignore",
    message=(
        "The coordinates are missing for some atoms. The fallback "
...
class _CommonMMCIFParser:
    """Represent common m m c i f parser."""

    cif: pdbx.CIFFile | pdbx.BinaryCIFFile

    def get_category_table(self, name: str) -> pd.DataFrame | None:
        """Return category table.

        Retrieve a category table from the CIF block and return it as a pandas
        DataFrame.
        ...
        Returns:
            Union[pd.DataFrame, None]: A pandas DataFrame containing the category data
    ...
    def pdb_id(self) -> str:
        """Extract and returns the PDB ID from the CIF block.
        ...
        if "entry" not in self.cif.block:
            return ""
        return str(self.cif.block["entry"]["id"].as_item()).lower()
    ...
    def release_date(self) -> str:
        ...
        def _is_valid_date_format(date_string: str) -> bool:
            try:
                datetime.strptime(date_string, "%Y-%m-%d")  # noqa: DTZ007 - PDB calendar dates, not timestamps
                _try_result = True
            except ValueError:
                return False
            else:
        ...
        if not (valid_date):
            message = f"Invalid date format: {date}, it should be yyyy-mm-dd format"
    ...
    def methods(self) -> list[str]:
        ...
        but about 233 entries have multi methods, such as ['X-RAY DIFFRACTION',
        'NEUTRON DIFFRACTION'].
        ...
        Returns:
            list[str]: such as ['X-RAY DIFFRACTION'], ['ELECTRON MICROSCOPY'],
            ['SOLUTION NMR', 'THEORETICAL MODEL'],
                ['X-RAY DIFFRACTION', 'NEUTRON DIFFRACTION'], ['ELECTRON MICROSCOPY',
                'SOLUTION NMR'], etc.
        ...
        return methods.as_array().tolist()

    def get_poly_res_names(
        self, atom_array: AtomArray | None = None
    ) -> dict[str, list[str]]:
        """Return poly res names.

        get 3-letter residue names by combining mmcif._entity_poly_seq and atom_array.

        if ref_atom_array is None: keep first altloc residue of the same res_id based
        in mmcif._entity_poly_seq
        ...
        poly_res_names = {}
        for entity_id in self.entity_poly_type:
            ...
            for i, res_id in enumerate(seq_nums):
                ...
                elif res_name_in_atom_array == seq_mon_ids[i]:
                    select_mask[i] = True
    ...
    def get_sequences(self, atom_array: AtomArray | None = None) -> dict:
    ...
    def entity_poly_type(self) -> dict[str, str]:
        ...
        return dict(zip(entity_poly.entity_id, entity_poly.type, strict=False))

    @functools.cached_property
    def entity_infos(self) -> dict:
        """Retrieve information about entities from the category table "entity".

        Returns:
            dict: A dictionary where each key is an entity ID and the value is another
                dictionary
    ...
    def replace_auth_with_label(atom_array: AtomArray) -> AtomArray:
        """Replace auth with label.

        Replace the author-provided chain ID with the label asym ID in the given
        AtomArray.

        This function addresses the issue described in
        https://github.com/biotite-dev/biotite/issues/553.
        It updates the `chain_id` of the `atom_array` to match the `label_asym_id` and
        resets the ligand
        residue IDs (`res_id`) for chains where the `label_seq_id` is ".". The residue
        IDs are reset
        ...
        for chain_id in chain_ids:
            ...
            num = 1
            for res_start, res_stop in itertools.pairwise(res_starts):
    ...
    def fix_arginine(atom_array: AtomArray) -> AtomArray:
        ...
        Arginine naming ambiguities are fixed (ensuring NH1 is always closer to CD than
        NH2).
        ...
        for start_i, stop_i in itertools.pairwise(starts):
    ...
    def create_empty_annotation_like(
        ...
        for k in source_array.get_annotation_categories():
            v = source_array.get_annotation(k)
            if k not in target_array.get_annotation_categories():
                target_array.set_annotation(
                    k, np.zeros(len(target_array), dtype=v.dtype)
                )
        return target_array


class ResidueMMCIFParser(_CommonMMCIFParser):
    """Parsing and extracting information from mmCIF files."""

    def __init__(
        self, mmcif_file: str | Path | None = None, mmcif_string: str | None = None
    ) -> None:
        self.mmcif_file = mmcif_file
        self.cif = self._parse(mmcif_file=mmcif_file, mmcif_string=mmcif_string)

    def _parse(
        self, mmcif_file: str | Path | None, mmcif_string: str | None = None
        ...
        if mmcif_file is not None:
            ...
            if mmcif_file.suffix == ".gz":
                ...
                with Path(mmcif_file).open() as f:
                    cif_file = pdbx.CIFFile.read(f)
            return cif_file
        if mmcif_string is not None:
            cif_file = io.StringIO(mmcif_string)
            return pdbx.CIFFile.read(cif_file)
        msg = "mmcif_file and mmcif_string are both None"
        raise ValueError(msg)

    def num_assembly_polymer_chains(self, assembly_id: str = "1") -> int | None:
        ...
        Args:
            assembly_id (str): The ID of the assembly to count polymer chains for.
                               Defaults to "1". If "all", counts chains for all
                               assemblies.

        Returns:
            int: The total number of polymer chains in the specified assembly.
                 If the oligomeric count is invalid (e.g., '?'), the function returns
                 None.
        ...
        for _assembly_id, _chain_count in zip(
            ...
            if assembly_id in ("all", _assembly_id):
                try:
    ...
    def get_structure(
        ...
        model: int = 1,
        bond_lenth_threshold: float | None = 2.4,
    ) -> AtomArray:
        ...
        bond_lenth_threshold: the threshold of bond length. If None, no filter will be
            applied.
        ...
        if atom_site is None:
            msg = "The file does not contain atom_site category table"
            raise ValueError(msg)

        if atom_site.row_count > 1000_000:
            message = "The file contains more than 1,000,000 atom_site rows"
            raise ValueError(message)

        model_atom_site = pdbx_convert._filter_model(atom_site, model)  # noqa: SLF001 - Biotite extension hook
        ...
        atoms.box = pdbx_convert._get_box(block)  # noqa: SLF001 - Biotite extension hook
        ...
        pdbx_convert._fill_annotations(  # noqa: SLF001 - Biotite extension hook
        ...
        if "struct_conn" in block:
            ...
            coord1 = atoms.coord[conn_bonds.as_array()[:, 0]]
            coord2 = atoms.coord[conn_bonds.as_array()[:, 1]]
            dist = np.linalg.norm(coord1 - coord2, axis=1)
            if bond_lenth_threshold is not None:
                conn_bonds = _replace_bond_array(
                    conn_bonds, conn_bonds.as_array()[dist < bond_lenth_threshold]
                )
            bonds = bonds.merge(conn_bonds)
        atoms.bonds = bonds

        # inference inter residue bonds missing in struct_conn, based on res_id
        # (auth_seq_id) and auth_asym_id, eg 5mfu
        ...
        atom_array = ResidueFilter.filter_altloc(atom_array, altloc=altloc)

        # inference inter residue bonds based on new res_id (label_seq_id).
        # the auth_seq_id is not reliable, some are discontinuous (8bvh), some with
        # insertion codes (6ydy).
        return ccd.add_inter_residue_bonds(atom_array, exclude_struct_conn_pairs=True)
    ...
    def expand_assembly(
        ...
        Args:
            ...
            assembly_id (str, optional): The assembly ID in mmCIF file. Defaults to
                "1".
                                         If assembly_id is "all", all assemblies will
                                         be returned.
        ...
        try:
            ...
            logger.info(
                "File has no 'pdbx_struct_assembly_gen' category, return original"
                " structure."
            )
            return structure
        ...
        try:
            ...
            logger.info(
                "File has no 'pdbx_struct_oper_list' category, return original "
                "structure."
            )
            return structure
        ...
        if assembly_id != "all":
            if assembly_id is None:
                ...
                msg = f"File has no Assembly ID '{assembly_id}'"
                raise KeyError(msg)

        ### Calculate all possible transformations
        transformations = pdbx_convert._get_transformations(struct_oper_category)  # noqa: SLF001 - Biotite extension hook

        ### Get transformations and apply them to the affected asym IDs
        assembly = None
        assembly_1_mask = []
        for operation_id, op_expr, asym_id_expr in zip(
            ...
            if assembly_id in ("all", operation_id):
                operations = pdbx_convert._parse_operation_expression(op_expr)  # noqa: SLF001 - Biotite extension hook
                asym_ids = asym_id_expr.split(",")
                # Filter affected asym IDs
                sub_structure = copy.deepcopy(
                    structure[..., np.isin(structure.label_asym_id, asym_ids)]
                )
                sub_assembly = pdbx_convert._apply_transformations(  # noqa: SLF001 - Biotite extension hook
        ...
        if assembly is None:
            message = f"Assembly {assembly_id!r} contains no matching operations"
            raise ValueError(message)
    ...
    def sort_chains_by_entity_id(self, atom_array: AtomArray) -> AtomArray:
        """Sort chains by entity id.

        Sort the chains in the given AtomArray by their entity IDs (label_entity_id).
        Some pdb entry has disordered chains, e.g. 6l4u, which will be sorted by
        entity_id.

        if the the number of atoms in chains with same entity_id is different,
        the chains will be sorted by the number of atoms in each chain in descending
        order.

        Args:
            atom_array (AtomArray): The AtomArray object containing the chains to be
                sorted.
        ...
        for start, end in itertools.pairwise(chain_starts):
            entity_id = atom_array.label_entity_id[start]
            atom_num = end - start
    ...
    def _check_if_no_polymer_remain(self, atom_array: AtomArray) -> bool:
    ...
    def get_bioassembly(
        ...
        max_chains: int | None = 20,
    ) -> dict[str, Any]:
        """Build the given biological assembly.

        Args:
            max_chains: Optional limit on retained assembly chains.
            assembly_id (str, optional): Assembly ID. Defaults to "1".
            max_assembly_chains (int, optional): Max allowed chains in the assembly.
                Defaults to 1000.
        ...
        Returns:
            ...
            including:
                ...
                - "num_assembly_polymer_chains": The number of polymer chains in the
                assembly.
                - "num_prot_chains": The number of protein chains in the assembly.
                - "entity_poly_type": The type of polymer entities.
                - "resolution": The resolution of the structure. Set to -1.0 if
        ...
        pipeline_functions: list[tuple[str, Callable[[AtomArray], AtomArray]]] = [
            ("remove_water", ResidueFilter.remove_water),
            ("remove_hydrogens", ResidueFilter.remove_hydrogens),
            (
                "remove_polymer_chains_all_residues_unknown",
                lambda aa: ResidueFilter.remove_polymer_chains_all_residues_unknown(
            ...
            (
                ...
                lambda aa: (
                    ResidueFilter.remove_polymer_chains_with_consecutive_c_alpha_too_far_away(
                        aa, self.entity_poly_type
                    )
                ),
            ),
            ...
            ("remove_ligand_absent_atoms", ResidueFilter.remove_ligand_absent_atoms),
            (
                "remove_ligand_unresolved_leaving_atoms",
                ResidueFilter.remove_ligand_unresolved_leaving_atoms,
            ),
            (
                "remove_element_X",
                ResidueFilter.remove_element_x,
                # remove X element (including ASX->ASP, GLX->GLU) after
                # the missing-atom repair stage.
            ),
            ("remove_unresolved_chains", ResidueFilter.remove_unresolved_chains),
        ]

        if set(self.methods) & CRYSTALLIZATION_METHODS:
            # AF3 SI 2.5.4 Crystallization aids are removed if the mmCIF method
            # information indicates that crystallography was used.
            pipeline_functions.append(
                (
                    "remove_crystallization_aids",
                    lambda aa: ResidueFilter.remove_crystallization_aids(
                        aa, self.entity_poly_type
                    ),
                )
            )
        ...
        atom_array = ResidueAddAtomArrayAnnot.add_token_mol_type(
            atom_array, self.entity_poly_type
        )
        atom_array = ResidueAddAtomArrayAnnot.add_centre_atom_mask(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_atom_mol_type_mask(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_distogram_rep_atom_mask(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_plddt_m_rep_atom_mask(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_cano_seq_resname(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_tokatom_idx(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_modified_res_mask(atom_array)
        if (
            atom_array.centre_atom_mask.sum()
            != atom_array.distogram_rep_atom_mask.sum()
        ):
            message = (
                "Invalid state: atom_array.centre_atom_mask.sum() == "
                "atom_array.distogram_rep_atom_mask.sum()"
            )
            raise ValueError(message)

        # remove asymmetric polymer ligand bonds (including protein-protein bond, like
        # disulfide bond)
        ...
        with stat_logger.log(
            ...
            atom_array = ResidueFilter.remove_asymmetric_polymer_ligand_bonds(
                atom_array, self.entity_poly_type
            )
            logger.atom_array = atom_array
        ...
        # rename chain_ids from A A B to A A.1 B and add asym_id_int, entity_id_int,
        # sym_id_int
        atom_array = ResidueAddAtomArrayAnnot.unique_chain_and_add_ids(atom_array)
        ...
        with stat_logger.log(
            ...
            removed_chain_ids = ResidueFilter.remove_clashing_chains(
                atom_array,
            )
            atom_array = atom_array[~np.isin(atom_array.chain_id, removed_chain_ids)]
            logger.atom_array = atom_array

        # add_mol_id before applying the two filters below to ensure that covalent
        # components are not removed as individual chains.
        atom_array = self.sort_chains_by_entity_id(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.find_equiv_mol_and_assign_ids(
            atom_array, self.entity_poly_type
        )
        ...
        if max_chains is not None:
            with stat_logger.log(
                ...
                atom_array, _ = ResidueFilter.too_many_chains_filter(
                    atom_array,
        ...
        atom_array = ResidueAddAtomArrayAnnot.add_ref_space_uid(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_ref_info_and_res_perm(atom_array)
        ...
        with stat_logger.log(
            ...
            atom_array = ResidueFilter.remove_dissociation(atom_array, all_chain_pairs)
            logger.atom_array = atom_array
    ...
    def find_non_ccd_leaving_atoms(
        ...
        """Repair residue/CCD leaving-atom mismatches.
        ...
        Args:
            ...
            indices_in_atom_array (list[int]): indices of equivalent non-CCD central
                atoms across different chains.
        ...
        for idx in indices_in_atom_array:
            ...
            ref_neighbor_idx, _types = component.bonds.get_bonds(idx_in_comp)
            # neighbor_atom only bond to central atom in CCD component
            ref_neighbor_idx = [
                i
                for i in ref_neighbor_idx
                if len(component.bonds.get_bonds(int(i))[0]) == 1
            ]
        ...
        max_id = np.argmax([len(names) for names in remove_atom_names])
    ...
    def build_ref_chain_with_atom_array(
        ...
        annots_to_indices = residue_map_annotations_to_atom_indices(
            atom_array, annot_keys=["label_entity_id", "res_id", "atom_name"]
        )

        # count inter residue bonds of each potential central atom for removing leaving
        # atoms later
        ...
        for entity_id in self.entity_poly_type:
            ...
            for res_id, res_name in enumerate(poly_res_names[entity_id]):
                # keep all leaving atoms, will remove leaving atoms later in this
                # function
            ...
            inter_bonds = ccd._connect_inter_residue(chain, res_starts)  # noqa: SLF001 - Biotite extension hook

            # skip std polymer bonds between residue with non-std polymer bonds
            bond_mask = np.ones(len(inter_bonds.as_array()), dtype=bool)
            for b_idx, (atom_i, atom_j, _b_type) in enumerate(inter_bonds.as_array()):
                ...
                # 1. Standard polymer bond missing in atom_array suggests possible
                ...
                for i, ci in zip(same_pos_i, same_pos_i_chain_id, strict=False):
                    for j, cj in zip(same_pos_j, same_pos_j_chain_id, strict=False):
                        if ci == cj:
            ...
            inter_bonds = _replace_bond_array(
                inter_bonds, inter_bonds.as_array()[bond_mask]
            )
            chain.bonds = (
                chain.bonds.merge(inter_bonds)
                if chain.bonds is not None
                else inter_bonds
            )
        ...
        inter_residue_bonds = residue_get_inter_residue_bonds(atom_array)
        for i in inter_residue_bonds.flat:
            ...
            # remove leaving atoms if central atom has inter residue bond in any copy of
            # a entity
        ...
        residue_maps: dict[str, dict[int, AtomArray]] = {}
        for entity_id, raw_chain in entity_residues.items():
            chain = raw_chain
            ...
            for start, stop in itertools.pairwise(starts):
                ...
                for i in range(start, stop):
                    ...
                    # num of remove leaving groups equals to num of inter residue bonds
                    component = ccd.get_component_atom_array(
                        ccd_code=res_name, keep_leaving_atoms=True
                    )

                    if component.central_to_leaving_groups is None:
                        # The leaving atoms might be labeled wrongly. The residue
                        # remains as it is.
                        break

                    # central_to_leaving_groups:dict[str, list[list[str]]], central atom
                    # name to leaving atom groups (atom names).
            ...
            residue_maps[entity_id] = {
                int(r.res_id[0]): r for r in struc.residue_iter(chain)
            }
        return residue_maps
    ...
    def make_new_residue(
        ...
        res_start: int,
        res_stop: int,
        annots_to_indices: dict[tuple, list],
    ) -> AtomArray:
        """Make new residue.

        make new residue from atom_array[res_start:res_stop], ref_chain is the
        reference chain.
        ...
        Args:
            ...
            annots_to_indices (dict[tuple, list]): entity_id, res_id, atom_name ->
                indices in atom_array.
        ...
        if ref_residue.central_to_leaving_groups is None:
            # ambiguous: one leaving group bond to more than one central atom, keep same
            # atoms with PDB entry.
            return atom_array[res_start:res_stop]
        ...
        for raw_i, central_atom_name in enumerate(old_atom_names):
            i = raw_i
            i += res_start
            bonds = atom_array.bond_map[i]
            ...
            if bond_count == 0:
                # central atom is not covalent to other residue, not remove leaving
                # atoms
                continue
            ...
            if central_atom_name in ref_residue.central_to_leaving_groups:
                ...
                if bond_count >= len(leaving_groups):
                    ...
                    # not remove leaving atoms of B and BE, if all leaving atoms is
                    # exist in atom_array
                    if (central_atom_name in ["B", "BE"]) and (not not_exist_group):
                        continue
        ...
        return self.create_empty_annotation_like(atom_array, new_residue)
    ...
    def add_missing_atoms_and_residues(self, atom_array: AtomArray) -> AtomArray:
        ...
        Args:
            atom_array (AtomArray): structure with missing residues and atoms, from PDB
                entry.

        Returns:
            AtomArray: structure added missing residues and atoms (label
                atom_array.is_resolved as False).
        ...
        for atom_i, atom_j, _b_type in atom_array.bonds.as_array():
        ...
        for c_start, c_stop in itertools.pairwise(chain_starts):
            ...
            prev_res_id = 0
            curr_res_id = 0
            for r_start, r_stop in itertools.pairwise(c_res_starts):
                curr_res_id = int(atom_array.res_id[r_start])
                ...
                for k in residue_fields:
                    v = atom_array.get_annotation(k)[r_start]
                    new_residue.get_annotation(k)[:] = v
                ...
                for old_idx in range(r_start, r_stop):
                    ...
                    if old_name not in name_to_index_new:
                        ...
                        # For residues or small molecules with CCD codes, atoms outside
                        # of the CCD code's defined set of atom names are removed.
                ...
                if len(res_o2n_amap) > len(res_mismatch_idx):
                    # Match residues only if more than half of their resolved atoms are
                    # matched.
                    # e.g. 1gbt GBS shows 2/12 match, not add to o2n_amap, all atoms are
                    # marked as is_resolved=False.
            ...
            for k in chain_fields:
                chain_array.get_annotation(k)[:] = atom_array.get_annotation(k)[c_start]
        ...
        for k in atom_fields:
            if k not in atom_array.get_annotation_categories():
                continue
            new_array.get_annotation(k)[new_idx] = atom_array.get_annotation(k)[old_idx]
        ...
        # some non-leaving atoms are not in the new_array for atom name mismatch, e.g.
        # 4msw TYF
        ...
        delattr(atom_array, "bond_map")

        # add peptide bonds and nucleic acid bonds based on CCD type
        return ccd.add_inter_residue_bonds(
    ...
    def find_interfaces(
        ...
        *,
        ...
        Args:
            ...
            keep_all_entity_chain_pair (bool, optional): Whether to keep all chain
                pairs. Defaults to False.

        Returns:
            tuple:
                dict[tuple[str, str], list[tuple[str, str]]]: entity pair to chain
                pairs.
                                                            Only include chains in asym
                                                            unit
                                                            and interfaces which at
                                                            least have
        ...
        chain_id_to_entity = {
            ...
            for chain_id, chain_start in zip(
                *np.unique(atom_array.chain_id, return_index=True), strict=False
            )
        }
        ...
        for chain_i in np.unique(atom_array.chain_id[atom_array.is_resolved]):
            ...
            for chain_j in chain_j_array:
                ...
                # Sort by entity pair
                sorted_pairs = sorted(
                    zip([entity_i, entity_j], [chain_i, chain_j], strict=False),
                    key=lambda x: x[0],
                )
                entity_key, chain_pair = zip(*sorted_pairs, strict=False)
    ...
    def make_interface_indices(
        ...
        *,
        ...
        As described in SI 2.5.1, interfaces defined as pairs of chains with minimum
        heavy atom
        ...
        Args:
            ...
            include_all_chain_pairs (bool, optional): Whether to include all chain
                pairs. Defaults to False.

        """
        chain_indices_dict = {i["chain_id"]: i for i in chain_indices_list}
        interface_indices_list = []
        for chain_pairs in entity_pair_to_chain_pairs.values():
            if not chain_pairs:
                continue
    ...
    def add_sub_mol_type(
        ...
        Args:
            ...
            lig_polymer_bond_chain_id (np.ndarray): a chain id list of ligands that are
                bonded to polymer.

        Returns:
            dict[str, Any]: A dict of chain or interface indices info with
            "sub_mol_[i]_type" field.
    ...
    def add_eval_type(indices_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: A dict of chain or interface indices info with "eval_type"
            field.

        """
        if indices_dict["mol_type_group"] not in [
            "intra_nuc",
            "nuc_prot",
        ] or "dna_rna_hybrid" in [
            indices_dict["sub_mol_1_type"],
            indices_dict["sub_mol_2_type"],
        ]:
            eval_type = indices_dict["mol_type_group"]
        elif indices_dict["mol_type_group"] == "intra_nuc":
            nuc_type = str(indices_dict["sub_mol_1_type"]).rsplit("_", maxsplit=1)[-1]
            eval_type = f"intra_{nuc_type}"
        else:
            nuc_type1 = str(indices_dict["sub_mol_1_type"]).rsplit("_", maxsplit=1)[-1]
            nuc_type2 = str(indices_dict["sub_mol_2_type"]).rsplit("_", maxsplit=1)[-1]
            eval_type = "dna_prot" if "dna" in [nuc_type1, nuc_type2] else "rna_prot"
    ...
    def make_indices(
        ...
        *,
        ...
        Args:
            pdb_cluster_file: PDB sequence cluster mapping file.
            bioassembly_dict (dict): dict from MMCIFParser.get_bioassembly().
            cluster_file (str): PDB cluster file. Defaults to None.
            skip_expanded_chains (bool): Whether to skip expanded chains ("." in
                chain_id).
                                         Defaults to True.
            include_all_chain_pairs (bool): Whether to include all chain pairs.
                Defaults to False.
        ...
        interface_indices_list = self.make_interface_indices(
            chain_indices_list,
            entity_pair_to_chain_pairs,
            include_all_chain_pairs=include_all_chain_pairs,
        )
        ...
        for chain_dict in chain_indices_list:
            ...
            chain_dict_out.update({k.replace("_", "_2_"): "" for k in chain_dict})
        ...
        polymer_lig_bonds = get_ligand_polymer_bond_mask(atom_array=atom_array)
        if len(polymer_lig_bonds) == 0:
            lig_polymer_bond_chain_id = np.empty(0, dtype=str)
        else:
            lig_polymer_bond_chain_id = atom_array.chain_id[
                np.unique(polymer_lig_bonds[:, :2])
            ]

        for raw_indices in sample_indices_list:
            indices = raw_indices
            for i in ["1", "2"]:
...
class PoseBusterMMCIFParser(ResidueMMCIFParser):
    """Represent pose buster m m c i f parser."""
    ...
    def get_altloc_by_ligand_label_asym_id(self, lig_label_asym_id: str) -> str:
        ...
        """
        atom_site = self.cif.block["atom_site"]
    ...
    def get_structure_dict(self, lig_label_asym_id: str) -> dict[str, Any]:
        ...
        pipeline_functions: list[Callable[[AtomArray], AtomArray]] = [
            ResidueFilter.remove_water,
            ResidueFilter.remove_hydrogens,
            ResidueFilter.remove_element_x,
            self.fix_arginine,
            # and add annotation is_resolved (False for missing atoms)
            self.add_missing_atoms_and_residues,
        ...
        # rename chain_ids from A A B to A0 A1 B0 and add asym_id_int, entity_id_int,
        # sym_id_int
        atom_array = ResidueAddAtomArrayAnnot.unique_chain_and_add_ids(atom_array)
        ...
        atom_array = ResidueFilter.remove_unresolved_chains(atom_array)

        # numerical encoding of (chain id, residue index)
        atom_array = ResidueAddAtomArrayAnnot.add_ref_space_uid(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_ref_info_and_res_perm(atom_array)

        # Another PDB (8F4J) was too large to inference the entire system (over 5120
        # tokens)
        # so we only included protein chains within 20 Å of the ligand of interest.
    ...
    def cut_struct_by_ligand(
        ...
        return atom_array[(other_neighbor_mask & other_components) | neighbor_prot_mask]
    ...
    def filter_indices_by_lig(
        ...
        Args:
            indices_list (list[dict[str, Any]]): The complete indices list generated
                                                 by the make_indices method in the
                                                 superclass.
            structure_dict (dict): The structure_dict generated by the
                get_structure_dict method.
            lig_label_asym_id (str): The label_asym_id of interested ligand.
        ...
        pocket_indices_list = []
        for indices_dict in indices_list:
            chain_ids = {indices_dict["chain_1_id"], indices_dict["chain_2_id"]}
            if (
                indices_dict["type"] == "chain" and len(pocket_chains & chain_ids) == 1
            ) or len(pocket_chains & chain_ids) == 2:
                indices_dict["is_pocket"] = 1
                pocket_indices_list.append(indices_dict)

        return lig_indices_list + pocket_indices_list


class RecentPDBMMCIFParser(ResidueMMCIFParser):
    """Represent recent p d b m m c i f parser."""

    def get_bioassembly(
        self,
        assembly_id: str = "1",
        max_assembly_chains: int | None = None,
        max_chains: int | None = None,
    ) -> dict[str, Any]:
        """Ref: AlphaFold3 SI Chapter 6.1.

        The recent PDB evaluation set construction started by taking all 10,192 PDB
        entries
        ...
        Each entry in the date range was expanded from the asymmetric unit to
        Biological Assembly 1,
        then two filters were applied:

        Filtering to non-NMR entries with resolution better than 4.5 Å, leaving 9,636
        complexes.
        Filtering to complexes with less than 5,120 tokens under our tokenization
        scheme, leaving 8,856 complexes.

        Args:
            max_assembly_chains: Optional limit on expanded assembly chains.
            max_chains: Optional limit on retained assembly chains.
            assembly_id (str, optional): Assembly ID. Defaults to "1".
        ...
        pipeline_functions: list[Callable[[AtomArray], AtomArray]] = [
            ...
            # and add annotation is_resolved (False for missing atoms),
            self.add_missing_atoms_and_residues,
            ResidueFilter.remove_ligand_absent_atoms,
            ResidueFilter.remove_ligand_unresolved_leaving_atoms,
            ResidueFilter.remove_unresolved_chains,
            lambda aa: ResidueFilter.remove_crystallization_aids(
                aa, self.entity_poly_type
            ),
        ]
        ...
        if (
            max_assembly_chains is not None
            and len(np.unique(atom_array.chain_id)) > max_assembly_chains
        ):
            return bioassembly_dict
        if max_chains is not None:
            atom_array, _ = ResidueFilter.too_many_chains_filter(
                atom_array, max_chains_num=max_chains
            )
            if atom_array is None:
                return bioassembly_dict
        ...
        if atom_array is None:
            # The distance between the central atoms in any two chains is greater than
            # 15 angstroms.
            return bioassembly_dict
        ...
        for start, end in itertools.pairwise(chain_starts):
            ...
            max_res_num_per_chain = max(max_res_num_per_chain, res_num)
    ...
    def change_lig_mol_type(indices_dict: dict[str, Any]) -> dict[str, Any]:
        ...
        our ligand exclusion list and glycans Bonded and non-bonded ligands are
    ...
    def change_cluster_id(
        ...
        Individual polymer chains were clustered at a 40% sequence similarity
        clustering
        ...
        Polymer-polymer interfaces are given a cluster ID of (polymer1_cluster,
        polymer2_cluster).
        Polymer-ligand interfaces are given a cluster ID of the polymer_cluster only.
        Polymer-modified_residue interfaces are given a cluster ID (polymer_cluster,
        CCD-code).
        ...
        Returns:
            dict[str, Any]: A dict of chain or interface indices info with "cluster_id"
            changed.
        ...
        for i in ["1", "2"]:
            ...
            if "modified_" in sub_mol_type:
                ...
                if sub_mol_type == "modified_prot":
                    ...
                    msg = f"Unknown sub_mol_type: {sub_mol_type}"
                    raise KeyError(msg)

                un_std_res = sorted(set(chain_all_res_name) - set(std_res))
    ...
    def modified_recent_pdb_indices(
        ...
        Peptide-peptide interfaces, peptide monomers and modified residues within
        peptides
        (where a peptide here is defined as a protein with less than 16 residues)
        were not included in scoring as their homology to the training set was not
        determined.

        The system can predict other entities like DNA/RNA hybrids, Peptide Nucleic
        Acids (PNA) and (D) polypeptides,
        but these entities and interfaces involving them were not scored as they are
        ...
        for raw_indices_dict in sample_indices_list:
            indices_dict = raw_indices_dict
            ...
            if any(d.get("modified_peptides", False) for d in entity_dicts):
                # Modified peptide interfaces currently remain in the index.
                pass
...
class DistillationMMCIFParser(ResidueMMCIFParser):
    """Represent distillation m m c i f parser."""

    def get_structure_dict(
        self,
        *,
        add_missing_atom: bool = True,
        ccd_mols: dict[str, Chem.Mol] | None = None,
    ) -> dict[str, Any]:
        """Get an AtomArray from a CIF file of distillation data.

        Args:
            add_missing_atom (bool, optional): Whether to add missing atoms. Defaults
                to True.
            ccd_mols (dict[str, Chem.Mol], optional): CCD code to mol dict. Defaults to
                None.
        ...
        # numerical encoding of (chain id, residue index)
        atom_array = ResidueAddAtomArrayAnnot.add_ref_space_uid(atom_array)
        atom_array = ResidueAddAtomArrayAnnot.add_ref_info_and_res_perm(
...
class _CommonAddAtomArrayAnnot:
    """Represent common add atom array annot."""
    ...
    def add_token_mol_type(
        ...
        Args:
            atom_array (AtomArray): Biotite AtomArray object.
            sequences (dict[str, str]): A dict of label_entity_id -->
                canonical_sequence
    ...
    def add_modified_res_mask(atom_array: AtomArray) -> AtomArray:
        ...
        Modified residue scores are ranked according to the average pLDDT of the
        modified residue.
    ...
    def add_centre_atom_mask(atom_array: AtomArray) -> AtomArray:
        """Ref: AlphaFold3 SI Chapter 2.6.
            ...
            • A standard amino acid residue (Table 13) is represented as a single
            token.
            • A standard nucleotide residue (Table 13) is represented as a single
            token.
            • A modified amino acid or nucleotide residue is tokenized per-atom (i.e. N
            tokens for an N-atom residue)
            • All ligands are tokenized per-atom
        For each token we also designate a token centre atom, used in various places
        below:
            • Calpha for standard amino acids
            • C1' for standard nucleotides
            • For other cases take the first and only atom as they are tokenized
            per-atom.
        ...
        Returns:
            AtomArray: Biotite AtomArray object with "centre_atom_mask" annotation
                added.
    ...
    def add_distogram_rep_atom_mask(atom_array: AtomArray) -> AtomArray:
        ...
        • Cβ for protein residues (Calpha for glycine),
        • C4 for purines and C2 for pyrimidines.
        • All ligands already have a single atom per token.

        Due to the lack of explanation regarding the handling of "N" and "DN" in the
        article,
        it is impossible to determine the representative atom based on whether it is a
        ...
        Returns:
            AtomArray: Biotite AtomArray object with "distogram_rep_atom_mask"
                annotation added.
    ...
    def add_plddt_m_rep_atom_mask(atom_array: AtomArray) -> AtomArray:
        ...
        • Atoms such that the distance in the ground truth between atom l and atom m is
        less than 15 Å
            if m is a protein atom or less than 30 Å if m is a nucleic acid atom.
        • Only atoms in polymer chains.
        • One atom per token - Calpha for standard protein residues
        ...
        Returns:
            AtomArray: Biotite AtomArray object with "plddt_m_rep_atom_mask" annotation
                added.
    ...
    def add_ref_space_uid(atom_array: AtomArray) -> AtomArray:
        ...
        reference conformer.
        Each (chain id, residue index) tuple is assigned an integer on first
        appearance.
    ...
    def add_cano_seq_resname(atom_array: AtomArray) -> AtomArray:
        ...
        Residues that cannot be mapped to standard residues and ligands are all labeled
        as "UNK".

        Note: Some CCD Codes in the canonical sequence are mapped to three letters. It
            is labeled as one "UNK".

        Args:
            atom_array (AtomArray): Biotite AtomArray object

        Returns:
            AtomArray: Biotite AtomArray object with "cano_seq_resname" annotation
                added.
        ...
        for start, stop in itertools.pairwise(starts):
            ...
            if one_letter_code is None or len(one_letter_code) != 1:
                # Some non-standard residues cannot be mapped back to one standard
                # residue.
    ...
    def remove_bonds_between_polymer_chains(
        ...
        Returns:
            BondList: Biotite BondList object (copy) with bonds between polymer chains
                removed

        """
        if atom_array.bonds is None:
            return struc.BondList(len(atom_array))
        ...
        return _replace_bond_array(bonds, bonds.as_array()[~pp_bond_mask])
    ...
    def add_tokatom_idx(atom_array: AtomArray) -> AtomArray:
        ...
        Args:
            atom_array: Atoms receiving the token-to-atom index annotation.
    ...
    def unique_chain_and_add_ids(atom_array: AtomArray) -> AtomArray:
        """Assign a unique chain ID and add asym_id, entity_id, sym_id.
        ...
        for start, stop in itertools.pairwise(chain_starts):
        ...
        if not ("" not in chain_ids):
            message = "Invalid state: '' not in chain_ids"
            raise ValueError(message)
        # reset chain id
...
class ResidueAddAtomArrayAnnot(_CommonAddAtomArrayAnnot):
    """Represent residue add atom array annot.
    ...
    def find_equiv_mol_and_assign_ids(
        ...
        All atoms connected by covalent bonds are considered as a molecule, with unique
        mol_id (int).
        different copies of same molecule will assign same entity_mol_id (int).
        for each mol, assign mol_atom_index starting from 0.

        Args:
            pdb_id: Structure identifier used in diagnostics.
            atom_array (AtomArray): Biotite AtomArray object
            entity_poly_type (Optional[dict[str, str]]): label_entity_id to
                entity.poly_type.
                              Defaults to None.
        ...
        Returns:
            ...
            - entity_mol_id: equivalent molecules will assign same entity_mol_id,
            0-based int
        ...
        if entity_poly_type is None:
            ...
            bonds_filtered = (
                ResidueAddAtomArrayAnnot.remove_bonds_between_polymer_chains(
        ...
        for atom_indices in mol_indices:
            ...
            for i in itertools.pairwise(chain_ids_in_mol):
        ...
        if not (~np.isin(-1, atom_array.mol_id)):
            message = "Some mol_id is not assigned."
            raise ValueError(message)
        ...
        for mol_ids in entity_mol_dict.values():
            checked_mol_id = []
    ...
    @staticmethod
    def add_int_id(atom_array: AtomArray) -> AtomArray:
        """Assign unique chain IDs and add asym_id, entity_id and sym_id.
    ...
    def add_ref_feat_info(
        atom_array: AtomArray, ccd_mols: dict[str, Chem.Mol] | None = None
        ...
        Returns:
            tuple:
                ref_pos (numpy.ndarray): Atom positions in the reference conformer,
                                         with a random rotation and translation
                                         applied.
                                         Atom positions are given in Å.
                                         Shape=(num_atom, 3).
                ref_charge (numpy.ndarray): Charge for each atom in the reference
                    conformer. Shape=(num_atom)
                ref_mask ((numpy.ndarray): Mask indicating which atom slots are used in
                    the reference conformer. Shape=(num_atom)

        """
        molecule_key = tuple(ccd_mols.items()) if ccd_mols is not None else None

        info_dict = {}
        for ccd_id in np.unique(atom_array.res_name):
            # create ref conformer for each CCD ID
            ref_result = get_ccd_ref_info(str(ccd_id), ccd_mols=molecule_key)
            if ref_result:
        ...
        for atom_name, ref_space_uid in zip(
            atom_array.atom_name, atom_array.ref_space_uid, strict=False
        ):
    ...
    def add_res_perm(
        ...
        for start, stop in itertools.pairwise(starts):
            ...
            perm_idx_to_present_atom_idx = dict(
                zip(perm_atom_idx_in_res_order, curr_res_atom_idx, strict=False)
            )
            ...
            # replace the elem in new_perm_array according to the
            # perm_idx_to_present_atom_idx dict
            new_perm_array = np.vectorize(perm_idx_to_present_atom_idx.get)(
                perm_array_filtered
            )

            if not (new_perm_array.shape[1] <= 1000):
                message = "Invalid state: new_perm_array.shape[1] <= 1000"
                raise ValueError(message)
            if not (new_perm_array.shape[1] <= perm_array.shape[1]):
                message = (
                    "Invalid state: new_perm_array.shape[1] <= perm_array.shape[1]"
                )
                raise ValueError(message)
            if new_perm_array.shape[0] != stop - start:
                message = (
                    f"Number of atoms in residue ({stop - start}) "
                    f"does not match the number of permutations "
                    f"({new_perm_array.shape[0]}). "
    ...
    def add_ref_info_and_res_perm(
        ...
        Returns:
            AtomArray: The atom array with the 'ref_pos', 'ref_charge', 'ref_mask',
                'res_perm' annotations added.

        """
        ref_pos, ref_charge, ref_mask = ResidueAddAtomArrayAnnot.add_ref_feat_info(
            atom_array, ccd_mols
        )
        res_perm = ResidueAddAtomArrayAnnot.add_res_perm(atom_array, ccd_mols)

        str_res_perm = []  # encode [N_atom, N_perm] -> list[str]
        str_res_perm.extend("_".join([str(j) for j in i]) for i in res_perm)
        ...
        if not (
            ...
            message = (
                f"{len(atom_array)=}, {len(ref_pos)=}, "
                f"{len(ref_charge)=}, "
                f"{len(ref_mask)=}, {len(str_res_perm)=}"
            )
            raise ValueError(message)
...
class StructuralMMCIFParser(_CommonMMCIFParser):
    """Parsing and extracting information from mmCIF files."""

    def __init__(
        self,
        mmcif_file: str | Path | None = None,
        mmcif_string: str | None = None,
    ) -> None:
        self.mmcif_file = mmcif_file
        self.cif = self._parse(mmcif_file=mmcif_file, mmcif_string=mmcif_string)
    ...
    def get_structure(
        ...
        if atom_site.row_count > 1000_000:
            msg = "The file contains more than 1,000,000 atom_site rows"
            raise ValueError(msg)
        ...
        # some pdb have insertion codes, such as 4v5s
        # so we use label_seq_id to iter res
        atom_array = StructuralFilter.filter_altloc(atom_array, altloc=altloc)
    ...
    def build_ref_chain_with_atom_array(
        ...
        """
        # make entity-level annotations to atom indices mapping
        annots_to_indices = structural_map_annotations_to_atom_indices(
            atom_array, annot_keys=["label_entity_id", "res_id", "atom_name"]
        )
        ...
        # count inter residue bonds from atom_array for removing leaving atoms later
        inter_residue_bonds = structural_get_inter_residue_bonds(atom_array)
        for i in inter_residue_bonds.flat:
            bonds = atom_array.bond_map[i]
            bond_count = (
                (atom_array.res_id[bonds] != atom_array.res_id[i])
        ...
        # so the residue in reference chain can be used directly.
        entity_residues: dict[str, dict[int, AtomArray]] = {}
        for entity_id, raw_chain in entity_chains.items():
            chain = raw_chain
            keep_atom_mask = np.ones(len(chain), dtype=bool)
            starts = struc.get_residue_starts(chain, add_exclusive_stop=True)
            for start, stop in itertools.pairwise(starts):
...
class StructuralAddAtomArrayAnnot(_CommonAddAtomArrayAnnot):
    """Represent structural add atom array annot.
    ...
    def add_token_annotations(
        ...
        """
        atom_array = StructuralAddAtomArrayAnnot.add_token_mol_type(
            atom_array, entity_poly_type
        )
        atom_array = StructuralAddAtomArrayAnnot.add_centre_atom_mask(atom_array)
        atom_array = StructuralAddAtomArrayAnnot.add_atom_mol_type_mask(atom_array)
        atom_array = StructuralAddAtomArrayAnnot.add_distogram_rep_atom_mask(atom_array)
        atom_array = StructuralAddAtomArrayAnnot.add_plddt_m_rep_atom_mask(atom_array)
        atom_array = StructuralAddAtomArrayAnnot.add_cano_seq_resname(atom_array)
        atom_array = StructuralAddAtomArrayAnnot.add_tokatom_idx(atom_array)
        return StructuralAddAtomArrayAnnot.add_modified_res_mask(atom_array)
    ...
    def find_equiv_mol_and_assign_ids(
        ...
        if entity_poly_type is None:
            ...
            bonds_filtered = (
                StructuralAddAtomArrayAnnot.remove_bonds_between_polymer_chains(
                    atom_array[valid_mask], entity_poly_type
                )
            )
            mol_indices: list[np.ndarray] = get_molecule_indices(bonds_filtered)
