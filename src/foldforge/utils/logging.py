...
class WarningFilter(logging.Filter):
    """Represent warning filter."""

    def filter(self, record: logging.LogRecord) -> bool:
        # Filter out specific warnings by their message or other criteria
        """Filter ."""
        return "simtk.openmm" not in record.getMessage()


def get_logger(
    name: str = "", loglevel: str = "INFO", log_file_path: str | None = None
) -> logging.Logger:
...
def add_handlers(
    logger: logging.Logger, loglevel: str, log_file_path: str | None = None
) -> logging.Logger:
