...
@triton.jit
def _attention_core(  # noqa: PLR0915 - single Triton kernel program
    Q,
    K,
    V,
    mask,
    ...
    use_mask_1d: tl.constexpr,
    use_mask_2d: tl.constexpr,
    ...
    if use_bias:
        ...
        offs_base_bias = off_hz_bias * (N_CTX * N_CTX) + offs_m[:, None] * N_CTX
    ...
    for raw_start_n in range(0, N_CTX, BLOCK_N):
        start_n = raw_start_n
        ...
        if use_bias:
            bias_load_mask = (offs_m[:, None] < N_CTX) & (
                (start_n + offs_n)[None, :] < N_CTX
            )
            bias_data = tl.load(
                bias + offs_base_bias + offs_n + start_n, mask=bias_load_mask, other=0.0
            )
            qk += bias_data
        ...
        if use_mask_1d or use_mask_2d:
            ...
            qk = tl.where(mask_data[None, :] == 0.0, float("-1e9"), qk)
...
def dot_product_attention_triton(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    ...
    """Compute dot product attention triton."""
    if q.dtype not in [torch.float16, torch.bfloat16]:
        message = "triton flash attention only support float16/bfloat16 now"
    ...
    if Lq != Lk:
        message = "Invalid state: Lq == Lk"
        raise ValueError(message)
    if Lk != Lv:
        message = "Invalid state: Lk == Lv"
        raise ValueError(message)
    if Lk not in {16, 32, 64, 128}:
        message = "Invalid state: Lk in {16, 32, 64, 128}"
        raise ValueError(message)
    ...
    _attention_core.run(
        ...
        BLOCK_N=BLOCK,
        BLOCK_DMODEL=Lk,
        use_mask_1d=(mask is not None) and len(mask.shape) == 1,
        use_mask_2d=(mask is not None) and len(mask.shape) == 2,
...
def dot_product_attention_torch(
    ...
    """Compute dot product attention torch."""
    from team_gm.modules.blocks.attention_math import attention
    ...
    if mask is not None:
        if mask.ndim == 1:
            ...
            mask = mask[:, None, None, :]
    return attention(
        q, k, v, bias=bias, mask=mask, scale=q.shape[-1] ** -0.5, logits_float32=False
    )
...
def dot_product_attention(
    ...
    if fastnn_config.dot_product_attention_implementation == "torch":
        ...
        msg = (
            f"Unknown dot_product_attention_implementation: "
            f"{fastnn_config.dot_product_attention_implementation}"
        )
