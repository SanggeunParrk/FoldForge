...
from foldforge.data.msa.formats import parse_fasta
...
class MultipleChainsError(ParsingError):
    """Represent multiple chains error.
...
def encode_template_restype(chain_type: str, sequence: str) -> list[int]:
    """Encode a sequence of residues into integer IDs based on chain type."""
    ...
    msg = f"Unsupported chain type: {chain_type}"
    raise NotImplementedError(msg)


def get_pdb_id_and_chain(hit: "TemplateHit") -> tuple[str, str]:
    """Extract PDB ID and chain from hit name."""
    match = re.match(r"([a-zA-Z\d]{4})_([a-zA-Z0-9.]+)", hit.name)
    if not match:
        msg = f"Invalid hit name format: {hit.name}"
        raise ValueError(msg)
    return match.group(1).lower(), match.group(2)


class _CommonTemplateParser:
    """Represent common template parser."""

    @staticmethod
    def _mmcif_loop_to_list(
        prefix: str, parsed_info: MmCIFDict
    ) -> list[dict[str, str]]:
        """Extract an mmCIF loop into a list of dictionaries."""
        cols = []
        data = []
    ...
    def _get_release_date(parsed_info: MmCIFDict) -> str:
        """Return the oldest revision date from mmCIF data."""
    ...
    def _is_set(data: str) -> bool:
        """Check if mmCIF data is set (not '.' or '?')."""
        return data not in (".", "?")


class ResidueTemplateParser(_CommonTemplateParser):
    ...
    def _mmcif_loop_to_dict(
        ...
        """Extract an mmCIF loop into a dictionary keyed by a specific column."""
        entries = ResidueTemplateParser._mmcif_loop_to_list(prefix, parsed_info)
        return {entry[index_key]: entry for entry in entries}

    @staticmethod
    def _get_header(parsed_info: MmCIFDict) -> PdbHeader:
        """Extract basic header information (method, release date, resolution)."""
        header = {}
        experiments = ResidueTemplateParser._mmcif_loop_to_list("_exptl.", parsed_info)
        header["structure_method"] = ",".join(
            [exp.get("_exptl.method", "").lower() for exp in experiments]
        )

        release_date = ResidueTemplateParser._get_release_date(parsed_info)
        if release_date:
    ...
    def _get_protein_chains(parsed_info: MmCIFDict) -> dict[ChainId, Sequence[Monomer]]:
        """Extract valid protein chains and their sequences."""
        entity_poly_seqs = ResidueTemplateParser._mmcif_loop_to_list(
        ...
        chem_comps = ResidueTemplateParser._mmcif_loop_to_dict(
            "_chem_comp.", "_chem_comp.id", parsed_info
        )
        struct_asyms = ResidueTemplateParser._mmcif_loop_to_list(
    ...
    def parse_simple_cif(
        *,
        ...
        """Parse an simplified mmCIF string into an MmcifObject.

        Since the simplified mmCIF might lack complete polymer metadata, we extract the
        ...
        try:
            ...
            if structure is None:
                message = "mmCIF parser returned no structure"
                raise ValueError(message)  # noqa: TRY301 - normalize this record failure through the existing handler
            first_model = ResidueTemplateParser._get_first_model(structure)

            chains = list(first_model.get_chains())
            if not chains:
                return ParsingResult(
                    None, {(file_id, ""): "No chains found in simplified mmCIF."}
                )
            ...
            mmcif_obj = MmcifObject(
                ...
                raw_string=parser._mmcif_dict,  # noqa: SLF001 - Biotite extension hook
            )
    ...
    def parse(
        *,
        ...
        """Parse an mmCIF string into an MmcifObject."""
        errors = {}
        try:
            parser = MMCIFParser(QUIET=True)
            ...
            if structure is None:
                ...
                raise ValueError(message)  # noqa: TRY301 - normalize this record failure through the existing handler
            first_model = ResidueTemplateParser._get_first_model(structure)
            parsed_info = parser._mmcif_dict  # pylint: disable=protected-access  # noqa: SLF001 - Biotite extension hook
            ...
            header = ResidueTemplateParser._get_header(parsed_info)
            valid_chains = ResidueTemplateParser._get_protein_chains(parsed_info)
            if not valid_chains:
            ...
            for atom in ResidueTemplateParser._get_atom_site_list(parsed_info):
                if atom.model_num != "1":
                    continue
                ...
                if atom.mmcif_chain_id in valid_chains:
                    ...
                    ins_code = (
                        atom.insertion_code
                        if ResidueTemplateParser._is_set(atom.insertion_code)
                        else " "
                    )
                    pos = ResiduePosition(
                    ...
                    seq_to_structure_mappings[atom.author_chain_id][seq_idx] = (
                        ResidueAtPosition(
                            position=pos,
...
class TemplateHit:
    ...
    def query_to_hit_mapping(self) -> Mapping[int, int]:
        """Map 0-based query indices to 0-based hit indices."""
        mapping = {}
        mapping.update(
            {
                q_idx: h_idx
                for (q_idx, h_idx) in zip(
                    self.indices_query, self.indices_hit, strict=False
                )
...
class HHRParser:
    ...
    def _parse_hit(lines: Sequence[str]) -> TemplateHit:
        ...
        if not match:
            msg = f"Could not parse HHR summary: {summary}"
    ...
    @staticmethod
    def _update_residue_indices(seq: str, start: int, indices: list[int]) -> None:
...
class _CommonHmmsearchA3MParser:
    """Represent common hmmsearch a3 m parser."""
    ...
    def _parse_description(desc: str) -> HitMetadata:
        ...
        if not match:
            msg = f"Could not parse hmmsearch description: {desc}"
            raise ValueError(msg)
        return HitMetadata(
...
class ResidueHmmsearchA3MParser(_CommonHmmsearchA3MParser):
    ...
    def parse(
        ...
        parsed = list(zip(seqs, descs, strict=False))
        if skip_first:
            parsed = parsed[1:]

        idx_q = ResidueHmmsearchA3MParser._get_indices(query_seq, 0)
        hits = []
        for i, (h_seq, h_desc) in enumerate(parsed, start=1):
            if "mol:protein" not in h_desc:
                continue
            meta = ResidueHmmsearchA3MParser._parse_description(h_desc)
            cols = sum(1 for r in h_seq if r.isupper() and r != "-")
            idx_h = ResidueHmmsearchA3MParser._get_indices(h_seq, meta.start - 1)
            hits.append(
                TemplateHit(
                    i,
...
class StructuralTemplateParser(_CommonTemplateParser):
    """Class to parse mmCIF files into MmcifObject."""
    ...
    def _mmcif_loop_to_dict(
        ...
        """Extract an mmCIF loop into a dictionary keyed by a specific column."""
        entries = StructuralTemplateParser._mmcif_loop_to_list(prefix, parsed_info)
        return {entry[index_key]: entry for entry in entries}
    ...
    def _get_header(parsed_info: MmCIFDict) -> PdbHeader:
        """Extract basic header information (method, release date, resolution)."""
        header = {}
        experiments = StructuralTemplateParser._mmcif_loop_to_list(
            "_exptl.", parsed_info
        )
        header["structure_method"] = ",".join(
            [exp.get("_exptl.method", "").lower() for exp in experiments]
        ...
        release_date = StructuralTemplateParser._get_release_date(parsed_info)
        if release_date:
            header["release_date"] = release_date
        else:
            logger.warning(
    ...
    def _get_protein_chains(parsed_info: MmCIFDict) -> dict[ChainId, Sequence[Monomer]]:
        """Extract valid protein chains and their sequences."""
        entity_poly_seqs = StructuralTemplateParser._mmcif_loop_to_list(
            "_entity_poly_seq.", parsed_info
        )
        polymers = collections.defaultdict(list)
        for entry in entity_poly_seqs:
        ...
        chem_comps = StructuralTemplateParser._mmcif_loop_to_dict(
            "_chem_comp.", "_chem_comp.id", parsed_info
        )
        struct_asyms = StructuralTemplateParser._mmcif_loop_to_list(
            "_struct_asym.", parsed_info
        )
        ...
        *,
        ...
        try:
            ...
            if structure is None:
                message = "mmCIF parser returned no structure"
                raise ValueError(message)  # noqa: TRY301 - normalize this record failure through the existing handler
            first_model = StructuralTemplateParser._get_first_model(structure)
            parsed_info = parser._mmcif_dict  # pylint: disable=protected-access  # noqa: SLF001 - Biotite extension hook
            ...
            header = StructuralTemplateParser._get_header(parsed_info)
            valid_chains = StructuralTemplateParser._get_protein_chains(parsed_info)
            if not valid_chains:
                return ParsingResult(None, {(file_id, ""): "No protein chains found."})
            ...
            for atom in StructuralTemplateParser._get_atom_site_list(parsed_info):
                if atom.model_num != "1":
                    continue
                if auth_chain_id is not None and atom.author_chain_id != auth_chain_id:
                    continue
                ...
                if atom.mmcif_chain_id in valid_chains:
                    ...
                    ins_code = (
                        atom.insertion_code
                        if StructuralTemplateParser._is_set(atom.insertion_code)
                        else " "
                    )
                    pos = ResiduePosition(
                        chain_id=atom.author_chain_id,
...
class StructuralHmmsearchA3MParser(_CommonHmmsearchA3MParser):
    """Class to parse A3M files from hmmsearch."""
    ...
    def parse(
        ...
        idx_q = StructuralHmmsearchA3MParser._get_indices(query_seq, 0)
        hits = []
        for i, (h_seq, h_desc) in enumerate(parsed, start=1):
            if "mol:protein" not in h_desc:
                continue
            meta = StructuralHmmsearchA3MParser._parse_description(h_desc)
            cols = sum(1 for r in h_seq if r.isupper() and r != "-")
            idx_h = StructuralHmmsearchA3MParser._get_indices(h_seq, meta.start - 1)
            hits.append(
                TemplateHit(
                    i,
                    f"{meta.pdb_id}_{meta.chain}",
