...
@contextmanager
def _lm_dropout_context(model: Any, lm_dropout: float | None) -> Iterator[None]:
    """Compute lm dropout context.
    ...
    ``config.lm_dropout`` — disambiguated by ``config.type``. ``None``/``0`` is a
    no-op.
    """
    if not lm_dropout:
        yield
...
def clean_esmfold2_input(input: StructurePredictionInput) -> StructurePredictionInput:  # noqa: A002, C901, PLR0912, PLR0915 - external interface keyword
    """Compute clean esmfold2 input.
...
class ESMFold2InputBuilder:
    """Represent e s m fold2 input builder."""

    def __init__(self, ccd_db: CCDDatabase) -> None:
        self.ccd_db = ccd_db
    ...
    def __call__(
        ...
        """Prepare sequence features and chain metadata for inference."""
    ...
    def decode(
        ...
        *,
        ...
        num_diffusion_samples : int
            Number of diffusion samples present in the output (Bm = B *
        ...
        MolecularComplexResult or list[MolecularComplexResult]
            A single result when num_diffusion_samples == 1, otherwise a list of length
            Bm.
        ...
        plddts = output["plddt"]
        if sample_coords is None or plddts is None:
            message = "Decoding requires sample coordinates and per-atom pLDDT"
            raise ValueError(message)
        batch_size = sample_coords.shape[0]
