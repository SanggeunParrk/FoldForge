    ...
    for k, param in translation_dict.items():
        with torch.no_grad():
            ...
            try:
                ...
                if len(ref) != len(weights):
                    raise ValueError(f"{k}: stacked parameter count mismatch")
                for p, w in zip(ref, weights, strict=True):
                    if p.shape != w.shape:
                        raise ValueError(f"{k}: {p.shape} != {w.shape}")
                    p.copy_(w)
            except:
                print(k)
                print(ref[0].shape)
...
def import_jax_weights_(model, model_path: pathlib.Path):
    ...
    if missing or extra:
        raise ValueError(f"AF3 checkpoint mapping mismatch: missing={missing}, extra={extra}")
    assign(flat, params)
    model.__identifier__ = params["__meta__/__identifier__"]
    from foldforge.models.af3.fourier import _WEIGHT, _BIAS
