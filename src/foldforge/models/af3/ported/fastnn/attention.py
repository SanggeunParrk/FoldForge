...
from foldforge.models.af3.ported.fastnn import config as fastnn_config
...
def dot_product_attention_torch(
    ...
    mask: Optional[torch.Tensor] = None, bias: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    ...
    if mask is not None:
        if mask.ndim == 1:
            ...
            mask = mask[:, None, None, :]
    return attention(q, k, v, bias=bias, mask=mask, scale=q.shape[-1] ** -0.5,
