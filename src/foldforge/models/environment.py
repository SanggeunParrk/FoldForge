...
from foldforge.models.config.opendde.config.schema import (
...
FP32_FALLBACK_SM_MAJOR = 7
MAX_ERROR_MESSAGE_LENGTH = 160
MIN_CUEQUIVARIANCE_SM_MAJOR = 8
...
def probe_optional_module(module_name: str) -> OptionalModuleStatus:
    ...
    except Exception as exc:  # noqa: BLE001 - probe returns the backend failure as status
...
def _probe_mps(torch_module: ModuleType) -> tuple[bool, bool]:
    ...
    except Exception:  # noqa: BLE001 - probe returns the backend failure as status
        return False, False
    return built, available


def get_torch_runtime_info() -> TorchRuntimeInfo:
    """Return torch runtime info."""
...
def select_torch_device(  # noqa: C901 - independent hardware capability diagnostics
    ...
    if requested_device not in INFERENCE_DEVICE_CHOICES:
        ...
        msg = f"Invalid device {requested_device!r}. Choose from: {choices}."
    ...
    try:
        ...
        msg = (
            f"PyTorch is required for inference but could "
            f"not be imported: {_format_exception(exc)}"
        )
...
def get_cuequivariance_runtime_status(  # noqa: PLR0911 - independent hardware capability diagnostics
    device: torch.device,
    *,
    optional_modules: tuple[OptionalModuleStatus, ...] | None = None,
    ...
    if major < MIN_CUEQUIVARIANCE_SM_MAJOR:
        if major < FP32_FALLBACK_SM_MAJOR:
            return CuEquivarianceRuntimeStatus(
...
def nvidia_smi_summary() -> tuple[bool, str | None]:
    """Compute nvidia smi summary."""
    nvidia_smi = shutil.which("nvidia-smi")
    if nvidia_smi is None:
        return False, None
    try:
        result = subprocess.run(  # noqa: S603 - argument vector; no shell expansion
            [
                nvidia_smi,
...
def _runtime_recommendation(  # noqa: PLR0911 - independent hardware capability diagnostics
    torch_info: TorchRuntimeInfo,
    selected_device: torch.device | None,
    cueq_status: CuEquivarianceRuntimeStatus | None,
) -> str:
    if not torch_info.installed:
...
def format_doctor_report() -> str:
    ...
    cueq_status: CuEquivarianceRuntimeStatus | None = None
    ...
    lines = [
        ...
        "- nvidia-smi: "
        f"{nvidia_smi_info or ('found' if nvidia_smi_found else 'not found')}",
