...
from foldforge.models.opendde.ported.data.constants import rdkit_vdws
...
class Clash(nn.Module):
    ...
    def _check_clash_per_chain_pairs(
        ...
        asym_id_to_asym_mask,
        mol_id: Optional[torch.Tensor] = None,
        asym_id_to_mol_id: Optional[dict[int, int]] = None,
    ):
        ...
        if self.compute_af3_clash:
            ...
            af3_clash_details = torch.zeros(
                N_sample, N_chains, N_chains, 2, device=device, dtype=torch.float32
            )
        if self.compute_vdw_clash:
            ...
            vdw_clash_details = {}
            assert asym_id_to_mol_id is not None
