...
from foldforge.data.ccd.esm_view import (
...
def _compute_res_type(name: str, mol_type: int) -> int:  # noqa: PLR0911 - chemical residue classification
...
def tokenize_ligand_smiles(
    ...
    for atom, can_idx in zip(mol.GetAtoms(), canonical_order, strict=False):
        atom_name = atom.GetSymbol().upper() + str(can_idx + 1)
        if len(atom_name) > 4:
            msg = f"SMILES {smiles} has atom name longer than 4 chars: {atom_name}"
            raise ValueError(msg)
    ...
    if mol_no_h.GetNumConformers() == 0:
        msg = f"Failed to generate conformer for SMILES: {smiles}"
        raise ValueError(msg)
...
def _get_sequence_key(item: ProteinInput | RNAInput | DNAInput | LigandInput) -> str:
    ...
    msg = f"Unknown input type: {type(item)}"
    raise ValueError(msg)


def build_chains_from_input(
    input: StructurePredictionInput,  # noqa: A002 - external interface keyword
    seed: int | None = None,
    ...
    for item in input.sequences:
        ...
        for chain_id_str in ids:
            ...
            if isinstance(item, ProteinInput):
                ...
                if item.ccd is not None:
                    if item.smiles is not None:
                        warnings.warn(
                            "Both ccd and smiles provided, using ccd", stacklevel=2
                        )
                    ...
                    msg = "LigandInput must have either ccd or smiles"
                    raise ValueError(msg)
            else:
                msg = f"Unknown input type: {type(item)}"
                raise TypeError(msg)
            ...
            space_uid_offset += len({a.space_uid for a in new_atoms})
            asym_id += 1
...
def compute_frame_indices(
    ...
    for t in tokens:
        ad = token_atoms.get(t.token_index, {})
        fallback = next(iter(ad.values())) if ad else 0
...
def compute_token_bonds(
    ...
    # Build per-residue atom name -> token_index mapping for ligands and modified
    # residues
...
def compute_representative_atoms(
    ...
    for t in tokens:
        ad = token_atoms.get(t.token_index, {})
        fallback_idx = next(iter(ad.values())) if ad else 0
...
def compute_msa_features(
    ...
    msa_res, del_counts, _paired = construct_paired_msa(
        chain_msas,
...
def compute_distogram_conditioning(
    ...
    chains: list[ChainInfo],
    tokens: list[TokenInfo],
    disto_center: torch.Tensor,  # noqa: ARG001 - shared callback or fixture signature
    min_dist: float = 2.0,
    max_dist: float = 22.0,
    num_bins: int = 64,
    ...
    for dc in input.distogram_conditioning:
        ...
        if distogram.shape != (n_chain, n_chain):
            msg = (
                f"Distogram shape {distogram.shape} doesn't "
                f"match chain length {n_chain}"
            )
            raise ValueError(msg)
