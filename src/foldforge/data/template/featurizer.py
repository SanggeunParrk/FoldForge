...
from typing import Self as ResidueSelf
from typing import Self as StructuralSelf
...
from foldforge.data.features.atoms import pad_to
...
from foldforge.data.template.features import (
    ...
    ResidueTemplateFeatures,
    ResidueTemplateHitFeaturizer,
    StructuralTemplateFeatures,
    StructuralTemplateHitFeaturizer,
)
from foldforge.data.template.features import (
    TEMPLATE_FEATURES as RESIDUE_TEMPLATE_FEATURES,
)
from foldforge.data.template.features import (
    TEMPLATE_FEATURES as STRUCTURAL_TEMPLATE_FEATURES,
)
from foldforge.data.template.parser import (
    HHRParser,
    ResidueHmmsearchA3MParser,
    StructuralHmmsearchA3MParser,
)
...
type ResidueBatchDict = dict[str, np.ndarray]


type ResidueFeatureDict = Mapping[str, np.ndarray]
...
class TemplateSourceManager:
    ...
    def fetch_template_paths(
        ...
        """Fetch template file paths from the configured sources.
        ...
        template_paths = []
        for i, (path, method) in enumerate(
            zip(self.raw_paths, self.indexing_methods, strict=False)
        ):
            ...
            for subpath in possible_subpaths:
                full_path = str(Path(dir_path).joinpath(subpath))
                if Path(full_path).exists():
                    template_paths.append(full_path)
        return template_paths


class _CommonTemplateFeatureAssemblyLine:
    """Represent common template feature assembly line."""

    def __init__(self, max_templates: int = 4) -> None:
        self.max_templates = max_templates


class ResidueTemplateFeatureAssemblyLine(_CommonTemplateFeatureAssemblyLine):
    """Represent residue template feature assembly line.
    ...
    def assemble(
        ...
        for info in bioassembly.values():
            ...
            # Templates are currently only supported for protein chains with sufficient
            # length
            ...
            if (entity_id not in polymer_entity_features[skip_chain]) or (
                ...
                if skip_chain or not templates:
                    template_features = ResidueTemplateFeatures.empty_template_features(
                        num_tokens
                    )
                else:
                    # Package and fix template features
                    template_features = (
                        ResidueTemplateFeatures.package_template_features(
                            hit_features=templates
                        )
                    )
                    template_features = ResidueTemplateFeatures.fix_template_features(
                        template_features=template_features,
                        num_res=num_tokens,
                    )
                # Reduce to requested maximum number of templates
                template_features = _residue_reduce_template_features(
                ...
                template_features = polymer_entity_features[skip_chain][entity_id]
        ...
        # Pad the number of templates to max_templates for each chain to allow
        # concatenation
        ...
        merged_example = {
            ...
            if ft in RESIDUE_TEMPLATE_FEATURES
        }
...
class TemplateFeaturizer:
    ...
    Args:
        ...
        prot_seq_or_filename_to_templatedir_jsons: JSON files mapping sequences/PDB IDs
    ...
    def __init__(
        ...
        prot_template_cache_dir: str | None = None,
        ...
        **_kwargs: Any,
        ...
        # Validate inputs
        if len(prot_template_raw_paths) != len(
            prot_seq_or_filename_to_templatedir_jsons
        ):
            message = "Mismatch between raw paths and JSON mapping files."
            raise ValueError(message)
        if not (len(kalign_binary_path) > 0):
            message = "kalign_binary_path must be provided."
            raise ValueError(message)
        if not (len(release_dates_path) > 0):
            message = "release_dates_path must be provided."
            raise ValueError(message)
        if not (len(obsolete_pdbs_path) > 0):
            message = "obsolete_pdbs_path must be provided."
            raise ValueError(message)
        if len(prot_template_raw_paths) != len(prot_indexing_methods):
            message = "Mismatch between raw paths and indexing methods."
            raise ValueError(message)

        # Currently, multiple source training strategy is not determined
        if len(prot_template_raw_paths) != 1:
            message = "Only single source template supported."
            raise ValueError(message)

        if stage != "train":
            max_template_date = "2021-09-30"
        elif ("distillation" in dataset_name) or ("openprotein" in dataset_name):
            max_template_date = "2018-04-30"
        else:
            max_template_date = "3000-01-01"

        self.stage = stage
        self.max_template_date = datetime.strptime(max_template_date, "%Y-%m-%d")  # noqa: DTZ007 - PDB calendar dates, not timestamps
        ...
        self.online_featurizer = ResidueTemplateHitFeaturizer(
    ...
    def get_template(
        ...
        """Fetch and processes templates for a single sequence.
        ...
        if hmmsearched_hhr:
            ...
            hits = ResidueHmmsearchA3MParser.parse(
        ...
        return [dict(feature) for feature in result.features], raw_hit_count, time_track

    def make_template_features(
        self,
        bioassembly_dict: dict[str, Any],
        selected_token_indices: np.ndarray | None,
        entity_to_asym_id_int: Mapping[str, Iterable[int]],
    ) -> dict[str, np.ndarray]:
        """Generate template features for a bioassembly.
        ...
        for aid, eid in asym_to_entity_id.items():
            ...
            if seq is None:
                ...
                if not (atom_array.is_ligand[ligand_mask].all()):
                    message = "Unknown molecule type"
                    raise ValueError(message)
        ...
        if self.stage == "train" and "release_date" in bioassembly_dict:
            query_release_date = datetime.strptime(  # noqa: DTZ007 - PDB calendar dates, not timestamps
        ...
        for aid, sequence in asym_id_to_sequence.items():
            ...
            chain_type = poly_types_mapping.get(
                entity_poly_type.get(eid, "non-polymer"), "non-polymer"
            )

            if drop_template or chain_type != PROTEIN_CHAIN:
                templates, raw_hit_count, time_track = [], 0, {}
            elif sequence in seq_to_templates_cache:
                templates, raw_hit_count, time_track = seq_to_templates_cache[sequence]
            else:
        ...
        if not get_safe_entity_id_for_template_copy(template_meta_infos):
            logger.warning(
                "PDB %s has inconsistent sequences within same entity.", f"{pdb_id}"
            )

        # Coordinate mapping to standardized indices
        ca = atom_array[centre_atom_indices]
        std_idxs = residue_map_to_standard(
            ca.asym_id_int, ca.res_id, template_meta_infos
        )

        # Assemble final features
        max_t = 4  # Default to 4
        assembly_line = ResidueTemplateFeatureAssemblyLine(max_templates=max_t)
    ...
    def get_last_profile(self) -> dict[str, float | int]:
        """Return profiling information for the last call."""
        return self._last_profile


@dataclasses.dataclass(frozen=True)
class ResidueTemplates:
    """Dataclass containing template features."""

    # aatype: [num_templates, num_res]  # noqa: ERA001 - format example
    aatype: np.ndarray
    atom_positions: np.ndarray
    atom_mask: np.ndarray

    @classmethod
    def from_data_dict(cls, batch: ResidueBatchDict) -> ResidueSelf:
    ...
    def as_data_dict(self) -> ResidueBatchDict:
    ...
    def as_feature_dict(self) -> ResidueBatchDict:
        ...
        config = ResidueTemplates._DGRAM_CONFIG
        ...
        for i in range(num_templates):
            ...
            pb_pos, pb_mask = ResidueTemplateFeatures.pseudo_beta_fn(
                aatype, pos, mask, is_ligand=is_lig
            )
            pb_mask_2d = pb_mask[:, None] * pb_mask[None, :]

            dgram = ResidueTemplateFeatures.dgram_from_positions(pb_pos, config=config)
            all_dgrams[i] = dgram * pb_mask_2d[..., None]
            all_pb_masks[i] = pb_mask_2d

            uv, bb_mask_2d = ResidueTemplateFeatures.compute_template_unit_vector(
                aatype, pos, mask
            )
...
def _residue_reduce_template_features(
    template_features: ResidueFeatureDict, max_templates: int
) -> ResidueFeatureDict:
    ...
    fields = (*RESIDUE_TEMPLATE_FEATURES, "template_release_timestamp")
...
def get_safe_entity_id_for_template_copy(
    ...
    """Identify entity IDs that have consistent sequences across all chains."""
    eid_to_seqs = {}
    for info in bioassembly.values():
        eid = info["entity_id"]
        seq = info["sequence"]
...
class ResidueInferenceTemplateFeaturizer:
    ...
    def make_template_feature(
        ...
        *,
        ...
        online_template_featurizer: ResidueTemplateHitFeaturizer | None = None,
    ) -> dict[str, np.ndarray]:
        """Generate template features during inference.
        ...
        for eid, info in enumerate(bioassembly):
            seq, count, ctype, t_path = "", 0, "non-polymer", ""
            ...
            if "proteinChain" in info:
                ...
                count, ctype = info["ligand"]["count"], "non-polymer"
                seq = "X" * (atom_array.asym_id_int == curr_asym_id).sum()

            templates = []
            if (
                use_template
                and "proteinChain" in info
                and "miniworldTemplates" in info["proteinChain"]
            ):
                from foldforge.data.inputs.lmdb import template_features

                if t_path:
                    msg = "Choose templatesPath or miniworldTemplates"
                    raise ValueError(msg)
                templates = template_features(
                    info["proteinChain"]["miniworldTemplates"], seq, "protenix"
                )
            if t_path and use_template and online_template_featurizer:
                if ctype != PROTEIN_CHAIN:
                    message = "Only protein templates are supported."
                    raise ValueError(message)
                if t_path.endswith(".json"):
                    ...
                    if t_path.endswith(".hhr"):
                        ...
                        msg = f"Unsupported template format: {t_path}"
                        raise ValueError(msg)
                ...
                logger.info(
                    "Found %s templates for sequence %s", f"{len(templates)}", f"{seq}"
                )
        ...
        # Assemble features
        return (
            ResidueTemplateFeatureAssemblyLine(max_templates=4)
            .assemble(template_meta_infos, std_idxs)
            .as_feature_dict()
        )


type StructuralBatchDict = dict[str, np.ndarray]


type StructuralFeatureDict = Mapping[str, np.ndarray]


type MutableFeatureDict = dict[str, Any]


class StructuralTemplateFeatureAssemblyLine(_CommonTemplateFeatureAssemblyLine):
    ...
    def assemble(
        ...
        polymer_entity_features: dict[bool, dict[Any, StructuralFeatureDict]] = {
            True: {},
            False: {},
        }
        # Identify entities where template features can be safely copied (same sequence)
        ...
        for info in bioassembly.values():
            ...
            if (entity_id not in polymer_entity_features[skip_chain]) or (
                ...
                if skip_chain or not templates:
                    template_features = (
                        StructuralTemplateFeatures.empty_template_features(num_tokens)
                    )
                else:
                    # Package and fix template features
                    template_features = (
                        StructuralTemplateFeatures.package_template_features(
                            hit_features=templates
                        )
                    )
                    template_features = (
                        StructuralTemplateFeatures.fix_template_features(
                            template_features=template_features,
                            num_res=num_tokens,
                        )
                    )
                # Reduce to requested maximum number of templates
                template_features = _structural_reduce_template_features(
                    template_features, self.max_templates
                )
                if entity_id in safe_entity_ids:
                    polymer_entity_features[skip_chain][entity_id] = template_features
        ...
        merged_example = {
            ft: np.concatenate([c[ft] for c in np_chains_list], axis=1)
            for ft in np_chains_list[0]
            if ft in STRUCTURAL_TEMPLATE_FEATURES
        }
...
class StructuralTemplates:
    ...
    @classmethod
    def from_data_dict(cls, batch: StructuralBatchDict) -> StructuralSelf:
        """Construct instance from a data dictionary."""
        return cls(
            aatype=batch["template_aatype"],
            atom_positions=batch["template_atom_positions"],
    ...
    def as_data_dict(self) -> StructuralBatchDict:
        """Convert to a standard data dictionary."""
        return {
            "template_aatype": self.aatype,
            "template_atom_positions": self.atom_positions,
    ...
    def as_feature_dict(self) -> StructuralBatchDict:
        ...
        config = StructuralTemplates._DGRAM_CONFIG
        for i in range(num_templates):
            aatype = self.aatype[i]
            mask = self.atom_mask[i]
            pos = self.atom_positions[i] * mask[..., None]

            pb_pos, pb_mask = StructuralTemplateFeatures.pseudo_beta_fn(
                aatype, pos, mask
            )
            pb_mask_2d = pb_mask[:, None] * pb_mask[None, :]

            dgram = StructuralTemplateFeatures.dgram_from_positions(
                pb_pos, config=config
            )
            all_dgrams[i] = dgram * pb_mask_2d[..., None]
            all_pb_masks[i] = pb_mask_2d

            uv, bb_mask_2d = StructuralTemplateFeatures.compute_template_unit_vector(
                aatype, pos, mask
            )
            all_unit_vectors[i] = uv * bb_mask_2d[..., None]
            all_bb_masks[i] = bb_mask_2d
...
def _structural_reduce_template_features(
    template_features: StructuralFeatureDict, max_templates: int
) -> StructuralFeatureDict:
    """Reduces templates to the requested maximum number."""
    num_t = template_features["template_aatype"].shape[0]
    keep_mask = np.arange(num_t) < max_templates
    fields = STRUCTURAL_TEMPLATE_FEATURES
    return {k: v[keep_mask] for k, v in template_features.items() if k in fields}


class StructuralInferenceTemplateFeaturizer:
    """Simplified featurizer for inference, leveraging the same assembly logic."""
    ...
    def make_template_feature(
        ...
        *,
        use_template: bool = True,
        online_template_featurizer: StructuralTemplateHitFeaturizer | None = None,
    ) -> dict[str, np.ndarray]:
        """Generate template features during inference.
        ...
        for eid, info in enumerate(bioassembly):
            ...
            if "proteinChain" in info:
                ...
                count, ctype = info["ion"]["count"], "non-polymer"
            ...
            if (
                ...
                if t_path:
                    ...
                    raise ValueError(msg)
                templates = template_features(
                    info["proteinChain"]["miniworldTemplates"], seq, "opendde"
                )
            if t_path and use_template and online_template_featurizer:
                if ctype != PROTEIN_CHAIN:
                    message = "Only protein templates are supported."
                ...
                if t_path.endswith(".hhr"):
                    ...
                    hits = StructuralHmmsearchA3MParser.parse(
                        query_seq=seq, a3m_str=content, skip_first=False
                    )
                else:
                    msg = f"Unsupported template format: {t_path}"
        ...
        # Coordinate mapping
        ca = atom_array[atom_array.centre_atom_mask.astype(bool)]
        std_idxs = structural_map_to_standard(
            ca.asym_id_int, ca.res_id, template_meta_infos
        )

        # Assemble features
        return (
            StructuralTemplateFeatureAssemblyLine(max_templates=4)
            .assemble(template_meta_infos, std_idxs)
            .as_feature_dict()
        )
