...
from foldforge.eval.permutation.checks import save_permutation_error
from foldforge.eval.permutation.matching import correct_symmetric_chains
from foldforge.eval.permutation.pocket import (
...
def run(
    ...
    **kwargs: Any,
    ...
    dict[str, torch.Tensor], dict[str, float], list[torch.Tensor], list[torch.Tensor]
]:
    ...
    Args:
        ...
        input_feature_dict (dict[str, Union[torch.Tensor, int, float, dict]]): A
            dictionary containing input features.
        label_full_dict (dict[str, Union[torch.Tensor, int, float, dict]]): A
            dictionary containing full label information.
        max_num_chains (int, optional): The maximum number of chains to consider.
            Defaults to -1 (no limit).
        permute_label (bool, optional): Whether to permute the label. Defaults to True.
        permute_by_pocket (bool, optional): Whether to permute by pocket (for
            PoseBusters dataset). Defaults to False.
        error_dir (str, optional): Directory to save error data. Defaults to None.
        **kwargs: Additional keyword arguments.

    Returns:
        tuple[dict]: A tuple containing the output dictionary, log dictionary, permuted
        prediction indices, and permuted label indices.
    """
    if pred_coord.dim() > 2 and permute_label is not False:
    ...
    try:
        if permute_by_pocket:
            ...
            if permute_label:
                message = "Invalid state: not permute_label"
        ...
        save_permutation_error(
            data={
                ...
                "max_num_chains": max_num_chains,
                "permute_label": permute_label,
                "dataset_name": input_feature_dict.get("dataset_name"),
                "pdb_id": input_feature_dict.get("pdb_id"),
            },
            error_dir=error_dir,
        )
        output_dict, log_dict, permute_pred_indices, permute_label_indices = (
