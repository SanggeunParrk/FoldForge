...
_SKIP_RANDOM_INIT = ContextVar("foldforge_protenix_skip_random_init", default=False)

@contextmanager
def skip_random_init():
    token = _SKIP_RANDOM_INIT.set(True)
    try:
        yield
    finally:
...
fastln_is_installed = False  # FoldForge configures its shared engine normalizers.
if fastln_is_installed:
    from foldforge.models.protenix.ported.model.layer_norm.layer_norm import FusedLayerNorm
...
def _attention(
    ...
    from team_gm.modules.blocks.attention_math import attention_with_biases
    return attention_with_biases(query, key, value, biases=biases, normalize=softmax_no_cast)
...
def _tri_attention(
    ...
    Args:
        ...
        biases:
            List of biases that broadcast to [*, H, Q, K]
    """
    from foldforge.models.protenix.ported.model.tri_attention.op import TriAttentionFunction
...
class OuterProductMean(nn.Module):
    ...
    def _opm(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        from team_gm.modules.blocks.attention_math import outer_product_projection
        return outer_product_projection(a, b, project=self.linear_out)
