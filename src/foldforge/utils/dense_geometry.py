...
def unstack(value: torch.Tensor, dim: int = -1) -> list[torch.Tensor]:
    """Compute unstack."""
    return [
        torch.squeeze(v, dim=dim) for v in torch.chunk(value, value.shape[dim], dim=dim)
    ]
...
class Vec3Array:
    ...
    def norm(self, epsilon: float = 1e-6) -> torch.Tensor:
        ...
        if epsilon:
            norm2 = torch.maximum(
                norm2, torch.tensor(epsilon**2, dtype=norm2.dtype, device=norm2.device)
            )
...
class Rigid3Array:
    ...
    def inverse(self) -> Self:
        ...
        return type(self)(inv_rotation, inv_translation)
