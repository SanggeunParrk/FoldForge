...
from foldforge.eval.permutation import atoms as atom_permutation
from foldforge.eval.permutation import chains as chain_permutation
...
class SymmetricPermutation:
    ...
    def __init__(self, configs, error_dir: str | None = None) -> None:
        self.configs = configs
        if error_dir is not None:
            self.chain_error_dir = str(Path(error_dir) / ("chain_permutation"))
            self.atom_error_dir = str(Path(error_dir) / ("atom_permutation"))
        else:
    ...
    def permute_label_to_match_mini_rollout(
        ...
        This is mainly used to align label structure to the mini-rollout structure
        during training.

        Args:
            mini_coord (torch.Tensor): Coordinates of the predicted mini-rollout
                structure.
        ...
        """
        if mini_coord.dim() != 3:
            message = "Invalid state: mini_coord.dim() == 3"
            raise ValueError(message)

        log_dict = {}
        # 1. ChainPermutation: permute ground-truth chains to match mini-rollout
        # prediction
        ...
        # 2. AtomPermutation: permute ground-truth atoms to match mini-rollout
        # prediction
    ...
    def permute_diffusion_sample_to_match_label(
        ...
        """Compute permute diffusion sample to match label.
        ...
        Args:
            ...
            permute_by_pocket (bool): Whether to permute by pocket (for PoseBusters
                dataset). Defaults to False.
        """
        if pred_dict["coordinate"].size(-2) != label_dict["coordinate"].size(-2):
            message = (
                "Cannot perform per-sample permutation on predicted structures if"
                " the label structure has more atoms."
            )
            raise ValueError(message)
        ...
        if (
            stage != "train"
            # During training stage, the label_dict is cropped after mini-rollout
            # permutation.
        ):
        ...
        if permute_pred_indices and atom_perm_pred_indices is not None:
            ...
            if len(permute_pred_indices) != len(atom_perm_pred_indices):
                message = (
                    "Invalid state: len(permute_pred_indices) == "
                    "len(atom_perm_pred_indices)"
                )
                raise ValueError(message)
            for chain_perm_indices, atom_perm_indices in zip(
                permute_pred_indices, atom_perm_pred_indices, strict=False
            ):
    ...
    def get_chain_mask_from_atom_mask(
        ...
        This method maps atoms to their corresponding token indices and then to their
        asym IDs. It then filters these asym IDs based on the atom mask and returns a
        mask indicating which atoms belong to the filtered chains.

        Args:
            atom_mask (torch.Tensor): A boolean atom mask. Shape: [N_atom].
            atom_to_token_idx (torch.Tensor): A tensor mapping each atom to its
                corresponding token index. Shape: [N_atom].
            token_asym_id (torch.Tensor): A tensor containing the asym ID for each
                token. Shape: [N_token].
        ...
        if atom_asym_id.size(0) != atom_mask.size(0):
            message = "Invalid state: atom_asym_id.size(0) == atom_mask.size(0)"
            raise ValueError(message)
    ...
    def get_asym_id_match(
        ...
        Args:
            permute_indices (torch.Tensor): indices that specify the permuted ordering
                of atoms.
                [N_atom]
            atom_to_token_idx (torch.Tensor):  each entry maps an atom to its
        ...
        Returns:
            asym_id_match (Dict[int])
                A dictionary where the key is the original asym ID and the value is the
                permuted asym ID.
        """
        ...
        asym_id_match = {}
        for raw_ori_aid in unique_asym_ids:
            ori_aid = raw_ori_aid
            ori_aid = ori_aid.item()
            asym_mask = original_atom_asym_id == ori_aid
            perm_aid = permuted_atom_asym_id[asym_mask]

            if len(torch.unique(perm_aid)) != 1:
                message = "Permuted asym ID must be unique for each original ID."
                raise ValueError(message)
    ...
    def permute_summary_confidence(
        ...
        chain_pair_keys: list[str] | None = None,
    ):
        """Permute summary confidence based on predicted indices.

        Args:
            summary_confidence_list (list[dict]): List of summary confidence
                dictionaries.
            permute_pred_indices (list[torch.Tensor]): List of predicted indices for
                permutation.
            ...
            chain_keys (list[str], optional): Keys for chain-level confidence metrics.
                Defaults to ["chain_ptm", "chain_iptm", "chain_plddt"].
            chain_pair_keys (list[str], optional): Keys for chain pair-level confidence
                metrics. Defaults to ["chain_pair_iptm", "chain_pair_iptm_global",
                "chain_pair_plddt"].
        """
        ...
        if chain_keys is None:
            chain_keys = ["chain_ptm", "chain_iptm", "chain_plddt", "chain_gpde"]
        if len(summary_confidence_list) != len(permute_pred_indices):
            message = (
                "Invalid state: len(summary_confidence_list) == "
                "len(permute_pred_indices)"
            )
            raise ValueError(message)
        ...
        for _i, (raw_summary_confidence, perm_indices) in enumerate(
            zip(summary_confidence_list, permute_pred_indices, strict=False)
        ):
            summary_confidence = raw_summary_confidence
    ...
    def permute_heads(
        ...
        Args:
            ...
            permute_pred_indices (list): A list of tensors, each containing the
                predicted indices for the permutation of a diffusion sample.
            atom_to_token_idx (torch.Tensor): A tensor mapping each atom to its
                corresponding token index. Shape: [N_atom].
            rep_atom_mask (torch.Tensor): A boolean mask indicating which atoms are
                representative. Shape: [N_atom].
        ...
        for i, perm_indices in enumerate(permute_pred_indices):
            ...
            for key in ["plddt", "resolved"]:
                if key in pred_dict:
                    if pred_dict[key].size(-2) != len(perm_indices):
                        message = (
                            "Invalid state: pred_dict[key].size(-2) == "
                            "len(perm_indices)"
                        )
                        raise ValueError(message)
            ...
            for key in ["pae", "pde"]:
                if key in pred_dict:
                    if not (
                        ...
                        message = (
                            "Invalid state: pred_dict[key].size(-2) == "
                            "pred_dict[key].size(-3) == len(perm_token_indices)"
                        )
                        raise ValueError(message)
                    pred_dict[key] = pred_dict[key].to(perm_token_indices.device)
                    if pred_dict[key].device != perm_token_indices.device:
                        message = (
                            "Invalid state: pred_dict[key].device == "
            ...
            if "contact_probs" in pred_dict:
                ...
                if not (
                    ...
                    message = (
                        "Invalid state: contact_probs_i.size(-1) == "
                        "contact_probs_i.size(-2) == len(perm_token_indices)"
                    )
                    raise ValueError(message)
    ...
    def permute_inference_pred_dict(
        ...
        Args:
            ...
            label_dict (dict): Label dictionary.
            permute_by_pocket (bool, optional): Whether to permute by pocket. Defaults
                to False.
        """
