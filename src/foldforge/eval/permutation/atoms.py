...
import torch
from team_gm.modules.checkpoints.tensor_utils import expand_at_dim, pad_at_dim

from foldforge.eval.permutation.checks import Checker, save_permutation_error
from foldforge.eval.rmsd import rmsd, self_aligned_rmsd
...
def run(
    ...
    Returns:
        if permute_label = True,
            output_dict: a dictionary in the following form, recording the permuted
                label.
                {
    ...
    try:
        ...
        _try_result = ({"coordinate": permuted_coord}, info_dict, indices_permutation)

    except Exception as e:
        error_message = str(e)
        if dataset_name:
            logger.exception("dataset: %s", f"{dataset_name}")
        if pdb_id:
            logger.exception("pdb id: %s", f"{pdb_id}")
...
def collect_residues_with_symmetric_atoms(
    ...
    Args:
        coord (torch.Tensor): Coordinates of atoms.
            [N_atom, 3]
        coord_mask (torch.Tensor): The mask indicating whether the atom is resolved in
            GT.
            [N_atom]
        ref_space_uid (torch.Tensor): Each (chain id, residue index) tuple has a unique
            ID.
            [N_atom]
        atom_perm_list (list[list]): The atom permutation list, where each sublist
            contains
                                   the permutation information of the corresponding
                                   residue.
    ...
    res_start_end = list(
        zip(start_positions[:-1].tolist(), start_positions[1:].tolist(), strict=False)
    )  # [N_res, 2]
    N_res = len(res_start_end)
    if N_res != len(torch.unique(ref_space_uid)):
        message = "Invalid state: N_res == len(torch.unique(ref_space_uid))"
    ...
    for start, end in res_start_end:
        if len(torch.unique(ref_space_uid[start:end])) != 1:
            message = "Invalid state: len(torch.unique(ref_space_uid[start:end])) == 1"
            raise ValueError(message)
        ...
        N_res_atom, _N_perm = perm.size()

        # Basic checks
        if perm.min().item() != 0:
            message = "Invalid state: perm.min().item() == 0"
            raise ValueError(message)
        if perm.max().item() != N_res_atom - 1:
            message = "Invalid state: perm.max().item() == N_res_atom - 1"
            raise ValueError(message)
...
def collect_permuted_coords(
    ...
    Args:
        ...
        perm_list (list[torch.Tensor]): list of permutations.
            Each element is a long tensor of shape [N_perm, N_res_atom]. The value
            N_perm
    ...
    N_total_perm = 0
    for perm, res_coord, res_coord_mask in zip(
        perm_list, coord_list, coord_mask_list, strict=False
    ):
        # Basic shape checks
        N_perm, N_res_atom = perm.size()
        if res_coord.size(-1) != 3:
            message = "Invalid state: res_coord.size(-1) == 3"
            raise ValueError(message)
        if not (res_coord.size(0) == res_coord_mask.size(0) == perm.size(-1)):
            message = (
                "Invalid state: res_coord.size(0) == res_coord_mask.size(0) == "
                "perm.size(-1)"
            )
            raise ValueError(message)
        ...
        if res_coord_permuted.size() != (N_perm, N_res_atom, 3):
            message = (
                "Invalid state: res_coord_permuted.size() == (N_perm, N_res_atom, 3)"
            )
            raise ValueError(message)
        if res_coord_mask_permuted.size() != (N_perm, N_res_atom):
            message = (
                "Invalid state: res_coord_mask_permuted.size() == (N_perm, N_res_atom)"
            )
            raise ValueError(message)
    ...
    # Shape check
    if perm_coord.size() != (N_total_perm, MAX_N_res_atom, 3):
        message = (
            "Invalid state: perm_coord.size() == (N_total_perm, MAX_N_res_atom, 3)"
        )
        raise ValueError(message)
    if perm_coord_mask.size() != (N_total_perm, MAX_N_res_atom):
        message = (
            "Invalid state: perm_coord_mask.size() == (N_total_perm, MAX_N_res_atom)"
        )
        raise ValueError(message)
...
class AtomPermutation:
    """Represent atom permutation.

    Class for assigning the optimal permutations of true coordinates/pred coordinates
    and coordinate masks.

    Args:
        eps (float): A small number used in alignment. Defaults to 1e-8.
        run_checker (bool): If true, it applies more checkers to ensure the
            correctness. Defaults to False.
        global_align_wo_symmetric_atom (bool):  If true, the global alignment before
            AtomPermutation will not consider atoms with permutation. Defaults to
            False.
    """
    ...
    def check_input_shape(
        ...
        if true_coord.dim() != 2:
            message = "Invalid state: true_coord.dim() == 2"
            raise ValueError(message)
        if true_coord_mask.dim() != 1:
            message = "Invalid state: true_coord_mask.dim() == 1"
            raise ValueError(message)
        if ref_space_uid.dim() != 1:
            message = "Invalid state: ref_space_uid.dim() == 1"
            raise ValueError(message)

        if true_coord.size(-1) != 3:
            message = "Invalid state: true_coord.size(-1) == 3"
            raise ValueError(message)
        if true_coord.size(-2) != N_atom:
            message = "Invalid state: true_coord.size(-2) == N_atom"
            raise ValueError(message)
        if ref_space_uid.size(-1) != N_atom:
            message = "Invalid state: ref_space_uid.size(-1) == N_atom"
            raise ValueError(message)
        if len(atom_perm_list) != N_atom:
            message = "Invalid state: len(atom_perm_list) == N_atom"
            raise ValueError(message)

        if pred_coord.dim() not in [2, 3]:
            message = "Invalid state: pred_coord.dim() in [2, 3]"
            raise ValueError(message)  # for simplicity
        if pred_coord.shape[-2:] != (N_atom, 3):
            message = "Invalid state: pred_coord.shape[-2:] == (N_atom, 3)"
            raise ValueError(message)
    ...
    def global_align_pred_to_true(
        ...
        if true_coord.dim() < pred_coord.dim():
            if pred_coord.dim() != 3:
                message = "Invalid state: pred_coord.dim() == 3"
                raise ValueError(message)  # [Batch, N_atom, 3]
    ...
    @staticmethod
    def get_identity_permutation(batch_shape, N_atom, device) -> torch.Tensor:
        """Return identity permutation.

        Return identity permutation indices if no multiple-permutation exists for every
        residue.
        ...
        if len(batch_shape) != 1:
            message = "Invalid state: len(batch_shape) == 1"
            raise ValueError(message)
        return torch.stack([identity for _ in range(batch_shape[0])], dim=0)

    @staticmethod
    def _optimize_per_residue_permutation_by_rmsd(  # noqa: PLR0915 - reference atom assignment algorithm
        ...
        """Compute optimize per residue permutation by rmsd.

        Find the optimal permutations of true coordinates and coordinate masks to
        minimize the.
        RMSD between true coordinates and predicted coordinates.

        Args:
            per_residue_pred_coord_list (torch.Tensor): List of residues. Each element
                records
                the predicted atom coordinates of one residue. Each element has shape
                [N_res_atom, 3] or [Batch, N_res_atom, 3]
            per_residue_coord_list (list[torch.Tensor]): List of residues. Each element
                records
                the atom coordinates of one residue. Each element has shape
                [N_res_atom, 3].
            per_residue_coord_mask_list (list[torch.Tensor]): List of residues. Each
                element records
                the atom coordinate masks of one residue. Each element has shape
                [N_res_atom].
            per_residue_perm_list (list[torch.Tensor]): List of residues. Each element
                records
                the atom permutations of one residue. Each element has shape [N_perm,
                N_res_atom].
        ...
        Returns:
            best_permutation_list (list[torch.Tensor]): List of residues. Each element
                records the
                optimal permutation that should apply to true coordinates for one
                residue. Each element
                has shape
                [N_res_atom] or [Batch, N_res_atom]
            is_permuted_list (list[torch.Tensor]): List of residues. Each element
                records whether the
                atoms in this residue is permuted. Each element has shape
                [] or [Batch]
            optimized_rmsd_list (list[torch.Tensor]): List of residues. Each element
                records the optimized
                rmsd of the residue. Each element has shape
                [] or [Batch]
            original_rmsd_list (list[torch.Tensor]): List of residues. Each element
        ...
        # Permute true coordinates & masks according to the permutations in
        ...
        if not (permuted_coord.size(-2) == permuted_coord_mask.size(-1) == N_max_atom):
            message = (
                "Invalid state: permuted_coord.size(-2) == "
                "permuted_coord_mask.size(-1) == N_max_atom"
            )
            raise ValueError(message)
            ...
            functools.reduce(
                ...
                [
                    ...
                    for N_perm, p_coord in zip(
                        per_residue_N_perm, per_residue_pred_coord_list, strict=False
                    )
                ],
                [],
            ),
        ...
        if pred_coord.shape[-3:] != (N_total_perm, N_max_atom, 3):
            message = (
                "Invalid state: pred_coord.shape[-3:] == (N_total_perm, N_max_atom, 3)"
            )
            raise ValueError(message)

        batch_shape = pred_coord.shape[:-3]
        if len(batch_shape) not in [0, 1]:
            message = "Invalid state: len(batch_shape) in [0, 1]"
            raise ValueError(message)
        if len(batch_shape) == 1:
        ...
        with torch.autocast("cuda", enabled=False):
            ...
            if per_res_rmsd.size() != (*batch_shape, N_total_perm):
                message = (
                    "Invalid state: per_res_rmsd.size() == batch_shape + "
                    "(N_total_perm,)"
                )
                raise ValueError(message)
        ...
        for N_perm, N_res_atom, perm in zip(
            per_residue_N_perm, per_residue_N_atom, per_residue_perm_list, strict=False
        ):
            ...
            if run_checker:
                if perm.size() != (N_perm, N_res_atom):
                    message = "Invalid state: perm.size() == (N_perm, N_res_atom)"
                    raise ValueError(message)
                if cur_res_rmsd.size() != (*batch_shape, N_perm):
                    message = (
                        "Invalid state: cur_res_rmsd.size() == batch_shape + (N_perm,)"
                    )
                    raise ValueError(message)
                if best_rmsd.size() != batch_shape:
                    message = "Invalid state: best_rmsd.size() == batch_shape"
                    raise ValueError(message)
                if best_j.size() != batch_shape:
                    message = "Invalid state: best_j.size() == batch_shape"
                    raise ValueError(message)
                if best_perm.size() != (*batch_shape, N_res_atom):
                    message = (
                        "Invalid state: best_perm.size() == batch_shape + (N_res_atom,)"
                    )
                    raise ValueError(message)
                Checker.are_permutations(best_perm, dim=-1)

                def _check_identity(j_value, perm) -> None:
                    if j_value > 0:
                ...
                if best_j.dim() == 0:
                    ...
                    for j_value, perm_j in zip(best_j, best_perm, strict=False):
    ...
    def __call__(  # noqa: PLR0915 - reference atom assignment algorithm
        self,
        pred_coord: torch.Tensor,
        true_coord: torch.Tensor,
        true_coord_mask: torch.Tensor,
        ...
        """Args:
            ...
            true_coord (torch.Tensor): true coordinates of atoms.
                [N_atom, 3]
            true_coord_mask (torch.Tensor): The mask indicating whether the atom is
                resolved.
                [N_atom]
            ref_space_uid (torch.Tensor): Each (chain id, residue index) tuple has a
                unique ID.
                [N_atom]
            atom_perm_list (list[list]): The atom permutation list, where each sublist
                contains
                                          the permutation information of the
                                          corresponding residue.
                len(atom_perm_list) = N_atom.
                len(atom_perm_list[i]) = N_perm for the residue of atom i.
            permute_label (bool, optional): If true, return indices permutations of the
                true coordinate.
                Otherwise, return indices permutations for the predicted coordinate.
                Defaults to True.
            alignment_mask (torch.Tensor, optional): Defaults to None. A mask
                indicating which atoms to
                consider while performing the alignment.
            verbose (bool, optional): Defaults to False.
            run_checker (bool, optional): Whether running more checks for debug.
                Defaults to False.
        ...
        if not per_residue_perm_list:
            logger.info(
                "%s", "No atom permutation is needed. Return the identity permutation."
            )
        ...
        for (start, end), per_residue_perm in zip(
            per_residue_position_list, per_residue_perm_list, strict=False
        ):
        ...
        for i, n in enumerate(per_residue_N_atom):
            atom_cutoff = next(
                (cutoff for cutoff in res_atom_cutoff if n <= cutoff),
                res_atom_cutoff[-1],
            )
            grouped_indices.setdefault(atom_cutoff, []).append(i)

        if len(
            functools.reduce(operator.iadd, list(grouped_indices.values()), [])
        ) != len(per_residue_perm_list):
            message = (
                "Invalid state: len(sum(list(grouped_indices.values()), [])) == "
                "len(per_residue_perm_list)"
            )
            raise ValueError(message)
        ...
        for atom_cutoff, residue_group in grouped_indices.items():
            if verbose:
                logger.info(
                    "%s", f"{len(residue_group)} residues have <={atom_cutoff} atoms."
                )
        ...
        residue_best_permutation = torch.cat(
            [
                ind[perm]
                for ind, perm in zip(
                    indices_list, residue_best_permutation_list, strict=False
                )
            ],
            dim=-1,
...
def correct_symmetric_atoms(
    ...
    """Compute correct symmetric atoms.

    Return optimally permuted true coordinates and masks according to the predicted
    coordinates.
    ...
    Args:
        ...
        atom_perm_list (list[list]): The atom permutation list, where each sublist
            ...
            len(atom_perm_list) = N_atom.
            len(atom_perm_list[i]) = N_perm for the residue of atom i.
        permute_label (bool): indicates whether permuted true coordinates are returned
            or
            predicted coordinates are returned.
        alignment_mask (torch.Tensor, optional): a mask indicating which atoms are
            considered while
            performing the alignment.
            [N_atom]
        eps (float, optional): A small number used in alignment. Defaults to 1e-8.
        global_align_wo_symmetric_atom (bool):  If true, the global alignment before
            AtomPermutation will not consider atoms has permutation.
    ...
    Returns:
        ...
        log_dict: logging info for the permutation
            percent_res_permuted (torch.Tensor): percentage of residues (excluding
                those with less than 3 atoms or identity perm only) that have been
                permuted
            best_aligned_rmsd_improved: rmsd improved after permutation, using
                self_aligned_rmsd
    """
    if pred_coord.dim() not in [2, 3]:
        message = "Invalid state: pred_coord.dim() in [2, 3]"
        raise ValueError(message)
    if pred_coord.size(-1) != 3:
        message = "Invalid state: pred_coord.size(-1) == 3"
        raise ValueError(message)
