...
class PredictionLoader:
    """Represent prediction loader."""

    def __init__(self, pred_fpath: str) -> None:
        self.pred_fpath = pred_fpath
        self._load_cif()
        self._load_confidence_pred()

    def _load_json(self, fpath: str) -> dict[str, Any]:

        with Path(fpath).open() as file:
            return json.load(file)

    def _convert_to_numpy(self, data: dict) -> dict:
    ...
    def _load_cif(self) -> None:
        if not (Path(self.pred_fpath).exists()):
            message = "prediction file path does not exist."
            raise ValueError(message)
        fpath_all_preds = [str(path) for path in Path(self.pred_fpath).glob("*.cif")]
    ...
    def _load_confidence_pred(self) -> None:
        fpath_all_confidences = [
            str(path) for path in Path(self.pred_fpath).glob("*.json")
        ]
...
def plot_contact_maps_from_pred(
    ...
    Args:
        ...
        rep_atom (str): type of representative atoms, commonly options are 'CA' and
            'CB'
...
def plot_confidence_measures_from_pred(
    ...
    fnames: list,
    show_global_confidence: bool,  # noqa: FBT001 - notebook plotting API
    *_args: Any,
    **_kwargs: Any,
) -> None:
    """Plot contact maps from a directory.
    ...
    Args:
        ...
        *args: Additional positional options accepted by the plotting interface.
        show_global_confidence: Whether to include global confidence scores.
...
def plot_3d(
    pred_id: int = 0,
    pred_loader: PredictionLoader | None = None,
    *,
    ...
    if pred_loader is None:
        message = "A prediction loader is required for 3D visualization"
        raise ValueError(message)
    fpath = pred_loader.cif_paths[pred_id]
    view.addModelsAsFrames(Path(fpath).read_text(), "cif")
    if color == "pLDDT":
        if not (pred_loader is not None):
            message = "Invalid state: pred_loader is not None"
            raise ValueError(message)
        if pred_loader.full_confidence_data is None:
            message = "pLDDT coloring requires full confidence data"
            raise ValueError(message)
    ...
    if show_sidechains:
        backbone_style = ["C", "O", "N"]
        view.addStyle(
            {
                "and": [
                    {"resn": ["GLY", "PRO"], "invert": True},
                    {"atom": backbone_style, "invert": True},
                ]
            },
...
def plot_best_confidence_measure(
    prediction_fpath: str, output_dir: str | None = None
) -> None:
    """Plot confidence_measure from a directory.

    Args:
        output_dir: Directory for the generated plots.
        prediction_fpath: Directory containing prediction structures and confidence
        files.
        full_confidence (list): list of full confidence metrics from prediction
        summary_confidence (list): list of summary confidence metrics from prediction
        fnames (list): list of prediction names
    ...
    if output_dir is None or output_dir == "":
        output_dir = prediction_fpath
    pred_loader = PredictionLoader(prediction_fpath)
    if (
        pred_loader.full_confidence_data is None
        or len(pred_loader.full_confidence_data) < 1
    ):
        logger.info("%s", "no full confidence data found, skip ploting.")
        return
    ...
    for data_idx in range(data_len):
        ...
        out_png_fpath = str(
            Path(output_dir) / (f"best_sample_confidence_{data_idx}.png")
        )
        plt.savefig(out_png_fpath)
    logger.info(
        "%s",
        f"save {data_len} best sample confidence plot successfully to {output_dir}",
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The prediction files to parse",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        required=False,
        default=None,
        help="ouput dir to save plot",
    )
    args = parser.parse_args()
    plot_best_confidence_measure(args.prediction_fpath, args.output_dir)
