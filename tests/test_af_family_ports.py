...
from foldforge.modules.af_family import (
    configure_model,
    transition_residual,
    triangle_residual,
)
...
def test_triangle_mask_and_weight_layout(family, outgoing):
    ...
    if family == "af3":
        mod = importlib.import_module(
            "foldforge.models.af3.ported.nn.triangle_multiplication"
        )
        layer = mod.TriangleMultiplication(c_pair=128, _outgoing=outgoing)
    else:
        mod = importlib.import_module(
            f"foldforge.models.{family}.ported.model.triangular.triangular"
        )
        cls = (
...
def test_transition_residual_once(family):
    ...
    cls = importlib.import_module(f"foldforge.models.{family}.ported.{path}").Transition
...
def test_af3_diffusion_preserves_random_noise_field():
    from foldforge.models.af3.ported.alphafold3 import AlphaFold3
    from foldforge.models.af3.ported.nn import diffusion_head
    ...
    model.gamma_0 = 0.8
    model.gamma_min = 1.0
    model.noise_scale = 1.003
    model.step_scale = 1.5
    model.diffusion_head = IdentityDenoiser()
    pos = torch.zeros(4, 24, 3)
    with mock.patch.object(
        diffusion_head,
        "random_augmentation",
        side_effect=lambda positions, mask: positions,  # noqa: ARG005
    ):
        result, _ = model._apply_denoising_step(
            None, {}, pos, torch.tensor(4.0), torch.ones(4, 24), torch.tensor(3.0)
        )
    assert result.std() > 0
...
def test_attention_native_bf16_without_autocast(family):
    mod = importlib.import_module(
