...
from foldforge.data.features.atoms import (
    residue_get_atom_mask_by_name as get_atom_mask_by_name,
)
...
class SubstructureRatios(TypedDict):
    """Sampling probabilities for complete chains and individual tokens."""
...
class ConstraintFeatureGenerator:
    """Generate constraint features by coordinating different featurizers.
    ...
    def __init__(
        ...
        ab_top2_clusters: set[str] | None = None,
    ) -> None:
        self.constraint = constraint_config
        self.ab_top2_clusters = ab_top2_clusters or set()
    ...
    def _canonicalize_contact_format(
        ...
        for id_num in ["1", "2"]:
            ...
            for key in ["entity", "copy", "position", "atom"]:
                ...
                if identifier_value is None and key != "atom":
                    message = (
                        "contact should have at least 3 identifiers('entity', 'copy', "
                        "'position')"
                    )
                    raise ValueError(message)
                if (key == "atom" and identifier_value is not None) and (
                    isinstance(identifier_value, int)
                ):
                    entity_dict = next(
                        iter(sequences[int(pair[f"entity{id_num}"]) - 1].values())
                    )
                    if "atom_map_to_atom_name" not in entity_dict:
                        message = (
                            "Invalid state: 'atom_map_to_atom_name' in entity_dict"
                        )
                        raise ValueError(message)
        ...
        if hash(tuple(_pair["id1"][:2])) == hash(tuple(_pair["id2"][:2])):
            msg = "A contact pair can not be specified on the same chain"
        ...
        if _pair["max_distance"] < _pair["min_distance"]:
            msg = "max_distance must be greater than min_distance"
            raise ValueError(msg)
    ...
    def _canonicalize_pocket_res_format(binder: dict, pocket_pos: dict) -> dict:
        if len(pocket_pos) != 3:
            message = "Invalid state: len(pocket_pos) == 3"
            raise ValueError(message)
        if hash((binder["entity"], binder["copy"])) == hash(
            (pocket_pos["entity"], pocket_pos["copy"])
        ):
            msg = "Pockets can not be the same chain with the binder"
            raise ValueError(msg)
        return pocket_pos
    ...
    def _log_constraint_feature(
        ...
        if atom_feature.sum() > 0:
            ...
            logger.info("loaded atom contact info:%s", f"{contact_info}")
        ...
        if token_feature.sum() > 0:
            ...
            logger.info("loaded contact info:%s", f"{contact_info}")
        ...
        if pocket_feature.sum() > 0:
            ...
            logger.info("loaded pocket info:%s", f"{pocket_info}")

    @staticmethod
    def generate_from_json(  # noqa: PLR0915 - ordered residue and atom constraint assembly
        token_array: TokenArray,
        atom_array: AtomArray,
        sequences: list[dict[str, Any]],
        constraint_param: dict,
    ) -> tuple[dict[str, Any], TokenArray, AtomArray]:
        """Compute generate from json."""
        feature_dict = {}
        ...
        for i, pair in enumerate(atom_contact_inputs):
            ...
            if np.size(atom_list_1) != 1 or np.size(atom_list_2) != 1:
                logger.info("Atom contact %s not found for the input", f"{i}")
                continue
        ...
        (
            feature_dict["contact_atom"],
            _,
        ) = atom_contact_featurizer.generate_atom_spec_constraint(
            atom_contact_specifics,
        ...
        for i, pair in enumerate(token_contact_inputs):
            ...
            if np.size(token_list_1) == 0 or np.size(token_list_2) == 0:
                logger.info("Contact %s not found for the input", f"{i}")
                continue
        ...
        if pocket := constraint_param.get("pocket", {}):
            ...
            if len(binder) != 2:
                message = "Invalid state: len(binder) == 2"
                raise ValueError(message)
            ...
            if num_binder == 0:
                logger.info("Binder does not exist: %s", binder)
            elif num_binder > 1:
                logger.info("Binder matches %s chains: %s", num_binder, binder)
            else:
                binder_token_list = atom_to_token_idx[atom_mask_binder]

                for j, raw_pocket_res in enumerate(pocket["contact_residues"]):
                    pocket_res = raw_pocket_res
                    pocket_res = (
                    ...
                    if np.size(pocket_token_list) == 0:
                        logger.info(
                            "Pocket residue %s not found for binder %s", j, binder
                        )
                        continue
                ...
                logger.info("#pocket:%s", f"{len(pocket_specifics)}")
        ...
        if constraint_param.get("structure"):
            message = "Explicit substructure constraints are not implemented"
        ...
        logger.info(
            "Loaded constraint feature: #atom contact:%s #contact:%s #pocket:%s",
            f"{len(atom_contact_specifics)}",
            f"{len(token_contact_specifics)}",
            f"{len(pocket_specifics)}",
        )
    ...
    def generate(
        ...
        (
            ...
            tokens_w_atom_contact,
        ) = contact_atom_featurizer.generate_atom_spec_constraint(
            contact_pairs,
    ...
    @staticmethod
    def expand_tokens[A: AtomArray | None](  # noqa: PLR0915 - ordered residue and atom constraint assembly
        token_array: TokenArray,
        atom_array: AtomArray,
            ...
            if old_token_idx in tokens_to_expand:
                ...
                if (
                    ...
                    for atom_idx, atom_name in zip(
                        token.atom_indices, token.atom_names, strict=False
                    ):
        ...
        if len(expanded_atoms) > 0:
            logger.info("Expanded atoms: %s", f"{expanded_atoms}")
            ...
            full_atom_array.centre_atom_mask = np.array(
                [
                    ...
                    for atom, mask in zip(
                        full_atom_array, full_atom_array.centre_atom_mask, strict=False
                    )
                ]
            )
    ...
    def _get_chain_interface_mask(
        self,
        pdb_indice: pd.DataFrame,
        atom_array_chain_id: np.ndarray,
    ...
    def _setup_constraint_generator(self, sample_indice: pd.Series) -> Any:
        """Set up random generator with optional fixed seed."""
        ...
        logger.info(
            "[constraint_seed seed]: %s for pdb %s",
            f"{constraint_seed}",
            f"{sample_indice.pdb_id}",
        )
    ...
    def _generate_pocket_features(
        ...
        if use_random_pocket:
            ...
            if self.constraint["pocket"].get("spec_binder_chain", False):
                ...
                if not (num_unique <= 1):
                    message = "Invalid state: num_unique <= 1"
                    raise ValueError(message)
                logger.info("found binder chains: %s", f"{num_unique}")
    ...
    def _generate_contact_features(
        ...
        pdb_indice: pd.DataFrame,
    ) -> tuple[torch.Tensor, Any, list[tuple[int, int, float, float]], set[int]]:
        ...
        if use_random_contact:
            ...
            if self.constraint["contact"].get("group", "complex") == "interface":
                ...
                interface_asym_pairs = [
                    ...
                    for c1, c2 in zip(chain_1_mask, chain_2_mask, strict=False)
            ...
            (
                ...
                {}, "complex", "continuous", size=0, distance_type="center_atom"
            )
        return (
            contact_constraint_feature,
            contact_featurizer,
    ...
    def _generate_contact_atom_features(
        ...
        if use_random_contact_atom:
            ...
            (
                ...
                sample_group=self.constraint["contact_atom"].get("group", "complex"),
        ...
        if use_random_substructure:
            ...
            if self.constraint["substructure"].get("spec_asym_id", False):
                ...
                # find antibody chain
                    ...
                    logger.info("No specific chain found!")
                ...
                if chain_choice == "binder":
                    ...
                    msg = f"Invalid spec_chain_type: {self.constraint['substructure'].get('spec_chain_type', 'binder')}"
                    raise ValueError(msg)
                ...
                if spec_chain_id != -1:
                    ...
                    if not (num_unique <= 1):
                        ...
                        raise ValueError(message)
                    logger.info(
                        "found %s chain: %s", f"{chain_choice}", f"{num_unique}"
                    )
    ...
    def _log_feature_statistics(self, feature_info: dict[str, Any]) -> dict[str, Any]:
        log_constraint = {}
        if feature_info.get("pocket_info") is not None:
        ...
        if feature_info.get("contact_info") is not None:
            asym_id, token_id, _res_id, distance = feature_info["contact_info"].values()
        ...
        if feature_info.get("contact_atom_info") is not None:
        ...
        if feature_info.get("substructure_info") is not None:
...
class ConstraintFeaturizer:
    """Represent constraint featurizer."""
    ...
    def encode(self, feature: torch.Tensor, feature_type: str, **kwargs: Any):
        """Encode ."""
        ...
        if feature_type == "continuous":
            return feature
        msg = f"Invalid feature_type: {feature_type}"
        raise RuntimeError(msg)

    def _get_base_info(self) -> None:
    ...
    def _get_generation_basics(self, distance_type: str = "center_atom") -> None:
        token_centre_atom_indices = self.token_array.get_annotation("centre_atom_index")
        centre_atoms = self.atom_array[token_centre_atom_indices]
        ...
        if distance_type == "center_atom":
            ...
            for i, j in np.ndindex(self.token_distance.shape):
                ...
                self.token_distance[i, j] = float(
                    np.min(all_atom_distance[atom_pairs_mask])
                )
        elif distance_type == "atom":
            msg = "Not implement in this class, please use ContactAtomFeaturizer"
            raise ValueError(msg)
        else:
            msg = f"Not recognized distance_type: {distance_type}"
            raise ValueError(msg)


class ContactFeaturizer(ConstraintFeaturizer):
    """Represent contact featurizer."""
    ...
    def get_valid_contact_feature(
        ...
        """Return valid contact feature.

        Find all valid pairs of entities that satisfy the given contact distance
        requirements.

        Parameters
        ----------
         - valid_contact_type: A two-charactor type to represent entity pairs under
         consideration.
            e.g. PP, PL, P_
         - max_distance_threshold: The maximum allowable distance for a contact to be
         considered valid.
         - min_distance_threshold: The minimum allowable distance for a contact to be
         considered valid.

        Returns
        -------
         - shape=(N_token, N_token), A matrix generate contact within the specified
         distance range.
        ...
        if key_type == "_":
            # intra chain contact
            if query_type != "P":
                message = "only support intra-protein contact for now"
                raise ValueError(message)
            ...
            # inter chain contact
            if not (query_type in "PL" and key_type in "PL"):
                message = f"[Error]contact type not support: {valid_contact_type}"
                raise ValueError(message)
        ...
        if min_distance_threshold == -1:  # default false, hard to determine
            # random select a min threshold in [0, max_distance_threshold), but may lead
            # to zero contact pairs
        ...
        # make feature
        return valid_chain_mask & self.token_resolved_maskmat & valid_dist_mask
    ...
    def _get_constraint_size(self, group: str, size: Any) -> list[int]:
        ...
        if group == "complex":
            ...
            message = f"Unsupported contact sampling group: {group}"
            raise ValueError(message)
        if size < 1 and size > 0:
            return torch.zeros(k).geometric_(size).int().tolist()
    ...
    def _sample_contacts(
        ...
        Args:
            - contact_valid_mask: shape=(N_token, N_token), all valid contact pairs,
            bool
        ...
        def _sample(valid_mask, cur_size) -> None:
            ...
            selected_indices = tuple(zip(*selected_indices, strict=False))
            if len(selected_indices) == 0:
                return

            # Convert to pairs format
            for i, j in zip(*selected_indices, strict=False):
        ...
        # if group by interface, get all unique interfaces, and sample contact from each
        # interface
        elif sample_group == "interface":
            # sample contacts from each interface iteratively

            for idx, (asym1, asym2) in enumerate(chain_pairs):
    ...
    def generate(
        ...
        chain_pairs: list[tuple[int, int]] | None = None,
        **_kwargs: Any,
    ...
    def generate_spec_constraint(
        self,
        contact_specifics: list[tuple[np.ndarray, np.ndarray, float, float]],
        feature_type: str,
...
class PocketFeaturizer(ConstraintFeaturizer):
    ...
    def get_valid_pocket_feature(
        ...
        """Parameters:
            ...
            - asym list
        Returns:
            - binder_pocket_valid_masks, shape=(N_asym, N_token), A matrix of all
            pocket tokens within the specified distance range.
            - max_distance_value, shape=(N_asym, N_token), A matrix of the
            max_distance_threshold to determine the pocket token for each binder chain
        """
        ...
        # get distance mask
        # Note: To simplify the implementation, we only consider token-token distance,
        # instead of any heavy atom distance for each residue for now.
        dist_mask = (
    ...
    def _sample_pocket(
        ...
        **_: object,  # do not use
        ...
        binder_dist_mat = max_distance_value[selected_binder][selected_pocket_res_mask][
            None, :
        ].expand(int((self.asymid == binder_asym_id).sum().item()), -1)
    ...
    def generate(
        ...
        size: Any,
        distance_type: Any,
        spec_binder_asym_id: int | None = None,
        **_: object,  # do not use
    ) -> tuple[torch.Tensor, set[int]]:
        """Trainint & evaluation."""
        constrained_tokens = set()
        ...
        for i, j, _feature_axis in nonzero_indices:
    ...
    def _get_constraint_size(self, size: float) -> int:
        """If size is not fixed, then we generate it randomly for each group."""
        if size < 1 and size > 0:
            return torch.zeros(1).geometric_(size).int().tolist()[0]
        if size >= 1 and isinstance(size, int):
            return size
        raise NotImplementedError
...
class ContactAtomFeaturizer(ContactFeaturizer):
    """Represent contact atom featurizer."""
    ...
    def _get_generation_basics(self, distance_type: str = "atom") -> None:  # noqa: ARG002 - shared callback or fixture signature
        # is_resolved mask
        self.token_resolved_mask = torch.tensor(
    ...
    def generate_atom_spec_constraint(
        self,
    ...
    def get_real_distance(self, atom_idx_1: int, atom_idx_2: int) -> float:
        ...
        return float(self.token_distance[atom_idx_1, atom_idx_2].item())


class SubStructureFeaturizer(ConstraintFeaturizer):
    """Represent sub structure featurizer."""
    ...
    def _get_distance_feature(
        ...
        Returns
        -------
            - distance_feature: For one_hot: [..., N_token, N_token] tensor with bin
            indices
                              For continuous: [..., N_token, N_token] tensor with
                              distance values
    ...
    def get_valid_substructure_feature(
        ...
        Parameters
        ----------
            - mol_type_pairs: Dict of type pairs (e.g. {'PP': threshold, 'LP':
            threshold})
                First type in pair is the one to be selected as substructure, threshold
                is the distance threshold to determine the interface
        Returns:
    ...
    def _get_constraint_size(self, size: Any) -> int:
        ...
        if size < 1 and size > 0:
            ...
            return int(
                torch.rand(1).geometric_(size, generator=self.generator).int().item()
            )
        if size >= 1 and isinstance(size, (int, float)):
    ...
    def _sample_substructure(
        ...
        full_ratio_idx = int(
            torch.randint(len(ratios["full"]), (1,), generator=self.generator).item()
        )
    ...
    def generate_spec_constraint(self, substructure_specifics: dict, feature_type):
        """Parse constraint from user specification.
