...
import torch
from team_gm.modules.checkpoints.tensor_utils import expand_at_dim

from foldforge.eval.permutation.alignment import apply_transform, get_optimal_transform
from foldforge.eval.rmsd import rmsd
...
def permute_pred_to_optimize_pocket_aligned_rmsd(  # noqa: PLR0915 - reference pocket assignment algorithm
    ...
    permuted_aligned_pred_coord (torch.Tensor): permuted and aligned coordinates of
        pred_coord.
        [N_sample, N_atom, 3]
    """
    log_dict = {}
    ...
    if pred_coord.size(-2) != true_coord.size(-2):
        message = "Atom numbers are difference."
    ...
    def _get_entity_and_asym_id(atom_mask):
        ...
        if not ((masked_asym_id[0] == masked_asym_id).all()):
            message = "Invalid state: (masked_asym_id[0] == masked_asym_id).all()"
            raise ValueError(message)
        if not ((masked_entity_id[0] == masked_entity_id).all()):
            message = "Invalid state: (masked_entity_id[0] == masked_entity_id).all()"
            raise ValueError(message)
    ...
    candidate_pockets = {}
    for raw_i in torch.unique(atom_asym_id[atom_entity_id == pocket_entity_id]):
        i = raw_i
        i = i.item()
        ...
        if pocket_mask.sum() != true_pocket_mask.sum():
            message = "Invalid state: pocket_mask.sum() == true_pocket_mask.sum()"
            raise ValueError(message)
        candidate_pockets[i] = pocket_mask.clone()
    candidate_ligands = {}
    for raw_j in torch.unique(atom_asym_id[atom_entity_id == ligand_entity_id]):
        j = raw_j
        j = j.item()
        ...
        if lig_mask_j.sum() != true_ligand_mask.sum():
            logger.warning(
                (
                    "The ligand selected by 'mol_id' has %s atoms.The true ligand "
                    "selected by 'asym_id' has %s atoms."
                ),
                f"{lig_mask_j.sum().item()}",
                f"{true_ligand_mask.sum().item()}",
            )
            lig_mask_j = lig_mask_j * torch.isin(
                mol_atom_index, mol_atom_index[true_ligand_mask]
            )
        if lig_mask_j.sum() != true_ligand_mask.sum():
            message = "Invalid state: lig_mask_j.sum() == true_ligand_mask.sum()"
            raise ValueError(message)
