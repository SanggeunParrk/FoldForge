...
from foldforge.data.constants import atom_types, residue_names, side_chains
...
NUM_RESTYPES_WITH_UNK_AND_GAP = residue_names.POLYMER_TYPES_NUM_WITH_UNKNOWN_AND_GAP
...
def _make_restype_pseudobeta_idx():
    """Return indices of residue's pseudo-beta."""
...
def _make_aatype_dense_atom_to_atom37():
    ...
    restype_dense_atom_to_atom37.extend(
        [0] * NUM_DENSE
        for _ in range(2 + len(residue_names.NUCLEIC_TYPES_WITH_UNKNOWN))
    )

    return torch.tensor(restype_dense_atom_to_atom37, dtype=torch.int32)
