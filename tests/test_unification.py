...
from foldforge.models.af_pairformer import AF3Pairformer, ProtenixPairformer
...
def test_protenix_and_opendde_use_identical_ccd_functions():
    from foldforge.models.opendde.ported.data.core import ccd as o
    from foldforge.models.protenix.ported.data.core import ccd as p
    ...
    for name in [
        ...
        assert getattr(o, name) is getattr(p, name)
...
def test_shared_pairformer_matches_released_order(family):
    ...
    cls = importlib.import_module(
        f"foldforge.models.{family}.ported.{path}"
    ).PairformerBlock
    source = cls()
    with torch.no_grad():
