...
from foldforge.eval.permutation.alignment import (
    apply_transform,
    get_optimal_transform,
    num_unique_matches,
)
from foldforge.eval.permutation.checks import Checker
    ...
    **kwargs: Any,
    ...
    Args:
        ...
        label_full_dict (dict[str, torch.Tensor]): A dictionary containing
            - coordinate: label_full_dict["coordinate"] and
            label_full_dict["coordinate_mask"]
                shape = [N_atom, 3] and [N_atom] (for coordinate_mask)
            - other keys: entity_mol_id, mol_id, mol_atom_index, pae_rep_atom_mask.
                shape = [N_atom]
            - extra keys: keys specified by extra_feature_keys.
        extra_label_keys (list[str]):
            - Additional features in label_full_dict that should be returned along with
            the permuted coordinates.
        max_num_chains (int): if the number of chains is more than this number, than
            skip permutation to
            avoid expensive computations.
        permute_label (bool): if true, permute the groundtruth chains, otherwise
            premute the prediction chains
    ...
    Return:
        ...
        permute_pred_indices / permute_label_indices:
            If batch_mode, this is a list of LongTensor. Otherwise, this is a
            LongTensor.
            The LongTensor gives the indices to permute either prediction or label.
    """
    if pred_dict["coordinate"].dim() not in [2, 3]:
        message = "Invalid state: pred_dict['coordinate'].dim() in [2, 3]"
    ...
    if permute_label:
        message = "Only supports prediction permutations in batch mode."
        raise ValueError(message)
    pred_coord = []
    log_dict = {}
    best_matches = []
    ...
    for _i, pred_coord_i in enumerate(pred_dict["coordinate"]):
        (
            best_match_i,
...
def _correct_symmetric_chains_for_one_sample(
    ...
    extra_label_keys: list[str] | None = None,
    **kwargs: Any,
):
    """Compute correct symmetric chains for one sample.

    Correct symmetric chains for a single sample by permuting either the predicted or
    ...
    if not permute_label:
        ...
        if pred_dict["coordinate"].size(-2) != label_full_dict["coordinate"].size(-2):
            message = (
                "Invalid state: pred_dict['coordinate'].size(-2) == "
                "label_full_dict['coordinate'].size(-2)"
            )
            raise ValueError(message)
    ...
    if permute_label:
        # Permute groundtruth coord and coord mask with indices, along the first
        # dimension.
...
class MultiChainPermutation:
    ...
    def process_input(
        ...
        Args:
            pred_dict (dict[str, torch.Tensor]): A dictionary containing
                entity_mol_id, mol_id, mol_atom_index, pae_rep_atom_mask, coordinate,
                is_ligand.
            ...
            max_num_chains (int): if the number of chains is more than this number,
                than skip permutation to
                avoid expensive computations.
            permute_label (bool): if true, permute the groundtruth chains, otherwise
                premute the prediction chains
        """
        ...
        if pred_mol_id.intersection(label_mol_id) != pred_mol_id:
            ...
            if pred_dict["coordinate"].size(-2) != label_full_dict["coordinate"].size(
                -2
            ):
                message = (
                    "Invalid state: pred_dict['coordinate'].size(-2) == "
        ...
        if n_label_chain > 20:
            logger.warning(
                "The label_full_dict contains %s asym chains.", f"{n_label_chain}"
            )

        if max_num_chains > 0 and n_label_chain > max_num_chains:
            logger.warning(
                (
                    "The label_full_dict contains %s asym chains (max_num_chains: "
                    "%s). Will skip chain permutation and keep the original chain "
                    "assignment."
                ),
                f"{n_label_chain}",
                f"{max_num_chains}",
    ...
    @staticmethod
    def check_pattern_and_create_mapping(
        mol_id1: torch.Tensor, mol_id2: torch.Tensor
    ) -> dict:
        """Check pattern and create mapping.

        Check if the patterns between two mol_id tensors match and create a mapping
        between them.
        ...
        if mol_id1.shape != mol_id2.shape:
            msg = "mol_id1 and mol_id2 must have the same shape"
            raise ValueError(msg)

        pattern_mapping = {}
        for id1, id2 in zip(mol_id1.tolist(), mol_id2.tolist(), strict=False):
            if id1 in pattern_mapping:
                if pattern_mapping[id1] != id2:
                    msg = (
                        f"Inconsistent pattern: {id1} mapped "
                        f"to different values in mol_id2"
                    )
                    raise ValueError(msg)
            else:
                if id2 in pattern_mapping.values():
                    msg = f"Value {id2} in mol_id2 already mapped to another value"
                    raise ValueError(msg)
    ...
    def _parse_atom_feature_dict(
        ...
        """Parse atom feature dict.

        Parse the atom feature dictionary and convert it to token features and per-asym
        token features.
        ...
        Returns:
            tuple: A tuple containing:
                - token_dict (dict): A dictionary containing the token features
                corresponding to the rep atoms.
    ...
    def _convert_to_token_dict(
        ...
        Convert the atom feature dictionary to a token feature dictionary based on the
        rep atom mask.
        ...
        Returns:
            dict[str, torch.Tensor]: A dictionary containing the token features
            corresponding to the rep atoms.
        """
        rep_atom_mask = rep_atom_mask.bool()
    ...
    @staticmethod
    def _convert_to_per_asym_feature_dict(
        asym_id: torch.Tensor, feature_dict: dict
    ) -> dict:
        """Convert to per asym feature dict.
        ...
        Returns:
            dict: A dictionary where keys are asym IDs and values are dictionaries of
                features corresponding to each asym.
        """
        out = {}
    ...
    def _get_entity_asym_mapping(
        ...
        Returns:
            tuple[dict]: A tuple containing two dictionaries:
                - entity_to_asym: A dictionary mapping entity IDs to their
                corresponding asym IDs.
                - asym_to_entity: A dictionary mapping asym IDs to their corresponding
                entity IDs.
        """
        entity_to_asym = {}
        asym_to_entity = {}
        for raw_ein in torch.unique(entity_id):
            ein = raw_ein
            ein = ein.item()
    ...
    def find_anchor_asym_chain_in_predictions(self) -> int:
        ...
        with the hope that in general the longer asyms are likely to have higher
        confident predictions.
        The prediction anchor asym is chosen from the set {a^pred_m} of all prediction
        asyms
        ...
        candidate_entities = {ent for ent, _ in valid_entity_asym}
    ...
    @staticmethod
    def _select_atoms_by_mol_atom_index(
        input_dict: dict, mol_atom_index: torch.Tensor
    ) -> dict:
        """Select atoms by mol atom index.
        ...
        if not ((out_dict["mol_atom_index"] == mol_atom_index).all()):
            message = (
                "Invalid state: (out_dict['mol_atom_index'] == mol_atom_index).all()"
            )
            raise ValueError(message)
        return out_dict

    def compute_best_match_heuristic(self) -> dict[int, int]:
        """Compute the best chain permutation between prediction and groundtruth.


        Returns:
            dict[int, int]: A dictionary mapping pred chain IDs to those of the
            groundtruth.
        """
        ...
        anchor_gt_asym_id: int | None = None
        if self.find_gt_anchor_first:
            # Randomly sample a groundtruth asym chain using this entity id
            anchor_gt_candidates = self.label_token_dict["entity_to_asym"][
                anchor_entity_id
            ].tolist()
            anchor_gt_asym_id = random.choice(anchor_gt_candidates)
        ...
        for raw_anchor_k in candidate_anchors:
            anchor_k = raw_anchor_k
            anchor_k = anchor_k.item()

            if anchor_gt_asym_id is not None:
                gt_anchor, pred_anchor = anchor_gt_asym_id, anchor_k
            else:
                gt_anchor, pred_anchor = anchor_k, anchor_pred_asym_id

            # Find atoms in GT chain to match atoms in predicted chain (which could be
            # cropped)
            ...
            # Sort the remaining chains by their length, so that longer chain chooses
            # its match first.
            ...
            if len(matched_asym) != len(self.pred_asym_dict):
                message = "Invalid state: len(matched_asym) == len(self.pred_asym_dict)"
                raise ValueError(message)
        ...
        if not (best_match is not None):
            message = "Invalid state: best_match is not None"
            raise ValueError(message)
    ...
    def _calculate_rmsd(self, pred_asym_id: int, gt_asym_id: int) -> float:
        """Calculate rmsd.

        Calculate the RMSD between the predicted and ground truth chains, either using
        the average of the representative atoms or all of them.
    ...
    def match_pred_asym_to_gt_asym(self, pred_asym_id: int, gt_asym_ids: list) -> tuple:
        """Compute match pred asym to gt asym.

        Match a predicted  chain to the groundtruth chain based on the average of the
        representative atoms.

        Args:
            pred_asym_id (int): The ID of the predicted asymmetric chain.
            gt_asym_ids (list[int]): A list or tensor of ground truth asymmetric chain
                IDs.

        Returns:
            tuple: A tuple containing:
                - best_gt_asym_id (int): The ID of the best matched ground truth
                asymmetric chain.
                - best_error (float): The distance error between the centers of mass of
                the best matched chains.
        """
        pred_asym_dict = self.pred_asym_dict[pred_asym_id]
        ...
        for raw_gt_asym_id in gt_asym_ids:
            gt_asym_id = raw_gt_asym_id
        ...
        if best_gt_asym_id is None:
            ...
            if not (len(unresolved_gt_asym_id) > 0):
                message = "Invalid state: len(unresolved_gt_asym_id) > 0"
                raise ValueError(message)
    ...
    def build_permuted_indice(
        ...
        Args:
            ...
            label_full_dict (dict): A dictionary containing the true coordinates and
                their masks.
            best_match (dict[int, int]): {pred_mol_id: gt_mol_id} best match between
                pred asym chains and gt asym chains
        ...
        if len(torch.unique(indices)) != len(indices):
            message = "Invalid state: len(torch.unique(indices)) == len(indices)"
            raise ValueError(message)
        return indices
    ...
    def __call__(
        ...
        Args:
            pred_dict (dict): A dictionary containing the predicted coordinates.
            label_full_dict (dict): A dictionary containing the groundtruth and its
                attributes.
            max_num_chains (int): Maximum number of chains allowed.

        Returns:
            tuple: A tuple containing:
                - best_match (dict[int, int]): The best match between predicted and
                groundtruth chains.
                - permute_pred_indices (torch.Tensor or None): Indices to permute the
                predicted coordinates.
                - permuted_indices (torch.Tensor): Indices to permute the groundtruth
                coordinates.
                - log_dict (dict): A dictionary detailing the permutation information.
        """
        match, _has_sym_chain = self.process_input(
