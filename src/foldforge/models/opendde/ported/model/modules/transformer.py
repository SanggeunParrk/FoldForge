...
from foldforge.models.opendde.ported.distributed.foldcp.atom_window import (
...
class DiffusionTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        """
        out = conditioned_residual(
            a, s,
            attention=lambda value: self.attention_pair_bias(
                a=value, s=s, z=z, n_queries=n_queries, n_keys=n_keys,
                inplace_safe=inplace_safe, chunk_size=chunk_size,
                enable_efficient_fusion=enable_efficient_fusion, extra_attn_bias=extra_attn_bias),
            transition=self.conditioned_transition_block,
    ...
    def forward_foldcp_local_z(
        ...
        out = conditioned_residual(
            a, s,
            attention=lambda value: self.attention_pair_bias.forward_foldcp_local_z(
                a=value, s=s, z_local=z_local, z_spec=z_spec, mesh=mesh,
                extra_attn_bias=extra_attn_bias, inplace_safe=inplace_safe,
                enable_efficient_fusion=enable_efficient_fusion,
                projected_bias_local=projected_bias_local, workspace=workspace),
            transition=self.conditioned_transition_block,
            residual_path=self.residual_path,
        )
        return out, s, z_local
    ...
    def forward_foldcp_window(
        ...
        out = conditioned_residual(
            a, s,
            attention=lambda value: self.attention_pair_bias.forward_foldcp_window(
                a=value, s=s, z_local=z_local, window_spec=window_spec,
                mesh=mesh, inplace_safe=inplace_safe),
            transition=self.conditioned_transition_block,
            residual_path=self.residual_path,
        )
        return out, s, z_local
