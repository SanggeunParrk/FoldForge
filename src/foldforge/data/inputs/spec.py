# Adapted from MiniWorld data/inputs/spec.py; same public format.
...
class RefinementSpec(BaseModel):
    ...
    def _check_chain_map(self) -> RefinementSpec:
        ...
        if self.start_sigma_y <= 0.0:
            msg = (
                f"RefinementSpec.start_sigma_y must be "
                f"positive, got {self.start_sigma_y}."
            )
            raise ValueError(msg)
        return self
...
class InferenceSpec(BaseModel):
    ...
    Fields:
      name: optional run identifier (used for output filenames). Defaults to the spec
          stem.
    ...
    def msa_pairing_mode(self) -> str:
        """Compute msa pairing mode.

        Resolved pairing mode for :class:`ComplexMSA`: ``mixed`` / ``paired_only`` /
        ``no_pairing``.
