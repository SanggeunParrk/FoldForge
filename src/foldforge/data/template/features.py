...
from foldforge.data.io import compat_pickle_load
from foldforge.data.msa.kalign import Kalign
from foldforge.data.template.parser import (
    AlignRatioError,
    CaDistanceError,
    DateError,
    DuplicateError,
    ...
    ResiduePosition,
    ResidueTemplateParser,
    SingleHitResult,
    StructuralTemplateParser,
...
RESIDUE__POLYMER_FEATURES: Final[Mapping[str, np.float64 | np.int32 | object]] = {
...
class ResidueTemplateFeatures:
    ...
    def package_template_features(
        *, hit_features: Sequence[Mapping[str, Any]]
        ...
        features_to_include = set(RESIDUE__POLYMER_FEATURES)
        features = {
        ...
        stacked_features = {}
        for k, raw_v in features.items():
            v = raw_v
            if k in RESIDUE__POLYMER_FEATURES:
                v = (
                    np.stack(v, axis=0)
                    if v
                    else np.array([], dtype=RESIDUE__POLYMER_FEATURES[k])
                )
    ...
    def fix_template_features(
        ...
        if not template_features["template_aatype"].shape[0]:
            template_features = ResidueTemplateFeatures.empty_template_features(num_res)
        else:
            template_release_timestamp = [
                ResidueTemplateFeatures.get_timestamp(x.decode("utf-8"))
    ...
    def empty_template_features(num_res: int, num_dense: int = 24) -> FeatureDict:
        """Create fully masked out template features."""
        return {
    ...
    def dgram_from_positions(
        ...
        if cache_key not in ResidueTemplateFeatures._dgram_cache:
            lower = np.linspace(
            ...
            upper[:-1] = lower[1:]
            upper[-1] = 1e8
            ResidueTemplateFeatures._dgram_cache[cache_key] = (lower, upper)
        lower_breaks, upper_breaks = ResidueTemplateFeatures._dgram_cache[cache_key]
    ...
    def compute_template_unit_vector(
        ...
        # Local frame: CA origin, C-CA is x-axis
        ...
        rotation = np.stack([e1, e2, e3], axis=-1)  # [num_res, 3, 3]
        diff = ca_pos[np.newaxis, :, :] - ca_pos[:, np.newaxis, :]
        unit_vector = np.einsum("ilk,ijl->ijk", rotation, diff)

        uv_norm = (
            np.sqrt(np.einsum("ijk,ijk->ij", unit_vector, unit_vector))[..., np.newaxis]
            + epsilon
        )
...
class TemplateHitFilter:
    ...
    def _is_after_cutoff(self, pdb_id: str, cutoff: datetime | None) -> bool:
        """Check if a PDB entry was released after the cutoff date."""
        if cutoff is None:
            return False
    ...
    def _assess_hit(
        ...
        """Perform quick pre-filtering on a hit."""
        ...
        if self._is_after_cutoff(pdb_code, cutoff):
            msg = f"Release date for {pdb_code} is after cutoff."
            raise DateError(msg)
        if align_ratio <= min_align_ratio:
            msg = f"Align ratio {align_ratio:.2f} <= {min_align_ratio}"
            raise AlignRatioError(msg)
        if t_seq in query_seq and len_ratio > max_subseq_ratio:
            msg = "Hit is a large duplicate of the query."
            raise DuplicateError(msg)
        if len(t_seq) < 10:
            msg = "Template sequence too short."
    ...
    def prefilter(
        ...
        try:
            ...
            return PrefilterResult(valid=False, error=str(e))

        if pdb_id not in self.release_dates:
            pdb_id = self.obsolete_pdbs.get(pdb_id, pdb_id)
            if pdb_id not in self.release_dates:
                return PrefilterResult(valid=False, error=f"{pdb_id} date unknown.")
        ...
        try:
            ...
            if self.strict and isinstance(e, (DateError, DuplicateError)):
                return PrefilterResult(valid=False, error=msg)
            return PrefilterResult(valid=False, warning=msg)
        return PrefilterResult(valid=True)


class ResidueTemplateHitProcessor:
    """Process a template hit to generate features."""
    ...
    def _fetch_or_read_cif(self, pdb_id: str) -> str:
        """Compute fetch or read cif.

        Read mmCIF from local dir, falling back to PDBe API if fetch_remote is enabled.
        """
        cif_path = str(Path(self._mmcif_dir) / (f"{pdb_id}.cif"))
        if Path(cif_path).exists():
            return self._read_file(cif_path)

        if not self._fetch_remote:
            msg = f"CIF not found: {cif_path}"
            raise FileNotFoundError(msg)

        url = self._PDBE_CIF_URL.format(pdb_id=pdb_id.lower())
        logger.info("Fetching mmCIF for %s from %s", f"{pdb_id}", f"{url}")
        response = requests.get(url, timeout=30)
        response.raise_for_status()
        cif_str = response.text

        Path(self._mmcif_dir).mkdir(parents=True, exist_ok=True)
        with Path(cif_path).open("w") as f:
            f.write(cif_str)
        logger.info("Cached mmCIF for %s at %s", f"{pdb_id}", f"{cif_path}")
    ...
    def _get_atom_coords(
        ...
        *,
        ...
        """Extract all-atom coordinates and mask for a specific chain."""
        chains = list(mmcif_object.structure.get_chains())
        relevant_chains = [c for c in chains if c.id == chain_id]
        if len(relevant_chains) != 1:
            msg = f"Expected 1 chain with id {chain_id}, found {len(relevant_chains)}"
            raise MultipleChainsError(msg)
        chain = cast("Chain", relevant_chains[0])
        ...
        for i in range(num_res):
            ...
            if not res_info.is_missing and res_info.position is not None:
                try:
                    res = chain[
                        (
                            res_info.hetflag,
                    ...
                    if (
                        res.get_resname() == "ARG" and all(all_mask[i, [cd, nh1, nh2]])
                    ) and (
                        np.linalg.norm(all_pos[i, nh1] - all_pos[i, cd])
    ...
    def _check_residue_distances(
        ...
        """Verify that distance between consecutive CA atoms is within limits."""
        ...
        if max_found > max_dist:
            ...
            msg = (
                f"Distance between residues at index {bad_idx} is "
                f"{max_found:.2f} > {max_dist}"
            )
            raise CaDistanceError(msg)
    ...
    def _get_atom_positions(
        ...
        *,
        ...
        pos, mask = self._get_atom_coords(
            mmcif_obj, auth_chain_id, _zero_center_positions=_zero_center
        )
    ...
    def _align_query_to_hit_index_mapping(
        ...
        """Align query to hit index mapping.

        Aligns the query sequence directly to the chain sequence in mmCIF and
        establishes index mapping.

        This method ensures that each residue in the query sequence finds its
        corresponding
        ...
        if not target_seq:
            if len(mmcif_obj.chain_to_seqres) == 1:
                actual_chain_id = next(iter(mmcif_obj.chain_to_seqres.keys()))
                target_seq = mmcif_obj.chain_to_seqres[actual_chain_id]
            else:
                msg = f"Chain {chain_id} not found."
                raise QueryToTemplateAlignError(msg)

        try:
            q_aln, t_aln = aligner.align([query_seq, target_seq])
        except Exception as e:
            msg = f"Query alignment failed: {e}"
            raise QueryToTemplateAlignError(msg) from e

        mapping = {}
        q_idx, t_idx, same, count = -1, -1, 0, 0
        for qa, ta in zip(q_aln, t_aln, strict=False):
            if qa != "-":
                q_idx += 1
            if ta != "-":
                t_idx += 1
        ...
        if count > 0 and (same / count < min_ratio):
            msg = "Insufficient similarity to query."
            raise QueryToTemplateAlignError(msg)
    ...
    def _extract_template_features(
        ...
        *,
        ...
        """Generate features for a template hit."""
        if not mmcif_obj or not mmcif_obj.chain_to_seqres:
            msg = f"No chains in {pdb_id}"
            raise NoChainsError(msg)

        warning = None
        try:
            all_pos, all_mask = self._get_atom_positions(
                mmcif_obj, chain_id, 150.0, _zero_center=_zero_center
            )
        except (CaDistanceError, KeyError) as e:
            msg = f"Failed to get atom data: {e}"
            raise NoAtomDataInTemplateError(msg) from e
        ...
        if np.sum(out_mask) < 5:
            msg = f"Empty atom mask for {pdb_id}_{chain_id}"
            raise TemplateAtomMaskAllZerosError(msg)
    ...
    def _update_realigned_hit(
        ...
        """Update the TemplateHit object with new alignment information.

        This method synchronizes the hit information (alignment columns, hit sequence,
        and index mappings) after a re-alignment between the query and the template
        structure.
    ...
    def process(
        ...
        hit: TemplateHit,
        max_date: datetime,
        release_dates: Mapping[str, datetime],  # noqa: ARG002 - shared callback or fixture signature
        obsolete_pdbs: Mapping[str, str],
        *,
        strict: bool = False,
    ) -> tuple[SingleHitResult, dict[str, float]]:
        """Process a single hit to generate features."""
        track = {}
        t_start = time.time()
        pdb_id, chain_id = get_pdb_id_and_chain(hit)
        ...
        if self._template_cache_dir:
            cache_path = str(
                Path(self._template_cache_dir) / (f"{pdb_id}_{chain_id}.pkl")
            )
            if Path(cache_path).exists():
                with Path(cache_path).open("rb") as f:
                    res = pickle.load(f)  # noqa: S301 - trusted local legacy dataset
            ...
            try:
                ...
                res = ResidueTemplateParser.parse(
        ...
        hit_date = datetime.strptime(date_str, "%Y-%m-%d")  # noqa: DTZ007 - PDB calendar dates, not timestamps
        ...
        try:
            ...
            feats, warn = self._extract_template_features(
                ...
                query_seq,
                actual_chain_id,
                _zero_center=self._zero_center_positions,
            )
        ...
        except Exception as e:  # noqa: BLE001 - one malformed template must not discard other hits
            return (
...
class _CommonTemplateHitFeaturizer:
    """Represent common template hit featurizer."""
    ...
    def _parse_release_dates(self, path: str) -> dict[str, datetime]:
        ...
        with Path(path).open() as f:
            data = json.load(f)
        return {
            pdb.lower(): datetime.strptime(v["release_date"], "%Y-%m-%d")  # noqa: DTZ007 - PDB calendar dates, not timestamps
            for pdb, v in data.items()
            if "release_date" in v
        }


class ResidueTemplateHitFeaturizer(_CommonTemplateHitFeaturizer):
    ...
    def __init__(
        ...
        release_dates_path: str | None = None,
        obsolete_pdbs_path: str | None = None,
        *,
        strict_error_check: bool = False,
        _shuffle_top_k_prefiltered: int | None = None,
        _zero_center_positions: bool = True,
        _max_template_candidates_num: int | None = None,
        fetch_remote: bool = False,
    ) -> None:
        self._mmcif_dir = mmcif_dir
        self._template_cache_dir = template_cache_dir
        ...
        if max_template_date:
            if isinstance(max_template_date, str):
                self._max_template_date = datetime.strptime(  # noqa: DTZ007 - PDB calendar dates, not timestamps
        ...
        if obsolete_pdbs_path and Path(obsolete_pdbs_path).exists():
            if obsolete_pdbs_path not in _OBSOLETE_PDBS_CACHE:
                with Path(obsolete_pdbs_path).open() as f:
        ...
        self._hit_processor = ResidueTemplateHitProcessor(
    ...
    def parse_json_templates(
        self, template_list: list[dict], query_sequence: str
    ) -> TemplateSearchResult:
        ...
        for idx, template_info in enumerate(template_list):
            ...
            mapping = dict(zip(q_indices, t_indices, strict=False))

            try:
                res = ResidueTemplateParser.parse_simple_cif(
                    file_id=f"temp_{idx}", mmcif_string=mmcif_str
                )
                if not res.mmcif_object:
                    errors.append(
                        f"No mmcif object generated for "
                        f"index {idx}. "
                        f"Errors: {res.errors}"
                    )
                    continue
                ...
                all_pos, all_mask = self._hit_processor._get_atom_positions(  # noqa: SLF001 - Biotite extension hook
                    res.mmcif_object,
                    chain_id,
                    150.0,
                    _zero_center=self._zero_center_positions,
            ...
            for q_idx, t_idx in mapping.items():
                if t_idx != -1 and t_idx < len(all_pos) and q_idx < num_query:
                    ...
                    out_mask[q_idx] = all_mask[t_idx]
                    out_seq[q_idx] = (
                        template_seq[t_idx] if t_idx < len(template_seq) else "-"
                    )
            ...
            features = {
                ...
                "template_sequence": out_seq_str.encode(),
                "template_aatype": np.array(aatype, dtype=np.int32),
                "template_domain_names": np.array(f"temp_{idx}".encode(), dtype=object),
    ...
    def get_templates(
        self,
        sequence_uid: str,  # noqa: ARG002 - shared callback or fixture signature
        query_sequence: str,
        hits: Sequence[TemplateHit],
        max_template_date: str | datetime | None = None,
    ) -> tuple[TemplateSearchResult, dict[str, float]]:
        """Process hits to generate template features.
        ...
        if max_template_date:
            if isinstance(max_template_date, str):
                cutoff = datetime.strptime(max_template_date, "%Y-%m-%d")  # noqa: DTZ007 - PDB calendar dates, not timestamps
            else:
                cutoff = max_template_date

        if cutoff is None:
            message = "Template search requires a maximum release date"
...
STRUCTURAL__POLYMER_FEATURES: Final[Mapping[str, DTypeLike]] = {
    "template_aatype": np.int32,
    "template_all_atom_masks": np.float64,
    "template_all_atom_positions": np.float64,
    "template_domain_names": object,
...
class StructuralTemplateFeatures:
    ...
    def package_template_features(
        *, hit_features: Sequence[Mapping[str, Any]]
        ...
        """Stacks polymer features, adds empty and keeps ligand features unstacked."""
        features_to_include = set(STRUCTURAL__POLYMER_FEATURES)
        features = {
            feat: [single_hit_features[feat] for single_hit_features in hit_features]
            for feat in features_to_include
        }
        ...
        for k, raw_v in features.items():
            v = raw_v
            if k in STRUCTURAL__POLYMER_FEATURES:
                v = (
                    np.stack(v, axis=0)
                    if v
                    else np.array([], dtype=STRUCTURAL__POLYMER_FEATURES[k])
                )
            stacked_features[k] = v
    ...
    def fix_template_features(
        ...
        """Convert template features to AlphaFold 3 format."""
        if not template_features["template_aatype"].shape[0]:
            template_features = StructuralTemplateFeatures.empty_template_features(
                num_res
            )
        else:
            template_release_timestamp = [
                StructuralTemplateFeatures.get_timestamp(x.decode("utf-8"))
                for x in template_features["template_release_date"]
            ]
    ...
    def dgram_from_positions(
        ...
        """Compute distogram from amino acid positions."""
        cache_key = (config.min_bin, config.max_bin, config.num_bins)
        if cache_key not in StructuralTemplateFeatures._dgram_cache:
            lower = np.linspace(
                config.min_bin, config.max_bin, config.num_bins, dtype=np.float32
            )
            lower = np.square(lower)
            ...
            upper[:-1] = lower[1:]
            upper[-1] = 1e8
            StructuralTemplateFeatures._dgram_cache[cache_key] = (lower, upper)
        lower_breaks, upper_breaks = StructuralTemplateFeatures._dgram_cache[cache_key]
    ...
    def compute_template_unit_vector(
        ...
        uv_norm = np.sqrt(np.einsum("ijk,ijk->ij", unit_vector, unit_vector))[
            ..., np.newaxis
        ]
...
class StructuralTemplateHitProcessor:
    """Process a template hit to generate features."""
    ...
    def _get_atom_coords(
        ...
        *,
        ...
        for i in range(num_res):
            ...
            if not res_info.is_missing and res_info.position is not None:
                try:
                    position = cast("ResiduePosition", res_info.position)
                    res = chain[
                        (
                            res_info.hetflag,
                            position.residue_number,
                            position.insertion_code,
                        )
                    ]
                    res = cast("Residue", res)
                    for atom in res.get_atoms():
    ...
    def _check_residue_distances(
        ...
        diffs = ca_pos[1:] - ca_pos[:-1]
        dists = np.sqrt(np.einsum("ij,ij->i", diffs, diffs))
        max_found = np.nanmax(dists)
        if max_found > max_dist:
            valid_indices = np.where(ca_mask)[0]
            bad_pos = int(np.nanargmax(dists))
            idx_a = int(valid_indices[bad_pos])
            idx_b = int(valid_indices[bad_pos + 1])
            msg = (
                f"Distance between residues {idx_a} and "
                f"{idx_b} is "
                f"{max_found:.2f} > {max_dist}"
            )
            raise CaDistanceError(msg)
    ...
    def _align_query_to_hit_index_mapping(
        ...
        """
        aligner = Kalign(binary_path=cast("str", self._kalign_binary_path))
        target_seq = mmcif_obj.chain_to_seqres.get(chain_id, "")
        actual_chain_id = chain_id
        if not target_seq:
            if len(mmcif_obj.chain_to_seqres) == 1:
    ...
    def process(
        ...
        *,
        ...
        if cache_path and Path(cache_path).exists():
            with Path(cache_path).open("rb") as f:
                res = compat_pickle_load(f)
            track["load"] = time.time() - t_start
        else:
            try:
                cif_str = self._fetch_or_read_cif(pdb_id)
                res = StructuralTemplateParser.parse(
                    file_id=pdb_id, mmcif_string=cif_str, auth_chain_id=chain_id
                )
                if cache_path:
                    Path(str(Path(cache_path).parent)).mkdir(
...
class StructuralTemplateHitFeaturizer(_CommonTemplateHitFeaturizer):
    """Featurizes template hits (e.g., from hmmsearch or HHSearch) into features.
    ...
    def __init__(
        ...
        *,
        ...
        self._hit_processor = StructuralTemplateHitProcessor(
            mmcif_dir=self._mmcif_dir,
            template_cache_dir=self._template_cache_dir,
            kalign_binary_path=self._kalign_binary_path,
            _zero_center_positions=self._zero_center_positions,
