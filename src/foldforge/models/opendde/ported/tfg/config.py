...
- `Term`: a wrapper around a potential implementation from
  `opendde.tfg.potentials`, with an evaluation interval, a weight schedule, and
...
from foldforge.models.opendde.ported.tfg import potentials
...
class Term:
    ...
    A term wraps:
    - a potential implementation (from `opendde.tfg.potentials`),
...
def build_terms(term_cfg: Mapping[str, Any] | None) -> list[Term]:
    ...
    This is the public term builder for TFG guidance, including schedule parsing,
    parameter templates, and potential lookup.
    """
    terms = []
    if term_cfg is None:
        return terms
...
# Backwards-compatible alias: keep the previous private name working for any
# existing importers. New code should use the public `build_terms`.
_build_terms = build_terms
...
def parse_tfg_config(guidance_cfg: Mapping[str, Any] | None) -> TFGConfig:
    ...
    terms = tuple(build_terms(term_cfg))
