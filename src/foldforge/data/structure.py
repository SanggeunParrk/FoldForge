...
from biotite.structure.io.pdbx import BinaryCIFCategory, CIFCategory
...
MAX_PERIODIC_BOX_OFFSET = 5


def _find_matches_by_dense_array(
    query_arrays: Sequence[np.ndarray], reference_arrays: Sequence[np.ndarray]
) -> np.ndarray:
    ...
    match_masks_for_all_columns = np.stack(
        [
            ...
            for query, reference in zip(query_arrays, reference_arrays, strict=False)
        ],
        axis=-1,
    )
...
def _find_matches_by_dict(
    ...
    reference_dict = {}
    ambiguous_keys = set()
    for ref_idx, ref_row in enumerate(zip(*reference_arrays, strict=False)):
    ...
    match_indices = []
    for query_idx, query_row in enumerate(zip(*query_arrays, strict=False)):
...
def _parse_inter_residue_bonds(
    atom_site: CIFCategory | BinaryCIFCategory,
    struct_conn: CIFCategory | BinaryCIFCategory,
) -> BondList:
    ...
    atom_indices: list[np.ndarray | None] = [None] * 2
    for i in range(2):
        reference_arrays = []
        query_arrays = []
        for col_name in columns:
            struct_conn_col_name = pdbx_convert._get_struct_conn_col_name(  # noqa: SLF001 - Biotite extension hook
                col_name, i + 1
            )
    ...
    if atoms_indices_1 is None or atoms_indices_2 is None:
        msg = "Missing atom indices for inter-residue bonds"
        raise InvalidFileError(msg)
...
@overload
def concatenate(atoms: Iterable[AtomArray]) -> AtomArray: ...


@overload
def concatenate(atoms: Iterable[AtomArrayStack]) -> AtomArrayStack: ...


def concatenate(  # noqa: C901, PLR0912 - preserve atom annotations, box and bond offsets together
    atoms: Iterable[AtomArray] | Iterable[AtomArrayStack],
    ...
    if not atom_list:
        message = "At least one atom array is required for concatenation"
        raise ValueError(message)

    length = 0
    depth = None
    element_type = None
    common_categories = set(atom_list[0].get_annotation_categories())
    box = None
    has_bonds = False
    for element in atom_list:
        ...
        if isinstance(element, AtomArrayStack):
            ...
            elif element.stack_depth() != depth:
                msg = "The stack depths are not equal"
    ...
    elif element_type == AtomArrayStack and depth is not None:
        concat_atoms = AtomArrayStack(depth, length)
    else:
        message = "Only AtomArray or AtomArrayStack objects can be concatenated"
        raise TypeError(message)
    concat_atoms.coord = np.concatenate(
        [element.coord for element in atom_list], axis=-2
    )
    for category in common_categories:
        concat_atoms.set_annotation(
            category,
            np.concatenate(
                [element.get_annotation(category) for element in atom_list], axis=0
            ),
        )
    concat_atoms.box = None if box is None else np.array(box.tolist())
    if has_bonds:
...
def bond_list_concatenate(bonds_lists: Iterable[BondList]) -> BondList:
    ...
    merged_bonds = np.concatenate([bond_list.as_array() for bond_list in bonds_lists])
    ...
    for bond_list in bonds_lists:
        stop = start + bond_list.as_array().shape[0]
        merged_bonds[start:stop, :2] += cum_atom_count
        cum_atom_count += bond_list.get_atom_count()
        start = stop
    ...
    return _replace_bond_array(merged_bond_list, merged_bonds)


def move_inside_box(
    coord: np.ndarray, box: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
    r"""Move all coordinates into the given box, with the box vectors.
    ...
    if not (np.all(np.abs(offset) < MAX_PERIODIC_BOX_OFFSET)):
        message = "Invalid state: np.all(np.abs(offset) < 5)"
        raise ValueError(message)
