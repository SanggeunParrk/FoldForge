...
from foldforge.models.protenix.ported.data.esm.compute_esm import compute_ESM_embeddings, load_esm_model
...
class ESMFeaturizer:
    ...
    def load_esm_embedding(self, sequence: str):
        ...
        assert x.size(0) == len(sequence), (
            ...
            + "The error occurs because embeddings were previously computed for a certain 'name', "
            + "and then a different JSON file with the same task name was used, causing ESM to fail loading the existing embeddings."
            + "You can resolve this by deleting the local esm_embeddings directory and retrying."
